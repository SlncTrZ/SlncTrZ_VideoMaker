...
(function() {
        ...
        container.innerHTML = `
          <div class="s-profile-anon">
            <div class="s-profile-anon-info">
              ...
              <div>
                <span class="s-profile-anon-label">${window.I18n?.t('settings.notLoggedIn') || 'ChÆ°a Ä‘Äƒng nháº­p'}</span>
                <span class="s-profile-storage-local">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
                  ${window.I18n?.t('settings.storageLocal') || 'LÆ°u trá»¯ local'}
                </span>
              </div>
            </div>
            <button class="s-btn s-btn-primary s-btn-sm" id="settingsLoginBtn">
              ...
              ${window.I18n?.t('auth.login') || 'ÄÄƒng nháº­p'}
            </button>
          </div>
        `;
  ...
  async function handleTelegramLink() {
    try {
      ...
      if (!auth?.token) {
        if (window.customDialog) {
          window.customDialog.alert(window.I18n?.t('settings.telegramLoginRequired') || 'Vui lÃ²ng Ä‘Äƒng nháº­p trÆ°á»›c khi liÃªn káº¿t Telegram.', { title: window.I18n?.t('settings.notLoggedIn') || 'ChÆ°a Ä‘Äƒng nháº­p', type: 'warning' });
        }
        return;
      }
  ...
  async function initAccountLinking() {
    ...
    try {
      ...
      // Kiá»ƒm tra tráº¡ng thÃ¡i liÃªn káº¿t Google
      const user = auth.user;
