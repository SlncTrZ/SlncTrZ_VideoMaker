...
(function() {
  ...
  class GenerationHistory {
    ...
    async init() {
      ...
      console.log('[SlncTrZ] GenerationHistory đã sẵn sàng (batching enabled)');
    }
    ...
    async saveRecord(record) {
      ...
      console.log('[SlncTrZ] History record buffered, queue size:', this._buffer.length);
    ...
    async _flushBuffer() {
      ...
      console.log('[SlncTrZ] Flushing', toFlush.length, 'history records');
      ...
      for (let i = 0; i < toFlush.length; i++) {
        ...
        try {
          ...
          console.warn('[SlncTrZ] History flush failed, saving to local:', e.message);
      ...
      console.log('[SlncTrZ] History flush completed');
    }
    ...
    async getHistory(page = 1, perPage = 20, isFavorite = null) {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Tải lịch sử thất bại:', e.message);
        return this._getLocal(page, perPage, isFavorite);
      }
    }
    ...
    async toggleFavorite(id) {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Toggle favorite thất bại:', e.message);
        return null;
      }
    }
    ...
    async deleteRecord(id) {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Xóa lịch sử thất bại:', e.message);
        return false;
      }
    }
    ...
    async _saveLocal(record) {
      try {
        ...
        console.error('[SlncTrZ] Lưu local thất bại:', e.message);
      }
    }
    ...
    async _getLocal(page, perPage, isFavorite = null) {
      try {
        ...
        console.error('[SlncTrZ] Đọc local thất bại:', e.message);
    ...
    async _toggleLocalFavorite(id) {
      try {
        ...
        console.error('[SlncTrZ] Toggle local favorite thất bại:', e.message);
        return null;
      }
    }
    ...
    async _deleteLocal(id) {
      try {
        ...
        console.error('[SlncTrZ] Xóa local thất bại:', e.message);
        return false;
      }
    }
  }
