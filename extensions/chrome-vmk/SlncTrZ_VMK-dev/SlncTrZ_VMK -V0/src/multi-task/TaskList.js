...
class TaskList {
  ...
  async _checkAndOpenTaskModal() {
    // Check quota limit before opening modal
    if (window.authManager?.isLoggedIn() && window.featureGate) {
      const quota = window.featureGate.checkQuota('tasks_max');
      console.log('[TaskList] Task quota check:', quota, 'tasks in list:', this.tasks?.length);
      ...
      if (!canCreate) {
        window.featureGate.showLoginPrompt(window.I18n?.t('tasks.trialExhaustedCreate') || 'Bạn đã hết lượt tạo task dùng thử. Đăng nhập để tiếp tục.');
        return;
      }
    }
    ...
    if (hasMore) {
      ...
      const loadMoreHtml = `
        <div class="slnctrz-load-more-row">
          <button class="slnctrz-load-more-btn" id="taskLoadMoreBtn">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
            ${loadMoreLabel}
            <span class="slnctrz-load-more-count">${this.tasks.length} / ${this._total}</span>
          </button>
        </div>`;
  ...
  _updateCard(cardEl, task) {
    ...
    // Update refs và results sections bằng cách re-render từ task data mới
    const newHtml = this.renderTaskCard(task);
    const temp = document.createElement('div');
    temp.innerHTML = newHtml;
    const newCard = temp.firstElementChild;
  ...
  _setupEventDelegation() {
    ...
    listContainer.addEventListener('click', (e) => {
      ...
      if (target.closest('.slnctrz-dot-menu-btn')) {
        e.stopPropagation();
        const dotMenu = target.closest('.slnctrz-dot-menu');
  ...
  async _doCloneTask(taskId) {
    ...
    // Check task limit before cloning
    if (window.authManager?.isLoggedIn() && window.featureGate) {
      const quota = window.featureGate.checkQuota('tasks_max');
      // Only block if limit is valid (> 0) and used >= limit
      if (quota.limit !== 'unlimited' && quota.limit > 0 && quota.used >= quota.limit) {
      ...
      const canCreate = await window.featureGate.canCreateTaskAsync();
      if (!canCreate) {
        window.featureGate.showLoginPrompt(window.I18n?.t('tasks.trialExhaustedClone') || 'Bạn đã tạo tối đa số task cho phép trong bản dùng thử.');
        return;
      }
    }
    ...
    return `
      <div class="task-card ${statusClass} ${isSelected ? 'selected' : ''} ${!isEnabled ? 'task-disabled' : ''} ${crossProjectClass}" data-task-id="${task.task_id}">
        ...
        <div class="task-card-info">
          ...
          <div class="task-card-meta">
            ${(() => {
              ...
              return `<span>${task.model || window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro'}</span><span>${flowRatioIcon} ${flowRatio}</span><span>x${task.quantity || 1}</span>`;
            })()}
            ${refPreviewHtml}
        ...
        <div class="task-card-actions">
          ...
          ` : `
            <div class="slnctrz-dot-menu" data-task-id="${task.task_id}">
              ...
              <div class="slnctrz-dropdown-menu hidden">
                ...
                ${['completed', 'error', 'failed'].includes(task.status) ? `
                ...
                <button class="slnctrz-dropdown-item download-btn">
                ...
                <button class="slnctrz-dropdown-item delete-btn slnctrz-dropdown-danger" ${isRunning ? 'disabled' : ''}>
  ...
  async runTask(taskId) {
    ...
    window.showNotification?.(window.I18n?.t('tasks.running', { name: task.task_name || 'Task' }) || `Đang chạy: ${task.task_name || 'Task'}`, 'success', 2000);
    if (window.eventBus) {
