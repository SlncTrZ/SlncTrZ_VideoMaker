...
class DiagramCanvas {
    ...
    this.container.innerHTML = `
      <div class="diagram-container">
        ...
        <div class="canvas-brand-zone">
          <div class="slnctrz-header-brand">
            <div class="slnctrz-header-logo">
              <img src="icons/icon-32.png" alt="SlncTrZ VM">
            </div>
            <span class="slnctrz-header-title">SlncTrZ VM</span>
            <span class="slnctrz-user-plan-badge hidden" id="canvasPlanBadge" data-plan="free" title="${window.I18n?.t('settings.planBadgeTooltip') || 'Gói & quota'}">Free</span>
          </div>
        </div>
      </div>
    `;
  ...
  bindEvents() {
    ...
    canvasContainer.addEventListener('wheel', (e) => {
      ...
      if (e.target.closest('input, textarea, select, .df-node-inline-dropdown, .slnctrz-node-picker')) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  addNode(type, posX = 100, posY = 100, data = {}, options = {}) {
    ...
    if (!options.skipQuotaCheck) {
      ...
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        ...
        if (quota.limit !== 'unlimited' && currentNodeCount >= quota.limit) {
          const dialog = window.customDialog || window.CustomDialog;
          if (dialog) {
            dialog.confirm(
              window.I18n?.t('node.nodeQuotaExhausted', { limit: quota.limit }) || `Your plan is limited to ${quota.limit} nodes per workflow. Upgrade to Premium for unlimited nodes.`,
              { title: window.I18n?.t('workflow.quotaReached') || 'Limit reached', type: 'warning', confirmText: window.I18n?.t('common.upgrade') || 'Upgrade', cancelText: window.I18n?.t('common.cancel') || 'Cancel' }
            ).then((shouldUpgrade) => {
              if (shouldUpgrade) {
                // In popup window: send message to sidePanel to open upgrade modal
                // In sidePanel: call openUpgradeModal directly
      ...
      // For trial users (not logged in): check FeatureGate
      else if (window.featureGate && !window.featureGate.canAddNode(currentNodeCount)) {
        window.featureGate.showLoginPrompt(window.I18n?.t('node.trialNodeLimit', { limit: window.featureGate.getConfig().workflows_max_node }) || `Bản dùng thử giới hạn tối đa ${window.featureGate.getConfig().workflows_max_node} node mỗi workflow.`);
        return null;
      }
    }

    const config = NodeTemplates.types[type] || NodeTemplates.types.generate;
  ...
  exportWorkflow() {
    ...
    Object.entries(homeData).forEach(([id, node]) => {
      ...
      const nodeData = {
        ...
        // Group C: ModelRegistry-aware fallback. Image default vì media_type default = 'Image'.
        model: d.model || window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro',
        ratio: d.ratio || 'Ngang',
      ...
      if (d.delay_seconds) nodeData.delay_seconds = d.delay_seconds;
  ...
  duplicateNode(nodeId) {
    ...
    const newData = {
      ...
      node_name: (node.data.node_name || nodeType) + ' ' + (window.I18n?.t('common.copySuffix') || '(bản sao)'),
  ...
  fitToScreen() {
    ...
    //     ├── .slnctrz-wf-toolbar { position:absolute, left:8 }   ← overlay đè lên canvas
    ...
    try {
      const searchRoot = this.container.closest('.workflow-editor-center')
        ...
        || document;
      const toolbar = searchRoot.querySelector?.('.slnctrz-wf-toolbar');
      if (toolbar) {
        toolbarFound = true;
  ...
  // WK-1.3.6: Color SVG path theo port type
  _colorEdgeByType(connection, portType) {
    if (!connection || !portType) return;
    const color = (NodeTemplates?.PORT_TYPES?.[portType]?.color) || '#6b7280';
  ...
  _showConnectionDeleteBtn(conn) {
    ...
    const r = 12;
    g.innerHTML = `<circle cx="${midPt.x}" cy="${midPt.y}" r="${r}" /><g class="delete-icon" transform="translate(${midPt.x - 6}, ${midPt.y - 7}) scale(0.55)"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></g>`;
  ...
  _showNodeContextMenu(clientX, clientY, nodeId) {
    ...
    const items = [];
    // EWT-11: Ẩn run-node khi template mode
    ...
    if (!isStart) {
      items.push({ action: 'copy-node', label: t('node.duplicate', 'Nhân bản'), shortcut: '⌘D', icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>' });
