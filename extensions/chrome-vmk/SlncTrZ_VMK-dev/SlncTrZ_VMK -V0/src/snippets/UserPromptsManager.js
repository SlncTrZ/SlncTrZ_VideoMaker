...
(function() {
  ...
  class UserPromptsManager {
    ...
    async init() {
      ...
      console.log('[SlncTrZ] UserPromptsManager đã sẵn sàng');
    }
    ...
    async loadPrompts(append = false) {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Tải snippets thất bại:', e.message);
        if (!append) {
          await this._loadLocal();
        }
      } finally {
    ...
    async savePrompt(promptData) {
      ...
      try {
        ...
        console.log('[SlncTrZ] Đã lưu snippet:', saved.title);
        return saved;
      } catch (e) {
        console.warn('[SlncTrZ] Lưu snippet thất bại:', e.message);
    ...
    async updatePrompt(id, promptData) {
      ...
      try {
        ...
        console.log('[SlncTrZ] Đã cập nhật snippet:', updated.title);
        return updated;
      } catch (e) {
        console.warn('[SlncTrZ] Cập nhật snippet thất bại:', e.message);
        return null;
      }
    }
    ...
    async deletePrompt(id) {
      ...
      try {
        ...
        this.prompts = this.prompts.filter(p => String(p.id) !== idStr);
        console.log('[SlncTrZ] Đã xóa snippet:', id);
        return true;
      } catch (e) {
        console.warn('[SlncTrZ] Xóa snippet thất bại:', e.message);
        return false;
      }
    }
    ...
    async _loadLocal() {
      try {
        ...
        console.error('[SlncTrZ] Đọc local snippets thất bại:', e.message);
        this.prompts = [];
      }
    }
    ...
    async _saveLocal() {
      try {
        ...
        console.error('[SlncTrZ] Lưu local snippets thất bại:', e.message);
      }
    }
  }
