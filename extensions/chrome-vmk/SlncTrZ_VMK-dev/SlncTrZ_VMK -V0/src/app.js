...
function hideLoadingOverlay() {
  ...
  function setupContactModal() {
    ...
    if (contactGuideLink) {
      const apiBase = window.authManager?.apiBaseUrl || 'https://labs.toby.vn/api/v1';
  ...
  function setupNotificationBell() {
    ...
    const headerActions = document.querySelector('.slnctrz-header-actions');
    if (!headerActions) {
      console.warn('[NotificationBell] Không tìm thấy .slnctrz-header-actions');
      return;
    }
    ...
    // Chèn trước slnctrz-header-user (login button area)
    const headerUser = headerActions.querySelector('.slnctrz-header-user');
    if (headerUser) {
