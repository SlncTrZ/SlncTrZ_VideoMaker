...
class PromptQueue {
  ...
  _checkJobDone(job) {
    ...
    if (window.ExecutionGate && job._executionToken) {
      const gateStatus = job.failedCount > 0 && job.completedCount === 0 ? 'failed' : 'success';
      ExecutionGate.complete(job._executionToken, gateStatus);
      job._executionToken = null;
    }
  ...
  /** Đọc settings từ StorageSettings */
  _readSettings() {
    const settings = window.storageSettings?.getSettings?.() || {};

    const batchSize = settings.queueBatchSize ?? 4;
    const maxMonitor = settings.queueMaxMonitor ?? 4;
    const restMin = settings.queueRestMin ?? 5;
    const restMax = settings.queueRestMax ?? 15;
    const execTimeout = settings.execTimeout ?? 180;
    let maxRetries = settings.execMaxRetries ?? 2;
  ...
  // ================================================================
  // FAR helpers — đọc setting từ StorageSettings (popup window) hoặc DOM (sidebar)
  // ================================================================

  /**
   * Đọc 1 setting cho FAR features (session refresh, recovery, rate-limit, backoff).
   * Ưu tiên StorageSettings cache > DOM input > fallback default.
  ...
  _readFarSetting(key, fallback) {
    const cached = window.storageSettings?.getSettings?.() || {};
