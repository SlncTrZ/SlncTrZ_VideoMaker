...
(function() {
  ...
  class BatchQueue {
    ...
    add(item) {
      ...
      console.log('[SlncTrZ] Queue: added item', queueItem.name);
      return queueItem;
    }
    ...
    remove(index) {
      ...
      if (this.isRunning && index === this.currentIndex) {
        console.log('[SlncTrZ] Queue: cannot remove running item');
        return;
      }
      ...
      window.eventBus.emit('queue:changed', { queue: this.queue });
      console.log('[SlncTrZ] Queue: removed item', removed.name);
    }
    ...
    async start() {
      if (this.isRunning) {
        console.log('[SlncTrZ] Queue: already running');
        return;
      }
      if (this.queue.length === 0) {
        console.log('[SlncTrZ] Queue: empty, nothing to run');
        return;
      }
      ...
      console.log('[SlncTrZ] Queue: started, total items:', this.queue.length);
    ...
    pause() {
      ...
      console.log('[SlncTrZ] Queue: paused');
    }
    ...
    async resume() {
      ...
      console.log('[SlncTrZ] Queue: resumed');
      await this._runNext();
    }
    ...
    stop() {
      ...
      window.eventBus.emit('queue:changed', { queue: this.queue });
      console.log('[SlncTrZ] Queue: stopped');
    }
    ...
    clear() {
      ...
      this._completedCount = 0;
      window.eventBus.emit('queue:changed', { queue: this.queue });
      console.log('[SlncTrZ] Queue: cleared');
    }
    ...
    async _runNext() {
      ...
      if (this.currentIndex >= this.queue.length) {
        ...
        console.log('[SlncTrZ] Queue: all items completed');
        return;
      }
      ...
      try {
        ...
        console.error('[SlncTrZ] Queue: item failed', item.name, err);
      }
    ...
    async _runPrompt(data) {
      ...
      // [Fix] Inject timing settings nếu chưa có trong data
      const settings = window.storageSettings?.getSettings?.() || {};
      const payload = {
        ...data,
        delayBetweenMs: data.delayBetweenMs ?? (settings.delayBetweenPrompts || 5) * 1000,
    ...
    async _runTask(data) {
      ...
      const payload = {
        ...
        delayBetweenMs: (settings.delayBetweenPrompts || 5) * 1000,
