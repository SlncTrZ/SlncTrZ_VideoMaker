...
(function() {
  ...
  function log(...args) {
    if (DEBUG_FILE_UPLOADER) console.log('[SlncTrZ:FileUploader]', ...args);
  }
      ...
      for (const id of stillMissing) {
        ...
        try {
          ...
          const fetchUrl = thumbUrl.includes('lh3.') || thumbUrl.includes('googleusercontent.com')
            ...
            : thumbUrl;
          let resp;
          // getMediaUrlRedirect URLs cần cookies labs.google → fetch qua content script
          if (fetchUrl.includes('getMediaUrlRedirect')) {
            log(`[Tầng 3] Fetching via content script (getMediaUrlRedirect) for ${id}`);
