...
class QueueMonitor {
  ...
  static DOWNLOAD_STATES = {
    PENDING:     { icon: '\u25CB', colorClass: 'slnctrz-queue-state--muted',  get label() { return window.I18n?.t('queue.downloadPending') || 'Chờ tải'; } },
    DOWNLOADING: { icon: '\u2B07', colorClass: 'slnctrz-queue-state--blue',   get label() { return window.I18n?.t('queue.downloadActive') || 'Đang tải'; } },
    COMPLETED:   { icon: '\u2713', colorClass: 'slnctrz-queue-state--green',  get label() { return window.I18n?.t('queue.downloadCompleted') || 'Xong'; } },
    FAILED:      { icon: '\u2717', colorClass: 'slnctrz-queue-state--red',    get label() { return window.I18n?.t('queue.downloadFailed') || 'Lỗi'; } },
  };

  // Realtime phase labels từ TileMonitor (override item.state khi có phase active)
  static PHASE_LABELS = {
    monitoring:     { colorClass: 'slnctrz-queue-state--yellow', get label() { return window.I18n?.t('queue.phaseMonitoring')   || 'Chờ kết quả'; } },
    fail_detected:  { colorClass: 'slnctrz-queue-state--red',    get label() { return window.I18n?.t('queue.phaseFailDetected')|| 'Phát hiện fail'; } },
    click_retry:    { colorClass: 'slnctrz-queue-state--orange', get label() { return window.I18n?.t('queue.phaseClickRetry')  || 'Đang click retry'; } },
    wait_retry:     { colorClass: 'slnctrz-queue-state--orange', get label() { return window.I18n?.t('queue.phaseWaitRetry')   || 'Đã click, chờ tile'; } },
    retry_skipped:  { colorClass: 'slnctrz-queue-state--muted',  get label() { return window.I18n?.t('queue.phaseRetrySkipped')|| 'Bỏ qua click (đã click trước)'; } },
    tier2_reload:   { colorClass: 'slnctrz-queue-state--purple', get label() { return window.I18n?.t('queue.phaseTier2Reload') || 'Reload Flow'; } },
    tier2_resubmit: { colorClass: 'slnctrz-queue-state--purple', get label() { return window.I18n?.t('queue.phaseTier2Resubmit')|| 'Submit lại'; } },
    // ChatGPT/Grok generation phases (from WorkflowExecutor)
    prompt_enhancing:   { colorClass: 'slnctrz-queue-state--blue',   get label() { return window.I18n?.t('exec.enhancingPrompt')   || 'Cải tiến prompt...'; } },
    chatgpt_generating: { colorClass: 'slnctrz-queue-state--blue',   get label() { return window.I18n?.t('exec.chatgptGenerating') || 'ChatGPT generating...'; } },
    grok_generating:    { colorClass: 'slnctrz-queue-state--blue',   get label() { return window.I18n?.t('exec.grokGenerating')    || 'Grok generating...'; } },
    completed:      { colorClass: 'slnctrz-queue-state--green',  get label() { return window.I18n?.t('queue.itemCompleted')    || 'Hoàn tất'; } },
    failed:         { colorClass: 'slnctrz-queue-state--red',    get label() { return window.I18n?.t('queue.itemFailed')       || 'Lỗi'; } },
    partial_fail:   { colorClass: 'slnctrz-queue-state--yellow', get label() { return window.I18n?.t('queue.itemPartialFail')  || 'Một phần lỗi'; } },
  };

  static ITEM_STATES = {
    PENDING:       { icon: '\u25CB', colorClass: 'slnctrz-queue-state--muted',   get label() { return window.I18n?.t('queue.itemPending') || 'Chờ'; } },
    SUBMITTING:    { icon: '\u25B6', colorClass: 'slnctrz-queue-state--blue',    get label() { return window.I18n?.t('queue.itemSubmitting') || 'Đang gửi'; } },
    SUBMITTED:     { icon: '\u25C9', colorClass: 'slnctrz-queue-state--blue-l',  get label() { return window.I18n?.t('queue.itemSubmitted') || 'Đã gửi'; } },
    MONITORING:    { icon: '\u231B', colorClass: 'slnctrz-queue-state--yellow',  get label() { return window.I18n?.t('queue.itemMonitoring') || 'Chờ kết quả'; } },
    RETRY_SUBMIT:  { icon: '\u21BB', colorClass: 'slnctrz-queue-state--orange',  get label() { return window.I18n?.t('queue.itemRetry') || 'Thử lại'; } },
    COMPLETED:     { icon: '\u2713', colorClass: 'slnctrz-queue-state--green',   get label() { return window.I18n?.t('queue.itemCompleted') || 'Hoàn tất'; } },
    PARTIAL_FAIL:  { icon: '\u26A0', colorClass: 'slnctrz-queue-state--yellow',  get label() { return window.I18n?.t('queue.itemPartialFail') || 'Một phần lỗi'; } },
    FAILED:        { icon: '\u2717', colorClass: 'slnctrz-queue-state--red',     get label() { return window.I18n?.t('queue.itemFailed') || 'Lỗi'; } },
    CANCELLED:     { icon: '\u2298', colorClass: 'slnctrz-queue-state--muted',   get label() { return window.I18n?.t('queue.itemCancelled') || 'Hủy'; } },
  };
  ...
  _createDOM() {
    ...
    this._statusCardsEl.className = 'slnctrz-queue-status-cards';
    this._statusCardsEl.innerHTML = `
      <div class="slnctrz-queue-status-card" data-executor="editor">
        <div class="slnctrz-queue-status-card__icon">
        ...
        <div class="slnctrz-queue-status-card__info">
          <span class="slnctrz-queue-status-card__label">Editor</span>
          <span class="slnctrz-queue-status-card__value" data-field="editor-state">-</span>
        </div>
      </div>
      <div class="slnctrz-queue-status-card" data-executor="tiles">
        <div class="slnctrz-queue-status-card__icon">
        ...
        <div class="slnctrz-queue-status-card__info">
          <span class="slnctrz-queue-status-card__label">Tiles</span>
          <span class="slnctrz-queue-status-card__value" data-field="tiles-state">-</span>
        </div>
      </div>
      <div class="slnctrz-queue-status-card" data-executor="download">
        <div class="slnctrz-queue-status-card__icon">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
        </div>
        <div class="slnctrz-queue-status-card__info">
          <span class="slnctrz-queue-status-card__label">Download</span>
          <span class="slnctrz-queue-status-card__value" data-field="download-state">-</span>
        </div>
      </div>
    `;
    ...
    this._jobsContainerEl.className = 'slnctrz-queue-jobs';
    this._container.appendChild(this._jobsContainerEl);

    // Download section
    this._downloadSectionEl = document.createElement('div');
    this._downloadSectionEl.className = 'slnctrz-queue-job slnctrz-queue-download-section';
    this._downloadSectionEl.style.display = 'none';
    this._downloadSectionEl.innerHTML = `
      <div class="slnctrz-queue-job__header">
        <span class="slnctrz-queue-job__owner-dot" style="background:#06b6d4"></span>
        <span class="slnctrz-queue-job__label">Auto Download</span>
        <span class="slnctrz-queue-job__badge slnctrz-queue-job__badge--running"></span>
        <span class="slnctrz-queue-job__counter"></span>
        <span class="slnctrz-queue-job__elapsed"></span>
        <button class="slnctrz-queue-job__toggle" title="${window.I18n?.t('queue.expand') || 'Mở rộng'} / ${window.I18n?.t('queue.collapse') || 'Thu nhỏ'}">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="6 9 12 15 18 9"></polyline>
          </svg>
        </button>
      </div>
      <div class="slnctrz-queue-job__progress">
        <div class="slnctrz-queue-job__progress-fill" style="background:#06b6d4"></div>
      </div>
      <div class="slnctrz-queue-job__body">
        <div class="slnctrz-queue-items slnctrz-queue-download-items"></div>
      </div>
    `;
    // Toggle accordion
    const dlHeader = this._downloadSectionEl.querySelector('.slnctrz-queue-job__header');
    dlHeader.addEventListener('click', () => {
      this._downloadSectionEl.classList.toggle('slnctrz-queue-job--expanded');
    });
    this._container.appendChild(this._downloadSectionEl);

    // Cache download section refs
    this._dlBadgeEl = this._downloadSectionEl.querySelector('.slnctrz-queue-job__badge');
    this._dlCounterEl = this._downloadSectionEl.querySelector('.slnctrz-queue-job__counter');
    this._dlProgressFill = this._downloadSectionEl.querySelector('.slnctrz-queue-job__progress-fill');
    this._dlItemsEl = this._downloadSectionEl.querySelector('.slnctrz-queue-download-items');

    // Empty state
    this._emptyStateEl = document.createElement('div');
    this._emptyStateEl.className = 'slnctrz-queue-empty';
  ...
  _executorStateText(state) {
    const map = {
      idle: window.I18n?.t('queue.executorIdle') || 'Sẵn sàng',
      running: window.I18n?.t('exec.running') || 'Đang chạy',
      paused: window.I18n?.t('common.pause') || 'Tạm dừng',
      stopped: window.I18n?.t('common.stop') || 'Dừng',
      queued: window.I18n?.t('queue.downloadPending') || 'Chờ tải',
      downloading: window.I18n?.t('queue.executorDownloading') || 'Đang tải xuống...',
    };
  ...
  _renderDownloadSection(downloadData) {
    ...
    if (this._dlBadgeEl.textContent !== stateText) {
      ...
      this._dlBadgeEl.className = 'slnctrz-queue-job__badge slnctrz-queue-job__badge--' +
    ...
    if (!this._downloadSectionEl.classList.contains('slnctrz-queue-job--expanded')) return;
  ...
  _createDownloadItemRow(dlItem) {
    ...
    el.className = 'slnctrz-queue-item';

    el.innerHTML = `
      <span class="slnctrz-queue-item__icon"></span>
      <span class="slnctrz-queue-item__prompt"></span>
      <span class="slnctrz-queue-item__state"></span>
    `;

    return {
      el,
      iconEl: el.querySelector('.slnctrz-queue-item__icon'),
      promptEl: el.querySelector('.slnctrz-queue-item__prompt'),
      stateEl: el.querySelector('.slnctrz-queue-item__state'),
      _lastState: null,
    };
  }
  ...
  _updateDownloadItemRow(ref, dlItem) {
    ...
    ref.iconEl.className = `slnctrz-queue-item__icon ${stateInfo.colorClass}`;
    ...
    ref.stateEl.className = `slnctrz-queue-item__state ${stateInfo.colorClass}`;

    const isActive = dlItem.state === 'DOWNLOADING';
    ref.el.classList.toggle('slnctrz-queue-item--active', isActive);
  }
  ...
  _createJobElement(job) {
    const el = document.createElement('div');
    el.className = 'slnctrz-queue-job';

    const ownerInfo = QueueMonitor.OWNER_COLORS[job.owner] || { bg: '#6b7280', label: job.owner };

    // Header
    const headerEl = document.createElement('div');
    headerEl.className = 'slnctrz-queue-job__header';
    headerEl.innerHTML = `
      <span class="slnctrz-queue-job__owner-dot" style="background:${ownerInfo.bg}"></span>
      <span class="slnctrz-queue-job__label"></span>
      <span class="slnctrz-queue-job__badge"></span>
      <span class="slnctrz-queue-job__counter"></span>
      <span class="slnctrz-queue-job__elapsed"></span>
      <button class="slnctrz-queue-job__pause" title="${window.I18n?.t('common.pause') || 'Tạm dừng'}" style="display:none;">
      ...
      <button class="slnctrz-queue-job__stop" title="${window.I18n?.t('common.stop') || 'Dừng'}">
    ...
    const pauseBtn = headerEl.querySelector('.slnctrz-queue-job__pause');
    const stopBtn = headerEl.querySelector('.slnctrz-queue-job__stop');
    ...
    progressEl.className = 'slnctrz-queue-job__progress';
    progressEl.innerHTML = '<div class="slnctrz-queue-job__progress-fill"></div>';
    el.appendChild(progressEl);

    // Body (collapsible)
    const bodyEl = document.createElement('div');
    bodyEl.className = 'slnctrz-queue-job__body';
    el.appendChild(bodyEl);

    // Item table container
    const itemsEl = document.createElement('div');
    itemsEl.className = 'slnctrz-queue-items';
    ...
    headerEl.addEventListener('click', () => {
      expanded = !expanded;
      el.classList.toggle('slnctrz-queue-job--expanded', expanded);
    });
    ...
    const refs = {
      ...
      bodyEl,
      progressEl,
      progressFill: progressEl.querySelector('.slnctrz-queue-job__progress-fill'),
      labelEl: headerEl.querySelector('.slnctrz-queue-job__label'),
      badgeEl: headerEl.querySelector('.slnctrz-queue-job__badge'),
      counterEl: headerEl.querySelector('.slnctrz-queue-job__counter'),
      elapsedEl: headerEl.querySelector('.slnctrz-queue-job__elapsed'),
      pauseBtn: headerEl.querySelector('.slnctrz-queue-job__pause'),
      stopBtn: headerEl.querySelector('.slnctrz-queue-job__stop'),
      itemsEl,
      itemEls: new Map(),
      _lastStatus: null,
      _lastPct: -1,
  ...
  _updateJobElement(refs, job) {
    ...
    if (refs._lastStatus !== effectiveStatus) {
      ...
      refs.badgeEl.className = 'slnctrz-queue-job__badge slnctrz-queue-job__badge--' + effectiveStatus;
      ...
      if (refs.pauseBtn) {
        ...
        if (job.status === 'paused') {
          ...
          refs.pauseBtn.className = 'slnctrz-queue-job__pause slnctrz-queue-job__pause--resume';
          ...
          refs.pauseBtn.className = 'slnctrz-queue-job__pause';
    ...
    if (refs.el.classList.contains('slnctrz-queue-job--expanded')) {
  ...
  _createItemRow(item) {
    ...
    el.innerHTML = `
      <span class="slnctrz-queue-item__icon"></span>
      <span class="slnctrz-queue-item__index"></span>
      <span class="slnctrz-queue-item__prompt"></span>
      <span class="slnctrz-queue-item__retry"></span>
      <span class="slnctrz-queue-item__time"></span>
      <span class="slnctrz-queue-item__state"></span>
    `;
    ...
    return {
      el,
      iconEl: el.querySelector('.slnctrz-queue-item__icon'),
      indexEl: el.querySelector('.slnctrz-queue-item__index'),
      promptEl: el.querySelector('.slnctrz-queue-item__prompt'),
      retryEl: el.querySelector('.slnctrz-queue-item__retry'),
      timeEl: el.querySelector('.slnctrz-queue-item__time'),
      stateEl: el.querySelector('.slnctrz-queue-item__state'),
      _lastState: null,
      _lastRetry: -1,
    };
  ...
  _updateItemRow(ref, item) {
    ...
    ref.iconEl.textContent = stateInfo.icon;
    ref.iconEl.className = `slnctrz-queue-item__icon ${useDisplay.colorClass}`;
    ...
    if (phaseStatus && ['click_retry', 'wait_retry'].includes(phaseStatus.phase) && phaseStatus.attempt > 0) {
      ...
      ref.retryEl.className = 'slnctrz-queue-item__retry slnctrz-queue-state--orange';
      ...
      ref.retryEl.className = 'slnctrz-queue-item__retry slnctrz-queue-state--yellow';
      ref.retryEl.title = '';
    } else {
      ref.retryEl.textContent = '';
      ref.retryEl.className = 'slnctrz-queue-item__retry';
      ref.retryEl.title = '';
    }
    ...
    ref.stateEl.className = `slnctrz-queue-item__state ${useDisplay.colorClass}`;
