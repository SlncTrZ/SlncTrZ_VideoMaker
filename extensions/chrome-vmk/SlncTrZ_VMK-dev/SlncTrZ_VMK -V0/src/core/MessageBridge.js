...
class MessageBridge {
  ...
  static async sendToContentScript(action, data = {}) {
    try {
      ...
      try {
        ...
        const isFireAndForgetAction = action === 'pq:trackerUpdate' || action === 'correctStaleFileIds';
  ...
  /**
   * Áp dụng cài đặt Google Flow (genType, ratio, model)
   */
  static async applySettings(genType, ratio, model, isFrames, quantity = 1) {
    return this.sendToContentScript('applySettings', { genType, ratio, model, isFrames, quantity });
  }
  ...
  static async uploadFilesToFlow(filesData) {
    ...
    await this._ensureFlowTabActive();
    return this.sendToContentScript('uploadFilesToFlow', { filesData });
  }
