...
class TileMonitor {
  constructor() {
    ...
    this._timeout = window.SystemConfig?.getTimeout('image_timeout_ms') || 300000;
    this._maxRetries = 2;
   ...
   */
  async _waitClaimedTilesComplete(claimedTileIds, timeoutMs = 180000) {
    const startTime = Date.now();
    ...
    while (Date.now() - startTime < timeoutMs) {
      ...
      for (const tileId of claimedTileIds) {
        const info = statuses[tileId];

        if (!info || info.status === 'not_found') {
          // Tile bị xóa khỏi DOM? Treat as failed
          failed.push(tileId);
          continue;
        }
        ...
        if (info.status === 'success') {
          ...
          // 'processing'
          processing.push(tileId);
        }
      }

      // Tất cả tiles đã settle (không còn processing)?
      if (processing.length === 0) {
        console.log(`[TileMonitor] _waitClaimedTilesComplete done: ${success.length} success, ${failed.length} failed`);
        return { success, failed, thumbnails };
      }

      // Còn tiles đang processing → tiếp tục poll
      await this._sleep(pollInterval);
    }

    // Timeout → get final status, treat processing as failed
