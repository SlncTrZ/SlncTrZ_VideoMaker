...
(function() {
  ...
  class NotificationManager {
    ...
    static async init() {
      ...
      console.log('[SlncTrZ] NotificationManager đã khởi tạo');
    }
    ...
    static notify(title, body, options = {}) {
      ...
      if (settings.browserNotification) {
        try {
          ...
          console.warn('[SlncTrZ] Browser notification failed:', e);
        }
      }
    ...
    static _playBeep() {
      try {
        ...
        console.warn('[SlncTrZ] Sound notification failed:', e);
      }
    }
    ...
    static async _loadNotificationSettings() {
      try {
        ...
        console.warn('[SlncTrZ] Failed to load notification settings:', e);
      }
    }
    ...
    static async _sendWebhook(eventType, data) {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Webhook send failed:', e);
      }
    }
      ...
      while (this._telegramQueue.length > 0) {
        ...
        try {
          ...
          if (/too many|rate limit|429/i.test(e.message)) {
            console.warn('[SlncTrZ] Telegram rate limited, waiting 10s...');
            await new Promise(r => setTimeout(r, 10000));
            // Re-queue the failed notification
            this._telegramQueue.unshift({ eventType, data });
          } else {
            console.warn('[SlncTrZ] Telegram notify failed:', e.message);
          }
        }
      }
    ...
    static _bindEvents() {
      ...
      window.eventBus.on('generation:complete', (data) => {
        NotificationManager.notify(
          'Tạo xong!',
          data?.message || 'Quá trình tạo ảnh/video đã hoàn tất.',
      ...
      window.eventBus.on('task:complete', (data) => {
        NotificationManager.notify(
          'Task hoàn tất!',
          data?.taskName || 'Task đã chạy xong.',
          { tag: 'task-complete' }
        );
      ...
      window.eventBus.on('workflow:complete', (data) => {
        NotificationManager.notify(
          'Workflow hoàn tất!',
          data?.workflowName || 'Workflow đã chạy xong.',
