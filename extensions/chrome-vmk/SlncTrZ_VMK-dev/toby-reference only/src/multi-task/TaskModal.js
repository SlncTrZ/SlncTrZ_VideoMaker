...
class TaskModal {
  ...
  render() {
    ...
    // Group C: Model defaults từ ModelRegistry (server-driven) thay vì hardcode.
    const _defaultImageModel = window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro';
    const _defaultVideoModel = window.ModelRegistry?.getDefault('flow', 'video') || 'Veo 3.1 - Fast';
    ...
    this.overlay.innerHTML = `
      <div class="task-modal">
        ...
        <div class="task-modal-body">
          ...
          <div class="task-provider-row">
            ${(() => {
              ...
              if (isEdit) {
                ...
                const canUseDefault = (defaultProvider === 'flow' && canUseFlow) ||
                                      (defaultProvider === 'chatgpt' && canUseChatGPT) ||
                                      (defaultProvider === 'grok' && canUseGrok);
                if (canUseDefault) {
                  currentProvider = defaultProvider;
                } else {
                  // Fallback: chọn provider đầu tiên không bị lock
                  if (canUseFlow) currentProvider = 'flow';
                  else if (canUseChatGPT) currentProvider = 'chatgpt';
                  else if (canUseGrok) currentProvider = 'grok';
                  else currentProvider = 'flow'; // all locked, will show overlay
                }
              }
            ...
            <div class="provider-tabs" id="taskProviderTabs" role="tablist">
              <button type="button" class="provider-tab ${currentProvider === 'flow' ? 'provider-tab--active' : ''}${!canUseFlow ? ' provider-tab-locked' : ''}" data-provider="flow" role="tab" aria-selected="${currentProvider === 'flow' ? 'true' : 'false'}" ${!canUseFlow ? `aria-disabled="true" data-tooltip="${flowTooltipText}" title="${flowTooltipText}"` : ''}>
                ${flowIcon}<span>${t('gen.providerFlow') || 'Google Flow'}</span>${flowCrownIcon}
              </button>
              <button type="button" class="provider-tab ${currentProvider === 'chatgpt' ? 'provider-tab--active' : ''}${!canUseChatGPT ? ' provider-tab-locked' : ''}" data-provider="chatgpt" role="tab" aria-selected="${currentProvider === 'chatgpt' ? 'true' : 'false'}" ${!canUseChatGPT ? `aria-disabled="true" data-tooltip="${tooltipText}" title="${tooltipText}"` : ''}>
                ${chatgptIcon}<span>${t('gen.providerChatGPT') || 'ChatGPT'}</span>${crownIcon}
              </button>
              <button type="button" class="provider-tab ${currentProvider === 'grok' ? 'provider-tab--active' : ''}${!canUseGrok ? ' provider-tab-locked' : ''}" data-provider="grok" role="tab" aria-selected="${currentProvider === 'grok' ? 'true' : 'false'}" ${!canUseGrok ? `aria-disabled="true" data-tooltip="${grokTooltipText}" title="${grokTooltipText}"` : ''}>
                ${grokIcon}<span>${t('gen.providerGrok') || 'Grok'}</span>${grokCrownIcon}
              </button>
          ...
          <div class="gen-compact-bar">
            ...
            <div class="gen-compact-item" id="taskImageModelGroup">`;
              ...
              <div class="input-group select-group compact-select">
                <select id="taskModel">
                  ${(window.ModelRegistry?.getModelsSync('flow', 'image') || []).map(m => `<option value="${this.escapeAttr(m.value)}" ${effectiveImageModel === m.value ? 'selected' : ''}>${this.escapeHtml(m.name)}</option>`).join('')}
                </select>
            ...
            <div class="gen-compact-item hidden" id="taskVideoModelGroup">
              <div class="input-group select-group compact-select">
                <select id="taskVideoModel">
                  ${(window.ModelRegistry?.getModelsSync('flow', 'video') || []).map(m => `<option value="${this.escapeAttr(m.value)}" ${effectiveVideoModel === m.value ? 'selected' : ''}>${this.escapeHtml(m.name.replace(/^Veo 3\.1 - /, 'Veo 3.1 '))}</option>`).join('')}
                </select>
                <svg class="select-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
              </div>
            </div>
            ...
            <div class="gen-compact-item" id="taskQuantityGroup">
              <div class="input-group compact-qty">
                ...
                <input type="number" id="taskQuantity" min="${window.ValidationRules?.getInt?.('quantity_min', 1) || 1}" max="${window.ValidationRules?.getInt?.('quantity_max', 4) || 4}" value="${effectiveQuantity}" />
          ...
          <div class="form-group" id="taskPromptTemplatesSection" style="display:none;">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;">
              <label style="font-size:12px;font-weight:500;">${t('tasks.promptTemplatesLabel')}</label>
            </div>
            <div class="input-group" style="margin-bottom:6px;">
              <select id="taskTemplateSelect" style="width:100%;">
            ...
            <div id="taskTemplatePreview" class="template-preview hidden"></div>
            <div class="template-actions" style="display:flex;gap:6px;">
              <button type="button" class="btn btn-secondary btn-sm" id="taskInsertTemplateBtn" disabled>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                ${t('tasks.insertToPrompt')}
              </button>
              <button type="button" class="btn btn-secondary btn-sm" id="taskReplaceTemplateBtn" disabled>
  ...
  bindEvents() {
    ...
    openPickerBtn?.addEventListener('click', () => {
      ...
      if (window.imagePickerModal) {
        // Post-audit fix: resolve maxSelections theo task.provider + task.media_type.
        const taskProvider = (this.task?.provider || 'flow').toLowerCase();
        const taskMode = String(this.task?.media_type || 'Image').toLowerCase();
        const grokMode = (this.task?.grok_mode || taskMode).toLowerCase();
        const resolvedMode = taskProvider === 'grok' ? grokMode : taskMode;
  ...
  async _doSave() {
    ...
    if (this.mode === 'create') {
      ...
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        ...
        // limit=5 means user can have 5 tasks (used 0,1,2,3,4 allowed, 5+ blocked)
      ...
      // For trial users (not logged in): check FeatureGate with async method
        ...
        if (!canCreate) {
          window.featureGate.showLoginPrompt(t('tasks.trialExhaustedCreate'));
          return;
        }
      }
    }
    ...
    // Group C: ModelRegistry-aware fallback (server-driven default).
    const model = mediaType === 'Video'
      ? (this.overlay?.querySelector('#taskVideoModel')?.value || window.ModelRegistry?.getDefault('flow', 'video') || 'Veo 3.1 - Fast')
      : (this.overlay?.querySelector('#taskModel')?.value || window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro');
    const videoInputType = this.overlay?.querySelector('#taskVideoInputType')?.value || 'Frames';
    const quantity = parseInt(this.overlay?.querySelector('#taskQuantity')?.value) || 1;
    ...
    // G-5.5 / CG-6.1: ChatGPT + Grok không có model picker — nhưng select element vẫn tồn tại
    // trong DOM (ẩn) với default value 'Nano Banana Pro' (image) hoặc 'Veo 3.1 - Fast' (video).
    // Save model=null cho 2 provider này để tránh data pollution + clarity backend.
    const persistedModel = (provider === 'chatgpt' || provider === 'grok') ? null : model;
    ...
    const taskData = {
      ...
      task_name: taskName || `Task ${new Date().toLocaleString('vi-VN')}`,
    ...
    if (fileIds) {
      ...
      for (const id of refIds) {
        ...
        if (cached) {
          ...
          if (thumbUrl) { refThumbs[id] = thumbUrl; continue; }
        }
      }
  ...
  /**
   * G: Check duplicate tabs cùng 1 provider URL → cảnh báo user.
   */
  _checkDuplicateProviderTabs(providerKey) {
  ...
  _updateRatioOptions() {
    ...
    const options = ratios.map(value => ({ value, label: `${_icon(value)} ${value}` }));
  ...
  async _downloadExternalUrl(url, filename) {
    ...
    try {
      if (provider === 'grok' && window.GrokSession?.getTabInfo && window.MessageBridge?.grokFetchImage) {
        const info = window.GrokSession.getTabInfo();
        ...
        const info = window.ChatGPTSession.getTabInfo();
  ...
  async _initTaskTemplates() {
    const templates = window.GenTab?.savedTemplates || [];
    this._templates = templates;

    const section = this.overlay?.querySelector('#taskPromptTemplatesSection');
    const select = this.overlay?.querySelector('#taskTemplateSelect');
    const preview = this.overlay?.querySelector('#taskTemplatePreview');
    const insertBtn = this.overlay?.querySelector('#taskInsertTemplateBtn');
    const replaceBtn = this.overlay?.querySelector('#taskReplaceTemplateBtn');

    if (templates.length > 0 && section) {
      section.style.display = '';
      let html = `<option value="">${t('tasks.selectTemplate')}</option>`;
      templates.forEach((tpl, idx) => {
        html += `<option value="${idx}">${this.escapeHtml(tpl.title || (tpl.prompt || '').substring(0, 50))}</option>`;
      });
    ...
    if (select) {
      select.addEventListener('change', () => {
        ...
        if (tpl && preview) {
          preview.textContent = tpl.prompt;
    ...
    if (insertBtn) {
      insertBtn.addEventListener('click', () => {
        const idx = parseInt(select?.value);
        const tpl = this._templates[idx];
        if (!tpl || !promptArea) return;
        const current = promptArea.value.trim();
        promptArea.value = current ? `${current}\n${tpl.prompt}` : tpl.prompt;
