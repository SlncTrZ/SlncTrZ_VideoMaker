...
class WorkflowEditor {
  ...
  async saveWorkflow() {
    ...
    if (this.mode === 'create') {
      ...
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        ...
        console.log('[TobyFlow] Workflow quota check:', quota);
  ...
  async _downloadProviderTile(fileId, providerData, promptText, index, nodeData, fileName) {
    ...
    if (!url || !window.MessageBridge) {
      console.warn('[TobyFlow] _downloadProviderTile: missing url/MessageBridge', { fileId, provider });
      return;
    }
    ...
    try {
      ...
      const fetchFn = provider === 'chatgpt'
        ...
        : provider === 'grok'
          ...
          : null;
      if (!fetchFn) {
        console.warn('[TobyFlow] _downloadProviderTile: no fetcher for provider', provider);
        return;
      }

      const fetchResp = await fetchFn(url, tab_id);
      if (!fetchResp?.success || !fetchResp.base64) {
        console.warn('[TobyFlow] _downloadProviderTile fetch fail:', provider, fetchResp?.error);
      ...
      if (!dlResp?.success) {
        console.warn(`[TobyFlow] ${provider} manual download fail:`, dlResp?.error);
      } else {
        console.log(`[TobyFlow] ${provider} manual download OK:`, filename);
      }
    } catch (err) {
