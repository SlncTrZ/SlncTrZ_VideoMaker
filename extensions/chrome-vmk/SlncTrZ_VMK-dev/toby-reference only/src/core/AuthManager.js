...
class AuthManager {
  constructor() {
    this.user = null;
    this.token = null;
    this.apiBaseUrl = 'https://labs.toby.vn/api/v1';
    ...
    return new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({
        ...
        if (response.success) {
          ...
          if (httpStatus === 401 && !_isRetry && endpoint !== 'auth/login' && endpoint !== 'auth/refresh' && endpoint !== 'sse/end-session') {
          ...
          if (httpStatus === 429) {
            ...
            console.warn('[TobyFlow] AuthManager: Rate limited (429), retry after', retryAfter, 's');

            // [Fix cascade] Set cooldown để mọi caller mới bị short-circuit ngay,
            // không hammer backend trong khi backend đang stress.
            this._rateLimitedUntil = Date.now() + (Number(retryAfter) || 60) * 1000;
