...
class FeatureGate {
  ...
  async _fetchEntitlements() {
    ...
    console.log('[TobyFlow] FeatureGate: Fetching entitlements, isLoggedIn:', wasLoggedIn, ', hasToken:', !!authToken, ', tokenPrefix:', authToken?.substring(0, 10));
    ...
    if (wasLoggedIn) {
      ...
      console.log('[TobyFlow] FeatureGate: Đã cập nhật entitlements từ server (logged-in), plan:', this.plan?.slug);
    } else {
      ...
      await this._saveCache();
      console.log('[TobyFlow] FeatureGate: Đã cập nhật entitlements từ server (anonymous), plan:', this.plan?.slug);
    }
