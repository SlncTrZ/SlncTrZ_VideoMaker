...
class ExecutionGate {
  ...
  static async complete(token, status = 'success', resultData = {}) {
    // Token null = offline fallback, van refresh FeatureGate de sync
    if (!token) {
  ...
  static async cancel(token) {
    // Token null = offline fallback, khong can bao server
    if (!token) return;
   ...
   *
   * Chỉ allow nếu có cached quota hợp lệ (< 1 hour)
   */
  static _fallbackCheck(action, promptCount, metadata = {}) {
    ...
    const OFFLINE_CACHE_TTL = 60 * 60 * 1000; // 1 hour - generous window for offline use
    ...
    // Nếu cache quá cũ (> 1 hour) hoặc không có → DENY (fail-closed)
    if (cacheAge > OFFLINE_CACHE_TTL) {
      console.warn('[ExecutionGate] fallback DENIED: cache too stale (' + Math.round(cacheAge / 60000) + ' min) - fail-closed');
      return {
        allowed: false,
        token: null,
