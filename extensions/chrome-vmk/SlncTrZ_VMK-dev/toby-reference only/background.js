...
async function rescheduleFlowSessionAlarm() {
  try {
    ...
    console.log('[TobyFlow] Flow session refresh alarm scheduled every', clampedMin, 'min');
  } catch (e) {
...
chrome.runtime.onInstalled.addListener((details) => {
  ...
  // (Guard flag `__tobyflowContentJsLoaded__` trong content.js là last line of defense.)
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'chromeDownload') {
    ...
    console.log(`[TobyFlow] chromeDownload: filename="${filename}", url="${url?.substring(0, 60)}..."`);
  ...
  if (message.action === 'apiRequest') {
    ...
    (async () => {
      try {
        ...
        const apiBaseUrl = stored.apiBaseUrl || 'https://labs.toby.vn/api/v1';
  ...
  if (message.action === 'chatgpt:injectScript') {
    ...
    (async () => {
      try {
        ...
        console.error('[TobyFlow] chatgpt:injectScript error:', err.message);
  ...
  if (message.action === 'gemini:injectScript') {
    ...
    (async () => {
      try {
        ...
        console.error('[TobyFlow] gemini:injectScript error:', err.message);
        sendResponse({ success: false, error: err.message || 'INJECT_FAILED' });
      }
    })();
    return true;
  ...
  if (message.action === 'grok:injectScript') {
    ...
    (async () => {
      try {
        ...
        console.error('[TobyFlow] grok:injectScript error:', err.message);
        sendResponse({ success: false, error: err.message || 'INJECT_FAILED' });
      }
    })();
    return true;
