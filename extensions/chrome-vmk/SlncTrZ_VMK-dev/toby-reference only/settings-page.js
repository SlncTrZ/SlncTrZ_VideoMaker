...
(function() {
  ...
  async function fetchAndMergeServerSettings() {
    try {
      ...
      if (resp?.success && resp.data?.settings_json) {
        ...
        // Server settings chỉ fill những field local chưa có (vd: user mới login, chưa config)
  ...
  function updateUI(s) {
    ...
    // Toggle hiển thị các cài đặt queue khi bật/tắt
  ...
  function collectSettings() {
    return {
      ...
      // Phase FAR — Flow auto-retry settings
  ...
  // ===== Bind Events =====
  /**
   * Ẩn/hiện các cài đặt queue dựa trên trạng thái toggle
   */
