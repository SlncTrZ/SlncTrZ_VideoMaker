...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // PATCHED: Offline — skip
  if (window.authManager?.apiBaseUrl && (!Array.isArray(window._cachedPlans) || window._cachedPlans.length === 0)) {
    try {
      ...
      if ((data.mode === 'edit' || data.mode === 'view' || data.mode === 'admin_preview') && data.workflow) {
        ...
        if (data.mode === 'edit') {
          ...
          if (smMode === 'api' && data.workflow.wf_id) {
            ...
            // PATCHED: Offline mode — skip API switch
          }
    ...
    _syncTimer = setInterval(async () => {
      ...
      try {
        ...
        for (const node of fresh.nodes) {
          ...
          // Phase 5 skip persist 'running' status → server vẫn 'pending'.
          // Polling thấy mismatch sẽ reset UI về 'pending' → bug mất UI/preview.
          // Giữ local state tiến bộ, chỉ sync khi server có status mới hơn.
          // Status order: pending < running < completed/failed/skipped
          const localIsAhead = (currentNode.status === 'running' && node.status === 'pending')
            || (currentNode.status === 'completed' && node.status === 'pending')
            || (currentNode.status === 'completed' && node.status === 'running')
            || (currentNode.status === 'failed' && node.status === 'pending')
            || (currentNode.status === 'failed' && node.status === 'running');
          if (localIsAhead) {
