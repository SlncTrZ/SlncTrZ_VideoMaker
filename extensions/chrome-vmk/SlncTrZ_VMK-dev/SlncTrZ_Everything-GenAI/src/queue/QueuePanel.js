...
(function() {
  ...
  class QueuePanel {
    ...
    render() {
      ...
      this._listEl = this.container.querySelector('.slnctrz-queue-list');
      this._emptyEl = this.container.querySelector('.slnctrz-queue-empty');
      this._progressEl = this.container.querySelector('.slnctrz-queue-progress');
      this._progressText = this.container.querySelector('.slnctrz-queue-progress-text');
      this._progressBar = this.container.querySelector('.slnctrz-queue-progress-bar-fill');
      this._updateList();
    }

    _template() {
      return `
        <div class="slnctrz-queue-panel">
          <div class="slnctrz-queue-header">
            <div class="slnctrz-queue-header-title">
            ...
            <div class="slnctrz-queue-controls">
              <button class="slnctrz-queue-ctrl-btn" id="queuePlayPauseBtn" title="${window.I18n?.t('queue.playPause') || 'Chạy / Tạm dừng'}">
                <svg class="slnctrz-queue-icon-play" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                  <polygon points="5 3 19 12 5 21 5 3"></polygon>
                </svg>
                <svg class="slnctrz-queue-icon-pause hidden" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                  <rect x="6" y="4" width="4" height="16"></rect>
                  <rect x="14" y="4" width="4" height="16"></rect>
                </svg>
              </button>
              <button class="slnctrz-queue-ctrl-btn" id="queueStopBtn" title="${window.I18n?.t('common.stop') || 'Dừng'}">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none">
                  <rect x="4" y="4" width="16" height="16" rx="2"></rect>
                </svg>
              </button>
              <button class="slnctrz-queue-ctrl-btn" id="queueClearBtn" title="${window.I18n?.t('queue.clearAll') || 'Xóa tất cả'}">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="slnctrz-queue-progress hidden">
            <span class="slnctrz-queue-progress-text"></span>
            <div class="slnctrz-queue-progress-bar">
              <div class="slnctrz-queue-progress-bar-fill" style="width: 0%"></div>
            </div>
          </div>
          <div class="slnctrz-queue-list"></div>
          <div class="slnctrz-queue-empty">
    ...
    _updateControls(isRunning, isPaused) {
      const playIcon = this.container.querySelector('.slnctrz-queue-icon-play');
      const pauseIcon = this.container.querySelector('.slnctrz-queue-icon-pause');
    ...
    _updateList() {
      ...
      this._listEl.innerHTML = queue.map((item, index) => `
        <div class="slnctrz-queue-item ${item.status === 'running' ? 'slnctrz-queue-item--running' : ''} ${item.status === 'completed' ? 'slnctrz-queue-item--completed' : ''} ${item.status === 'failed' ? 'slnctrz-queue-item--failed' : ''}"
             data-index="${index}" draggable="true">
          <div class="slnctrz-queue-item__drag">
          ...
          <div class="slnctrz-queue-item__icon">
            ${this._getTypeIcon(item.type)}
          </div>
          <div class="slnctrz-queue-item__info">
            <span class="slnctrz-queue-item__name">${this._escapeHtml(item.name)}</span>
            ${item.priority > 0 ? `<span class="slnctrz-queue-item__priority">P${item.priority}</span>` : ''}
          </div>
          <div class="slnctrz-queue-item__status">
            ${this._getStatusIcon(item.status)}
          </div>
          <button class="slnctrz-queue-item__delete" data-delete-index="${index}" title="${window.I18n?.t('common.delete') || 'Xóa'}">
      ...
      // Bind delete buttons
      this._listEl.querySelectorAll('.slnctrz-queue-item__delete').forEach(btn => {
    ...
    _bindDragEvents() {
      const items = this._listEl.querySelectorAll('.slnctrz-queue-item');
      items.forEach(item => {
        item.addEventListener('dragstart', (e) => {
          this._dragState = { fromIndex: parseInt(item.dataset.index) };
          item.classList.add('slnctrz-queue-item--dragging');
        ...
        item.addEventListener('dragover', (e) => {
          ...
          item.classList.add('slnctrz-queue-item--drag-over');
        });

        item.addEventListener('dragleave', () => {
          item.classList.remove('slnctrz-queue-item--drag-over');
        });
        ...
        item.addEventListener('dragend', () => {
          item.classList.remove('slnctrz-queue-item--dragging');
          this._dragState = null;
        });
      });
    }
    ...
    _getStatusIcon(status) {
      switch (status) {
        case 'running':
          return '<span class="slnctrz-queue-item__spinner"></span>';
        case 'completed':
