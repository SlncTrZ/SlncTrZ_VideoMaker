...
class TemplatesTab {
  ...
  async init() {
    ...
    console.log('[SlncTrZ] TemplatesTab: Đang khởi tạo...');
    ...
    console.log('[SlncTrZ] TemplatesTab: Đã khởi tạo');
  }
  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="slnctrz-templates-tab">
        <!-- Search + Filters compact -->
        <div class="section slnctrz-templates-search">
          <div class="slnctrz-templates-search-row">
            <div class="input-group slnctrz-templates-search-box">
              <svg class="slnctrz-templates-search__icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
              <input type="text" id="templateSearchInput" placeholder="${window.I18n?.t('templates.searchTemplate') || 'Tìm kiếm template...'}" />
            </div>
            <button class="btn btn-secondary btn-sm slnctrz-refresh-btn" id="templateRefreshBtn" title="Tải lại">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"></polyline><polyline points="1 20 1 14 7 14"></polyline><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>
            </button>
          </div>
          <!-- Filters: 1 row, 3 columns -->
          <div class="slnctrz-templates-filter-row">
          ...
          <!-- Label pills -->
          <div class="slnctrz-templates-labels">
            <button class="slnctrz-templates-label-btn" data-label="trending">${window.I18n?.t('templates.trending') || 'Xu hướng'}</button>
            <button class="slnctrz-templates-label-btn" data-label="new">${window.I18n?.t('templates.new') || 'Mới'}</button>
            <button class="slnctrz-templates-label-btn" data-label="featured">${window.I18n?.t('templates.featured') || 'Nổi bật'}</button>
            <div class="input-group select-group compact-select slnctrz-templates-premium-filter">
        ...
        <!-- Grid template -->
        <div class="section slnctrz-templates-grid" id="templatesGrid">
          <div class="slnctrz-templates-loading">${window.I18n?.t('common.loading') || 'Đang tải...'}</div>
        </div>

        <!-- Tải thêm -->
        <div class="section slnctrz-templates-load-more hidden" id="templatesLoadMore">
  ...
  _renderStar(filled) {
    if (filled) {
      return `<svg class="slnctrz-star slnctrz-star--filled" width="12" height="12" viewBox="0 0 24 24" fill="#f59e0b" stroke="#f59e0b" stroke-width="1"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>`;
    }
    return `<svg class="slnctrz-star slnctrz-star--empty" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#888" stroke-width="1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>`;
  }
  ...
  _renderRatingDisplay(avgRating, ratingsCount) {
    if (!ratingsCount || ratingsCount === 0) {
      return `<span class="slnctrz-template-rating slnctrz-template-rating--empty">${window.I18n?.t('templates.noRating') || 'Chưa có đánh giá'}</span>`;
    }
    ...
    return `
      <span class="slnctrz-template-rating">
        <span class="slnctrz-template-rating__stars">${starsHtml}</span>
        <span class="slnctrz-template-rating__text">${avgRating} (${ratingsCount})</span>
      </span>
    `;
  }
  ...
  _renderInteractiveRating(templateId) {
    let starsHtml = '';
    for (let i = 1; i <= 5; i++) {
      starsHtml += `<span class="slnctrz-star-interactive" data-rating="${i}" data-template-id="${templateId}">${this._renderStar(false)}</span>`;
    }
    return `
      <div class="slnctrz-template-rating-interactive" data-template-id="${templateId}">
        <span class="slnctrz-template-rating-interactive__label">${window.I18n?.t('templates.rateLabel') || 'Đánh giá:'}</span>
        <span class="slnctrz-template-rating-interactive__stars">${starsHtml}</span>
      </div>
    `;
  }
  ...
  _renderCardHtml(template) {
    ...
    const labelsHtml = labels.map(l =>
      `<span class="slnctrz-template-card__badge slnctrz-template-card__badge--${l.key}">${l.text}</span>`
    ).join('');

    const categoryName = template.category?.name || '';
    const mediaTypeBadge = template.media_type
      ? `<span class="slnctrz-template-card__media-badge">${template.media_type === 'Image' ? (window.I18n?.t('templates.typeImage') || 'Ảnh') : 'Video'}</span>`
      : '';

    const thumbnailHtml = template.thumbnail_url
      ? `<img src="${template.thumbnail_url}" alt="${this._escapeHtml(template.title)}" class="slnctrz-template-card__img" loading="lazy" />`
      : `<div class="slnctrz-template-card__placeholder">
    ...
    const premiumBadgeHtml = isPremium
      ? `<span class="slnctrz-template-card__badge slnctrz-template-card__badge--premium">
  ...
  async loadCategories() {
    try {
      ...
      if (window.featureGate && !window.featureGate.canUsePromptTemplates()) {
        console.log('[SlncTrZ] TemplatesTab: Feature prompt_templates disabled, bỏ qua load categories');
        return;
      }
      ...
      console.log('[SlncTrZ] TemplatesTab: Đã tải', this.categories.length, 'danh mục');
    } catch (err) {
      console.error('[SlncTrZ] TemplatesTab: Lỗi tải danh mục', err.message);
    }
  }
  ...
  async loadTemplates(append = false) {
    ...
    if (!append && grid) {
      grid.innerHTML = `<div class="slnctrz-templates-loading">${window.I18n?.t('common.loading') || 'Đang tải...'}</div>`;
    }
    ...
    try {
      ...
      if (window.featureGate && !window.featureGate.canUsePromptTemplates()) {
        ...
        console.log('[SlncTrZ] TemplatesTab: Feature prompt_templates disabled');
        this.isLoading = false;
        return;
      }
      ...
      console.log('[SlncTrZ] TemplatesTab: Đã tải', newTemplates.length, 'templates, trang', this.currentPage, '/', this.totalPages);
    } catch (err) {
      console.error('[SlncTrZ] TemplatesTab: Lỗi tải templates:', err.message);
      if (!append && grid) {
        ...
        grid.innerHTML = `<div class="slnctrz-templates-empty">${errorMsg}<br><small style="opacity:0.5;margin-top:4px;display:block;">${err.message || ''}</small></div>`;
      }
    } finally {
      this.isLoading = false;
    }
  ...
  _renderGrid(append = false) {
    ...
    if (this.templates.length === 0) {
      grid.innerHTML = `<div class="slnctrz-templates-empty">${window.I18n?.t('templates.noTemplatesFound') || 'Không tìm thấy template nào'}</div>`;
      this._renderedCount = 0;
      return;
    }
  ...
  _onGridScroll() {
    ...
    const scrollParent = grid.closest('.slnctrz-templates-tab') || grid.parentElement;
  ...
  async _useTemplate(templateId) {
    try {
      ...
      console.log('[SlncTrZ] TemplatesTab: Đang tải template', templateId);
      ...
      console.log('[SlncTrZ] TemplatesTab: Đã áp dụng template', template.title);
      ...
      console.error('[SlncTrZ] TemplatesTab: Lỗi sử dụng template', templateId, err.message);
      if (typeof sendLog === 'function') {
  ...
  _showTemplateGuide(template) {
    ...
    if (template.ref_file_count && template.ref_file_count > 0) {
      ...
      uploadGuideHtml = `
        <div class="slnctrz-template-guide__upload-box">
          <div class="slnctrz-template-guide__upload-header">
          ...
          <div class="slnctrz-template-guide__upload-list">
            ${uploadItems.map((item, idx) =>
              `<div class="slnctrz-template-guide__upload-item">${hasNumberPrefix ? '' : `<span class="slnctrz-template-guide__upload-num">${idx + 1}</span>`}<span>${this._escapeHtml(item)}</span></div>`
            ).join('')}
          </div>
        </div>
      `;
    ...
    if (diffConfig) {
      difficultyHtml = `
        <span class="slnctrz-template-guide__difficulty slnctrz-template-guide__difficulty--${template.difficulty}">
          ${diffConfig.icon}
    ...
    if (template.description) {
      descriptionHtml = `<p class="slnctrz-template-guide__desc">${this._escapeHtml(template.description)}</p>`;
    }
    ...
    guideDiv.className = 'slnctrz-template-guide';

    // Interactive rating
    const interactiveRatingHtml = this._renderInteractiveRating(template.id);

    guideDiv.innerHTML = `
      <div class="slnctrz-template-guide__header">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
        <span>${this._escapeHtml(template.title)}</span>
        <button class="slnctrz-template-guide__close" id="templateGuideClose" title="${window.I18n?.t('common.close') || 'Đóng'}">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
      </div>
      ${uploadGuideHtml}
      ${descriptionHtml}
      <div class="slnctrz-template-guide__footer">
        ${difficultyHtml}
  ...
  _bindInteractiveRating(container, templateId) {
    const starsContainer = container.querySelector('.slnctrz-template-rating-interactive__stars');
    if (!starsContainer) return;

    const starSpans = starsContainer.querySelectorAll('.slnctrz-star-interactive');
      ...
      span.addEventListener('mouseenter', () => {
        starSpans.forEach((s, i) => {
          ...
          if (i <= idx) {
            ...
            svgEl.classList.add('slnctrz-star--filled');
            svgEl.classList.remove('slnctrz-star--empty');
          } else {
            svgEl.setAttribute('fill', 'none');
            svgEl.setAttribute('stroke', '#888');
            svgEl.classList.remove('slnctrz-star--filled');
            svgEl.classList.add('slnctrz-star--empty');
          }
        });
      });
    ...
    starsContainer.addEventListener('mouseleave', () => {
      starSpans.forEach(s => {
        const svgEl = s.querySelector('svg');
        if (!s.classList.contains('slnctrz-star-interactive--selected')) {
          svgEl.setAttribute('fill', 'none');
          svgEl.setAttribute('stroke', '#888');
          svgEl.classList.remove('slnctrz-star--filled');
          svgEl.classList.add('slnctrz-star--empty');
  ...
  async _rateTemplate(templateId, rating, starsContainer) {
    try {
      console.log('[SlncTrZ] TemplatesTab: Đánh giá template', templateId, 'sao', rating);
      ...
      if (starsContainer) {
        ...
        starSpans.forEach((s, i) => {
          ...
          if (i < rating) {
            ...
            svgEl.classList.add('slnctrz-star--filled');
            svgEl.classList.remove('slnctrz-star--empty');
            s.classList.add('slnctrz-star-interactive--selected');
          } else {
            svgEl.setAttribute('fill', 'none');
            svgEl.setAttribute('stroke', '#888');
            svgEl.classList.remove('slnctrz-star--filled');
            svgEl.classList.add('slnctrz-star--empty');
            s.classList.remove('slnctrz-star-interactive--selected');
          }
        });
      }

      const card = this.container.querySelector(`.slnctrz-template-card[data-template-id="${templateId}"]`);
      if (card && data.avg_rating !== undefined) {
        const ratingEl = card.querySelector('.slnctrz-template-rating');
        if (ratingEl) {
      ...
      console.log('[SlncTrZ] TemplatesTab: Đã đánh giá template', templateId, '-> avg:', data.avg_rating);
    } catch (err) {
      console.error('[SlncTrZ] TemplatesTab: Lỗi đánh giá template', templateId, err.message);
      if (typeof sendLog === 'function') {
  ...
  bindEvents() {
    ...
    const labelBtns = this.container.querySelectorAll('.slnctrz-templates-label-btn');
    ...
    if (grid) {
      grid.addEventListener('click', (e) => {
        const useBtn = e.target.closest('.slnctrz-template-use-btn');
        if (useBtn) {
          e.stopPropagation();
        ...
        const card = e.target.closest('.slnctrz-template-card');
        if (card) {
          if (card.classList.contains('slnctrz-template-card--locked')) {
            if (typeof sendLog === 'function') {
    ...
    const scrollParent = this.container.querySelector('.slnctrz-templates-tab') || this.container;
    if (scrollParent) {
  ...
  async _loadTemplateRefImages(refImageUrls) {
    ...
    console.log('[SlncTrZ] TemplatesTab: Đang load', refImageUrls.length, 'ref images từ template');
    ...
    for (let i = 0; i < refImageUrls.length; i++) {
      ...
      try {
        ...
        if (!blob) {
          console.warn('[SlncTrZ] TemplatesTab: Không thể fetch blob từ', url);
          continue;
        }
        ...
        console.log('[SlncTrZ] TemplatesTab: Đã load ref image (deferred)', i + 1, 'với key', key);
      } catch (err) {
        console.error('[SlncTrZ] TemplatesTab: Lỗi load ref image', i, err.message);
      }
    }
    ...
    console.log('[SlncTrZ] TemplatesTab: Hoàn tất load (deferred upload)', newFileIds.length, 'ref images');
  }
  ...
  async _fetchBlobViaBackground(url) {
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({
        ...
        if (chrome.runtime.lastError) {
          console.error('[SlncTrZ] TemplatesTab: fetchBlob error', chrome.runtime.lastError.message);
          resolve(null);
          return;
        }
        if (response && response.success && response.base64) {
          ...
          try {
            ...
            console.error('[SlncTrZ] TemplatesTab: base64 decode error', err.message);
            resolve(null);
          }
        } else {
          resolve(null);
  ...
  _clearGenTabRefImages() {
    ...
    console.log('[SlncTrZ] TemplatesTab: Đã clear ref images cũ trong GenTab');
  }
