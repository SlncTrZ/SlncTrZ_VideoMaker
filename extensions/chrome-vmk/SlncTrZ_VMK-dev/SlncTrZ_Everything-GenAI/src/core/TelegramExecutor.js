...
class TelegramExecutor {
    ...
    switch (provider) {
      case 'flow':
        return {
          ...
          model: settings?.telegramFlowModel || window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana 2',
        };
      case 'chatgpt':
        return {
   ...
   * @param {Object} args - { prompt, ratio, model, count, genType, ref_image_url, ref_image_base64, ref_images_base64 }
   * genType: 'Image' (default) hoặc 'Video'
   * ref_images_base64: array of base64 strings (multi-image support)
   */
  static async _executeFlowGenerate(args) {
    console.log('[TelegramExecutor] _executeFlowGenerate (Flow provider)');
    const { prompt, ratio, model, count, ref_image_url, ref_image_base64, ref_images_base64, genType = 'Image' } = args;
  ...
  static async _executeGrokGenerate(args) {
    ...
    if (!modeResult.success) {
      console.warn('[TelegramExecutor] Grok setMode failed:', modeResult.error);
    }

    // Set ratio
    await window.GrokSession.setRatio(grokRatio);

    // Set video settings nếu là video
    if (mode === 'video') {
      await window.GrokSession.setVideoDuration(grokDuration);
      await window.GrokSession.setVideoResolution(grokResolution);
    } else {
      // Set image quality cho image mode
      await window.GrokSession.setImageQuality(grokImageQuality);
    }
   ...
   * ChatGPT ratios: 'story' (9:16), 'portrait' (3:4), 'square' (1:1), 'landscape' (4:3), 'widescreen' (16:9)
   */
