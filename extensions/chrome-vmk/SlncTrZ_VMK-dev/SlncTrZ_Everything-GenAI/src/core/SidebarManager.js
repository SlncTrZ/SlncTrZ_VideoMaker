...
class SidebarManager {
  static async init() {
    ...
    if (window.SlncTrZApp) {
      window.SlncTrZApp.init();
    }
  }
  ...
  static _bindCoreEvents() {
    ...
    if (clearLogsBtn) {
      clearLogsBtn.addEventListener('click', () => {
        ...
        window.showNotification?.(window.I18n?.t('logs.logsCleared') || 'Log Ä‘Ã£ xÃ³a', 'success');
      });
    }
    ...
    if (exportLogsBtn) {
      exportLogsBtn.addEventListener('click', () => {
        ...
        if (entries.length === 0) {
          console.log('[SlncTrZ] Kh\u00F4ng c\u00F3 log n\u00E0o \u0111\u1EC3 xu\u1EA5t');
          return;
        }
        ...
        a.href = url;
        a.download = `slnctrz-log-${Date.now()}.txt`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        console.log('[SlncTrZ] \u0110\u00E3 xu\u1EA5t', entries.length, 'm\u1EE5c log');
        window.showNotification?.(window.I18n?.t('logs.logsExported', { count: entries.length }) || `ÄÃ£ xuáº¥t ${entries.length} má»¥c log`, 'success');
      });
    }
      ...
      function updateThemeIcon(theme) {
        ...
        if (themeLabel) {
          themeLabel.textContent = isDark
            ? (window.I18n?.t('header.lightMode') || 'Cháº¿ Ä‘á»™ sÃ¡ng')
            : (window.I18n?.t('header.darkMode') || 'Cháº¿ Ä‘á»™ tá»‘i');
        }
      }
    ...
    // Monitor active tab URL â€” show overlay when not on Google Flow
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === 'local' && changes.af_settings) {
        ...
        console.log('[SlncTrZ] Settings updated from settings window');
      }
    });
  }
  ...
  /**
   * Restore active tab from storage khi sidebar má»Ÿ
   * Global sidePanel mode: chá»‰ cáº§n restore khi sidePanel init láº§n Ä‘áº§u
   */
  static async _restoreActiveTab(tabBtns, tabPanes) {
    try {
      ...
      const currentActive = document.querySelector('.slnctrz-tab.active');
      if (currentActive?.dataset.tab === savedTabId) return;

      // Find and activate the saved tab
      const targetBtn = document.querySelector(`.slnctrz-tab[data-tab="${savedTabId}"]`);
      if (!targetBtn) return;
      ...
      console.log('[SlncTrZ] Tab restored from storage:', savedTabId);
    } catch (err) {
      console.warn('[SlncTrZ] Tab restore failed:', err.message);
    }
  }
}
