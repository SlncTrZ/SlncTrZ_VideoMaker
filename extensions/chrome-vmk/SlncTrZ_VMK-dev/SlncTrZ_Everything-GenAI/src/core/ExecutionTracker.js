...
class ExecutionTracker {
  ...
  _createDOM() {
    ...
    this._el.className = 'slnctrz-exec-tracker slnctrz-exec-tracker--hidden';
    this._el.innerHTML = `
      <div class="slnctrz-exec-tracker__bar">
        <div class="slnctrz-exec-tracker__progress-fill"></div>
        <div class="slnctrz-exec-tracker__content">
          <span class="slnctrz-exec-tracker__icon">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
            </svg>
          </span>
          <span class="slnctrz-exec-tracker__label"></span>
          <span class="slnctrz-exec-tracker__phase"></span>
          <span class="slnctrz-exec-tracker__counter"></span>
          <span class="slnctrz-exec-tracker__elapsed"></span>
          <span class="slnctrz-exec-tracker__retry-status"></span>
          <span class="slnctrz-exec-tracker__error-badge slnctrz-exec-tracker__error-badge--hidden">0</span>
          <div class="slnctrz-exec-tracker__actions">
            <button class="slnctrz-exec-tracker__btn slnctrz-exec-tracker__btn--pause" title="${window.I18n?.t('common.pause') || 'Tạm dừng'}">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <rect x="6" y="4" width="4" height="16"></rect>
                <rect x="14" y="4" width="4" height="16"></rect>
              </svg>
            </button>
            <button class="slnctrz-exec-tracker__btn slnctrz-exec-tracker__btn--stop" title="${window.I18n?.t('common.stop') || 'Dừng'}">
    ...
    // Cache elements
    this._progressFill = this._el.querySelector('.slnctrz-exec-tracker__progress-fill');
    this._labelEl = this._el.querySelector('.slnctrz-exec-tracker__label');
    this._phaseEl = this._el.querySelector('.slnctrz-exec-tracker__phase');
    this._counterEl = this._el.querySelector('.slnctrz-exec-tracker__counter');
    this._elapsedEl = this._el.querySelector('.slnctrz-exec-tracker__elapsed');
    this._errorBadge = this._el.querySelector('.slnctrz-exec-tracker__error-badge');
    this._iconEl = this._el.querySelector('.slnctrz-exec-tracker__icon');
    this._pauseBtn = this._el.querySelector('.slnctrz-exec-tracker__btn--pause');
    this._stopBtn = this._el.querySelector('.slnctrz-exec-tracker__btn--stop');
    this._actionsEl = this._el.querySelector('.slnctrz-exec-tracker__actions');
    this._retryStatusEl = this._el.querySelector('.slnctrz-exec-tracker__retry-status');
  ...
  _onPipelineUpdate(snapshot) {
    ...
    if (this._state === 'hidden') {
      ...
      this._state = 'visible';
      this._el.className = 'slnctrz-exec-tracker';
  ...
  _hide() {
    ...
    if (this._retryStatusEl) {
      ...
      this._retryStatusEl.classList.remove('slnctrz-exec-tracker__retry-status--visible');
    }
  }
  ...
  _showRetryStatus(text) {
    ...
    this._retryStatusEl.classList.add('slnctrz-exec-tracker__retry-status--visible');
  ...
  _showCompletion() {
    ...
    this._errorBadge.classList.add('slnctrz-exec-tracker__error-badge--hidden');
  ...
  _render() {
    ...
    if (this._errorCount > 0) {
      ...
      this._errorBadge.classList.remove('slnctrz-exec-tracker__error-badge--hidden');
    } else {
      this._errorBadge.classList.add('slnctrz-exec-tracker__error-badge--hidden');
    }
  ...
  _handleStop() {
    ...
    // 3. Cancel execution token nếu có
