...
class FeatureGate {
  constructor() {
    ...
    this.cacheTTL = 5 * 60 * 1000; // 5 phút (giảm tải server)
    this.lastFetch = 0;
    ...
    this._freeDefaults = {
      ...
      // [[UNLOCKED]] All features enabled with unlimited quotas
    ...
    try {
      ...
      if (!this._isCacheValid()) {
        ...
        this.refresh().catch(err => {
          console.warn('[SlncTrZ] FeatureGate: Background refresh failed', err.message);
        });
      }

      console.log('[SlncTrZ] FeatureGate: Đã khởi tạo, plan:', this.plan?.slug || 'trial', ', isLoggedIn:', this.isLoggedIn());
    } catch (err) {
      console.warn('[SlncTrZ] FeatureGate: Lỗi khởi tạo, dùng giá trị mặc định', err.message);
  ...
  async refresh() {
    ...
    if (window.SseClient?.isConnected?.() && this._lastSseRefresh > Date.now() - this.cacheTTL) {
      console.log('[SlncTrZ] FeatureGate: Skip API refresh, using SSE-pushed data (last SSE:', new Date(this._lastSseRefresh).toLocaleTimeString(), ')');
  ...
  handleSseEntitlementsChanged(data) {
    ...
    if (this._isLoggingOut || !this.isLoggedIn()) {
      console.log('[SlncTrZ] FeatureGate: Bỏ qua SSE entitlements_changed (đang logout hoặc đã logout)');
      return;
    }
    ...
    this._saveCache().catch(err => {
      console.warn('[SlncTrZ] FeatureGate: Lỗi lưu SSE data vào cache', err.message);
    });

    console.log('[SlncTrZ] FeatureGate: Updated from SSE push, plan:', this.plan?.slug || 'trial');
  ...
  async _doRefresh() {
    try {
      ...
      console.error('[SlncTrZ] FeatureGate: Lỗi fetch entitlements', err.message);
  ...
  async resetForLogout() {
    console.log('[SlncTrZ] FeatureGate: Reset for logout...');
    ...
    if (this._refreshPromise) {
      console.log('[SlncTrZ] FeatureGate: Chờ existing refresh hoàn thành trước logout...');
      try {
    ...
    this._refreshPromise = (async () => {
      try {
        await this._fetchEntitlements();
        console.log('[SlncTrZ] FeatureGate: Đã fetch trial entitlements sau logout');
      } catch (err) {
        console.warn('[SlncTrZ] FeatureGate: Lỗi fetch trial entitlements, dùng defaults', err.message);
        throw err;
      }
    })();
   ...
   */
  async resetForLogin() {
    console.log('[SlncTrZ] FeatureGate: Reset for login...');
    ...
    if (this._refreshPromise) {
      console.log('[SlncTrZ] FeatureGate: Chờ existing refresh hoàn thành trước...');
      try {
        await this._refreshPromise;
      } catch (e) {
    ...
    this._refreshPromise = (async () => {
      try {
        await this._fetchEntitlements();
        console.log('[SlncTrZ] FeatureGate: Đã fetch user entitlements sau login, plan:', this.plan?.slug);
      } catch (err) {
        console.warn('[SlncTrZ] FeatureGate: Lỗi fetch user entitlements sau login', err.message);
        throw err;
      }
    })();
  ...
  async _fetchEntitlements() {
    // PATCHED: Offline — skip server fetch, use defaults
    ...
    console.log('[SlncTrZ] FeatureGate: Fetching entitlements, isLoggedIn:', wasLoggedIn, ', hasToken:', !!authToken, ', tokenPrefix:', authToken?.substring(0, 10));
    ...
    if (wasLoggedIn) {
      ...
      if (!this.isLoggedIn()) {
        console.warn('[SlncTrZ] FeatureGate: Discard logged-in response — user logged out giữa fetch');
        return;
      }
      ...
      console.log('[SlncTrZ] FeatureGate: Đã cập nhật entitlements từ server (logged-in), plan:', this.plan?.slug);
    } else {
      ...
      if (this.isLoggedIn()) {
        console.warn('[SlncTrZ] FeatureGate: Discard anonymous response — user đã login giữa fetch (race on reload). Triggering logged-in refresh...');
        // Schedule logged-in refresh ở next tick (tránh recursion trong _doRefresh)
        setTimeout(() => {
          this.refresh().catch(err => console.warn('[SlncTrZ] FeatureGate: post-discard refresh failed', err.message));
        }, 0);
        return;
      }
      ...
      // Merge với defaults để các feature không có trong response vẫn có giá trị
      this.entitlements = { ...this._freeDefaults, ...(response.features || {}) };
      this.plan = response.plan || { slug: 'trial', name: 'Trial' };
      this.lastFetch = Date.now();
      this._serverFetched = true;

      await this._saveCache();
      console.log('[SlncTrZ] FeatureGate: Đã cập nhật entitlements từ server (anonymous), plan:', this.plan?.slug);
    }
  ...
  async trackUsage(featureKey, action, quantity = 1) {
    try {
      if (this.isLoggedIn()) {
        ...
        console.log('[SlncTrZ] FeatureGate: Đã ghi nhận sử dụng (server)', featureKey, '+' + quantity);
      } else {
        // Anonymous: chỉ cập nhật local usage
        await this._incrementLocalUsage(featureKey, quantity);
        console.log('[SlncTrZ] FeatureGate: Đã ghi nhận sử dụng (local)', featureKey, '+' + quantity);
      }
    } catch (err) {
      console.warn('[SlncTrZ] FeatureGate: Lỗi ghi nhận sử dụng', featureKey, err.message);
    }
  }
  ...
  async canCreateTaskAsync() {
    ...
    // Đếm số task hiện tại trong storage
    let currentCount = 0;
    try {
      if (window.storageManager) {
        ...
        const stored = await this._storageGet('af_tasks');
        currentCount = Array.isArray(stored) ? stored.length : 0;
      }
    } catch (e) {
      console.warn('[FeatureGate] Error counting tasks:', e);
    }
  ...
  /**
   * Kiểm tra có thể tạo workflow không (async, đếm từ storage)
   * Check quota cho cả logged-in và anonymous users
   */
  async canCreateWorkflowAsync() {
    ...
    // Đếm số workflow hiện tại trong storage
    let currentCount = 0;
    try {
      if (window.storageManager) {
        ...
        const stored = await this._storageGet('af_workflows');
        currentCount = Array.isArray(stored) ? stored.length : 0;
      }
    } catch (e) {
      console.warn('[FeatureGate] Error counting workflows:', e);
    }
  ...
  async showModuleBlockedDialog(module) {
    if (!window.customDialog) {
      console.warn('[SlncTrZ] FeatureGate: customDialog chưa sẵn sàng');
      return;
    }
