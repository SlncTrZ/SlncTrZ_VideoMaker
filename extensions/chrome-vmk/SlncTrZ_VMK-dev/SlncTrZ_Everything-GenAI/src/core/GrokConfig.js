 ...
 *
 * Endpoint: GET /api/v1/system-config/grok (public, anonymous OK).
...
class GrokConfig {
  ...
  static async _doFetch() {
    ...
    // thay vì legacy /system-config/grok. Backend đã có ở provider_configs.api_config.
      ...
      try {
        chrome.runtime.sendMessage({
          ...
          if (resp?.success && errorPatterns && typeof errorPatterns === 'object') {
            ...
            // Fallback legacy nếu new endpoint không có data (vd v1 backend cũ)
            this._fallbackLegacy(resolve);
          }
        });
      } catch (e) {
  ...
  /** Fallback legacy /system-config/grok endpoint. */
  static _fallbackLegacy(resolve) {
    chrome.runtime.sendMessage({
      action: 'apiRequest',
      method: 'GET',
      endpoint: 'system-config/grok',
    }, async (resp) => {
