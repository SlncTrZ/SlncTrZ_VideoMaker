...
class PipelineFooter {
  ...
  _createDOM() {
    ...
    this._el.className = 'slnctrz-pipeline-footer slnctrz-pipeline-footer--hidden';
    this._el.innerHTML = `
      <div class="slnctrz-pipeline-footer__bar">
        <div class="slnctrz-pipeline-footer__progress-fill"></div>
        <div class="slnctrz-pipeline-footer__content">
          <span class="slnctrz-pipeline-footer__icon">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>
            </svg>
          </span>
          <span class="slnctrz-pipeline-footer__label">Pipeline</span>
          <span class="slnctrz-pipeline-footer__counter"></span>
          <span class="slnctrz-pipeline-footer__elapsed"></span>
          <span class="slnctrz-pipeline-footer__retry-status"></span>
          <div class="slnctrz-pipeline-footer__actions">
            <button class="slnctrz-pipeline-footer__btn slnctrz-pipeline-footer__btn--stop-all" title="${window.I18n?.t('pipeline.stopAll') || 'Dừng tất cả'}">
      ...
      <div class="slnctrz-pipeline-footer__jobs"></div>
    `;
    ...
    // Cache elements
    this._progressFill = this._el.querySelector('.slnctrz-pipeline-footer__progress-fill');
    this._labelEl = this._el.querySelector('.slnctrz-pipeline-footer__label');
    this._counterEl = this._el.querySelector('.slnctrz-pipeline-footer__counter');
    this._elapsedEl = this._el.querySelector('.slnctrz-pipeline-footer__elapsed');
    this._retryStatusEl = this._el.querySelector('.slnctrz-pipeline-footer__retry-status');
    this._jobsEl = this._el.querySelector('.slnctrz-pipeline-footer__jobs');
    this._stopAllBtn = this._el.querySelector('.slnctrz-pipeline-footer__btn--stop-all');
    ...
    // Toggle jobs expand
    this._el.querySelector('.slnctrz-pipeline-footer__bar').addEventListener('click', (e) => {
      if (e.target.closest('.slnctrz-pipeline-footer__btn')) return;
      if (this._state === 'completing') {
        this._hide();
        return;
      }
      this._jobsEl.classList.toggle('slnctrz-pipeline-footer__jobs--expanded');
    });
  }
  ...
  _showRetryStatus(text) {
    ...
    this._retryStatusEl.classList.add('slnctrz-pipeline-footer__retry-status--visible');
    ...
    this._retryTimer = setTimeout(() => {
      this._retryStatusEl.classList.remove('slnctrz-pipeline-footer__retry-status--visible');
  ...
  _show() {
    this._state = 'visible';
    this._el.className = 'slnctrz-pipeline-footer';
  ...
  _hide() {
    ...
    this._el.className = 'slnctrz-pipeline-footer slnctrz-pipeline-footer--hidden';
    this._el.removeAttribute('data-state');
    this._jobsEl.classList.remove('slnctrz-pipeline-footer__jobs--expanded');
  ...
  _renderJobs(jobs) {
    if (!this._jobsEl.classList.contains('slnctrz-pipeline-footer__jobs--expanded')) return;
    ...
    for (const j of jobs) {
      ...
      html += `<div class="slnctrz-pipeline-footer__job">`;
      html += `<span class="slnctrz-pipeline-footer__job-dot" style="background:${color};${!isPaused ? 'box-shadow:0 0 4px ' + color + ';' : ''}"></span>`;
      html += `<span class="slnctrz-pipeline-footer__job-label">${this._escHtml(j.label || j.owner)}</span>`;
      html += `<span class="slnctrz-pipeline-footer__job-pct">${jPct}%</span>`;

      // Per-job actions
      if (j.owner === 'prompts' && isPaused) {
        html += `<button class="slnctrz-pipeline-footer__job-btn slnctrz-pipeline-footer__job-btn--resume" data-action="resume" data-job-id="${j.id}" title="${window.I18n?.t('pipeline.resume') || 'Tiếp tục'}">
        ...
        html += `<button class="slnctrz-pipeline-footer__job-btn slnctrz-pipeline-footer__job-btn--pause" data-action="pause" data-job-id="${j.id}" title="${window.I18n?.t('pipeline.pause') || 'Tạm dừng'}">
      ...
      html += `<button class="slnctrz-pipeline-footer__job-btn slnctrz-pipeline-footer__job-btn--stop" data-action="stop" data-job-id="${j.id}" title="${window.I18n?.t('pipeline.stop') || 'Dừng'}">
