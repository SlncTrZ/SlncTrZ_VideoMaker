...
class GrokSession {
   ...
   *  3. Inject script (nếu chưa có flag __slnctrzGrokLoaded__).
   *  4. Kiểm tra login (form editor + login link).
   *
   * Trả về cache 60 giây nếu lần check trước thành công.
   *
   * @param {{ createIfMissing?: boolean, activate?: boolean, focusWindow?: boolean }} options
  ...
  static async ensureReady(options = {}) {
    ...
    const { createIfMissing = true, activate = true, focusWindow = false } = options;
  ...
  /**
   * Đảm bảo tab Grok active (giống ensureFlowTabActive / ChatGPT.ensureTabActive).
   * Dùng trước khi DOM interaction nặng.
   */
  static async ensureTabActive() {
    this._bindRuntimeListeners();
    if (!this._tabId) return;
    try {
      await this._sendToBackground('grok:ensureActive', { tabId: this._tabId });
    } catch (err) {
