...
class WorkflowTemplateList {
  ...
  render() {
    ...
    const canManageTemplates = window.authManager?.isAdmin?.() || false;
  ...
  async _apiCall(endpoint, method = 'GET', data = null) {
    ...
    // Anonymous: call via background.js
  ...
  _showVideoModalEmbed_DISABLED(videoUrl) {
    ...
    try {
      ...
      console.error('[SlncTrZ] Invalid video URL:', videoUrl, e);
      return;
    }

    if (!videoId) {
      console.error('[SlncTrZ] Could not extract YouTube video ID from:', videoUrl);
      return;
    }
  ...
  async _showUseTemplateConfirmation(template) {
    ...
    return new Promise((resolve) => {
      ...
      const modal = document.getElementById('useTemplateConfirmModal');
  ...
  async _copyTemplateToWorkflow(templateId) {
    ...
    const confirmed = await this._showUseTemplateConfirmation(template);
    if (!confirmed) {
