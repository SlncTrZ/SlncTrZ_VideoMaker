...
class WorkflowList {
  ...
  bindGlobalEvents() {
    if (window.eventBus) {
      ...
      window.eventBus.on('storage:workflow_deleted', () => this._debouncedLoadWorkflows());
    ...
    if (window.eventBus) {
      window.eventBus.on('project:changed', () => this.loadWorkflows());
    ...
    chrome.runtime.onMessage.addListener((msg) => {
      ...
      if (msg.action === 'workflowEditorClosed') {
        this._editingWfId = null;
        // Editor đóng → cần full refresh để sync state
        // Nếu đang trong cooldown, schedule load sau khi cooldown hết
  ...
  async _updateSingleWorkflowInList(wfId, partialData = null) {
    ...
    try {
      ...
      // Nếu workflow không có trong list (mới tạo?) → không làm gì, để loadWorkflows xử lý
    } catch (e) {
  ...
  _rerenderSingleWorkflowCard(wfId) {
    ...
    const dropdownMenu = card.querySelector('.slnctrz-dot-menu');
  ...
  _bindSingleCardEvents(card) {
    ...
    // 3-dot menu
    const dotMenu = card.querySelector('.slnctrz-dot-menu');
    if (dotMenu) {
      const menuBtn = dotMenu.querySelector('.slnctrz-dot-menu-btn');
      const dropdown = dotMenu.querySelector('.slnctrz-dropdown-menu');
  ...
  _updateCardRunningState(wfId, isRunning, finalStatus = null) {
    console.log('[WorkflowList] _updateCardRunningState:', wfId, isRunning, finalStatus);
    const card = this.container.querySelector(`.workflow-card[data-wf-id="${wfId}"]`);
    if (!card) {
      console.warn('[WorkflowList] Card not found for wfId:', wfId);
      return;
    }
  ...
  _bindToolbarEvents() {
    ...
    const handleCreate = async () => {
      // Check quota limit before opening editor
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        ...
        if (!canCreate) {
          window.featureGate.showLoginPrompt(window.I18n?.t('workflow.trialCreateExhausted') || 'Bạn đã hết lượt tạo workflow dùng thử. Đăng nhập để tiếp tục.');
          return;
        }
      }
  ...
  async render() {
    ...
    if (hasMore) {
      ...
      listContainer.insertAdjacentHTML('beforeend', `
        <div class="slnctrz-load-more-row">
          <button class="slnctrz-load-more-btn" id="wfLoadMoreBtn">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
            ${loadMoreLabel}
            <span class="slnctrz-load-more-count">${this.workflows.length} / ${this._total}</span>
          </button>
        </div>
      `);
    ...
    return `
      <div class="workflow-card" data-wf-id="${workflow.wf_id}" data-shared="true">
        ...
        <div class="workflow-card-actions">
          ...
          <div class="slnctrz-dot-menu" data-wf-id="${workflow.wf_id}">
            <button class="btn btn-secondary btn-sm slnctrz-dot-menu-btn" title="Menu">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="slnctrz-dropdown-menu hidden">
              <button class="slnctrz-dropdown-item view-shared-btn">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                  <circle cx="12" cy="12" r="3"></circle>
                </svg>
              ...
              <button class="slnctrz-dropdown-item duplicate-shared-btn">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div class="workflow-card-actions">
          ...
          ` : `
            <div class="slnctrz-dot-menu" data-wf-id="${workflow.wf_id}">
              ...
              <div class="slnctrz-dropdown-menu hidden">
                <button class="slnctrz-dropdown-item edit-btn">
                  ${isAdminViewing
                ...
                <button class="slnctrz-dropdown-item run-btn">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
                  ${window.I18n?.t('common.run') || 'Run'}
                </button>
                ` : ''}
                ${!isAdminViewing ? `
                <button class="slnctrz-dropdown-item reset-btn">
                ...
                <button class="slnctrz-dropdown-item share-btn ${!window.featureGate?.canUse('workflow_share_enabled') ? 'slnctrz-dropdown-item--locked' : ''}">
                ...
                ` : ''}
                ${!isAdminViewing ? `
                <button class="slnctrz-dropdown-item copy-btn">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                  ${window.I18n?.t('workflow.duplicate') || 'Duplicate'}
                </button>
                ` : ''}
                <button class="slnctrz-dropdown-item export-btn ${!window.featureGate?.canUse('workflow_export') ? 'slnctrz-dropdown-item--locked' : ''}">
                  <span class="wf-dropdown-icon-wrap">
                ...
                ${!isAdminViewing ? `
                <button class="slnctrz-dropdown-item delete-btn slnctrz-dropdown-danger" ${!canDelete ? 'disabled' : ''}>
  ...
  async _openWorkflow(wfId, cardEl) {
    ...
    if (cardEl) {
      ...
      if (actionsEl) {
        ...
        actionsEl.innerHTML = `<div class="slnctrz-loading-spinner" style="width:18px;height:18px;"></div>`;
      }
    }
  ...
  _closeAllDropdowns(container) {
    container?.querySelectorAll('.slnctrz-dropdown-menu').forEach(d => {
  ...
  async cloneWorkflow(wfId) {
    ...
    try {
      ...
      // Check quota limit for logged-in users
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        const quota = window.featureGate.checkQuota('workflows_max');
      ...
      if (!isCurrent && window.ProjectHelper) {
        ...
        const newWorkflow = {
          ...workflow,
          wf_id: newWfId,
          wf_name: (workflow.wf_name || 'Workflow') + ' ' + (window.I18n?.t('project.copySuffix') || '(bản sao)'),
          status: 'idle',
      ...
      // Record trial usage for not-logged-in users
  ...
  _formatRelativeTime(timestamp) {
    ...
    if (seconds < 60) return window.I18n?.t('common.loading') || 'Vừa xong';
  ...
  _showToast(message, duration = 2000) {
    let toast = document.querySelector('.slnctrz-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.className = 'slnctrz-toast';
  ...
  async _processImportFile(file) {
    ...
    try {
      ...
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        ...
        const canCreate = await window.featureGate.canCreateWorkflowAsync();
        if (!canCreate) {
          window.featureGate.showLoginPrompt(t('workflow.trialCreateExhausted') || 'Bạn đã hết lượt tạo workflow dùng thử. Đăng nhập để tiếp tục.');
          return;
        }
      }
      ...
      // 6. Record trial usage for not-logged-in users
      if (window.featureGate && !window.authManager?.isLoggedIn()) {
        await window.featureGate.recordWorkflowCreated();
      }
  ...
  static _renderStatusLabel(status) {
    ...
    if (s === 'error' || s === 'failed') return I?.t('workflow.statusFailed') || 'Thất bại';
    if (s === 'pending') return I?.t('workflow.statusPending') || 'Chờ xử lý';
    // idle: dùng common.idle hoặc fallback
    return I?.t('workflow.statusIdle') || I?.t('workflow.statusPending') || 'Sẵn sàng';
  }
  ...
  async handleDuplicateFromShared(wfId) {
    ...
    try {
      // Check quota trước khi duplicate
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        const quota = window.featureGate.checkQuota('workflows_max');
        if (quota.limit !== 'unlimited' && quota.limit > 0 && quota.used >= quota.limit) {
          const shouldUpgrade = await window.customDialog?.confirm(
