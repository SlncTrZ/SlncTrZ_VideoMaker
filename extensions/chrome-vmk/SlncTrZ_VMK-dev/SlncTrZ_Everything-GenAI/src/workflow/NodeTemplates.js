...
const NodeTemplates = {
  ...
  PORT_TYPES: {
    text:   { color: '#fbbf24', label: 'Text',   icon: 'T' },
    image:  { color: '#3b82f6', label: 'Image',  icon: 'I' },
    video:  { color: '#8b5cf6', label: 'Video',  icon: 'V' },
    any:    { color: '#d4d4d8', label: 'Any',    icon: '*' },
    frame:  { color: '#10b981', label: 'Frame',  icon: 'F' },
  },
  ...
  // Render media type indicator (image/video icon) — hiển thị bên cạnh title cho generate/grok node
  // mediaType: 'Image' | 'Video' (Generate node) hoặc grok_mode 'image'/'video' (Grok node)
  renderMediaTypeBadge(mediaType, isFrames = false) {
    if (!mediaType) return '';
    const isVideo = String(mediaType).toLowerCase() === 'video';
    const icon = isVideo ? this.icons.mediaVideo : this.icons.mediaImage;
    const cls = isVideo ? 'df-node-media-badge-video' : 'df-node-media-badge-image';
    const label = isVideo ? (isFrames ? 'Video Frames' : 'Video') : 'Image';
    return `<span class="df-node-media-badge ${cls}" title="${label}" data-tooltip="${label}">${icon}</span>`;
  },
  ...
  get types() {
    ...
    return {
      ...
      scheduler: {
        name: 'Scheduler', description: t('node.schedulerDesc', 'Kích hoạt workflow theo lịch (như n8n Cron)'),
        icon: 'schedule', color: 'scheduler', inputs: 0, outputs: 1, portType: 'any',
        ui: { trigger: true, terminal_sink: false },
        ports: {
          in: [],
          out: [{ name: 'trigger', type: 'any', label: t('node.portTrigger', 'Trigger output') }],
        },
      },
    ...
    if (type === 'image') {
      ...
      return `
        <div class="df-node ${!enabled ? 'df-node-disabled' : ''}${nodeHasPortsClass}" data-node-type="image" data-enabled="${enabled}">
          ...
          <div class="df-node-body">
            <div class="df-node-preview" data-node-preview>
    ...
    if (type === 'prompt') {
      ...
      return `
        <div class="df-node ${!enabled ? 'df-node-disabled' : ''}${nodeHasPortsClass}" data-node-type="prompt" data-enabled="${enabled}">
          ...
          <div class="df-node-body">
            ${promptText
              ? `<div class="df-node-prompt" title="${this.escapeAttr(promptText)}">${this.escapeHtml(promptText)}</div>`
              : `<div class="df-node-prompt df-node-prompt-empty">${window.I18n?.t('node.promptPlaceholder') || 'Nhập prompt...'}</div>`}
            <div class="df-node-settings-bar">
              ${modeBadge}
              ${refBadge}
              ${errorBadge}
      ...
      return `
        <div class="df-node ${!enabled ? 'df-node-disabled' : ''}${nodeHasPortsClass}" data-node-type="chatgpt" data-provider="openai" data-enabled="${enabled}">
          ...
          <div class="df-node-body">
            ...
            ${prompt ? `<div class="df-node-prompt" title="${this.escapeHtml(prompt)}">${this.escapeHtml(prompt)}</div>` : `<div class="df-node-prompt df-node-prompt-empty">${window.I18n?.t('node.promptPlaceholder') || 'Nhập prompt...'}</div>`}
            <div class="df-node-settings-bar">
    ...
    switch (type) {
      case 'generate': {
        ...
        const model = (isVideo ? settings?.defaultVideoModel : settings?.defaultImageModel)
          || window.ModelRegistry?.getDefault('flow', isVideo ? 'video' : 'image')
          || (isVideo ? 'Veo 3.1 - Fast' : 'Nano Banana 2');
  ...
  _SERVER_TYPES_TTL: 5 * 60 * 1000, // 5 phút — refresh khi user mở lại editor sau khi admin sửa cấu hình
  ...
  async _doFetch() {
    try {
      ...
      const resp = await fetch(`${baseUrl}/workflow-node-types`, {
        method: 'GET',
        headers,
