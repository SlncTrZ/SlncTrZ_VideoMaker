...
class WorkflowEditor {
  ...
  bindGlobalEvents() {
    ...
    if (window.eventBus) {
      ...
      this._ebHandlers = {};
      // PATCHED: Click node = mở form. Click+drag = kéo node (Drawflow tự xử lý).
      ...
      this._ebHandlers['edge:created'] = (data) => {
        ...
        try { this._refreshAllNodeWarningBadges(); } catch (e) {}
      ...
      this._ebHandlers['node:completed'] = async (data) => {
        ...
        if (Object.keys(resultThumbs).length > 0) {
          ...
          for (const [fid, info] of Object.entries(resultThumbs)) {
            if (info?.thumbnail) {
              ...
              // Persist type for video detection after reload
              thumbMap[fid] = type === 'video'
                ? { thumbnail: info.thumbnail, type: 'video' }
                : info.thumbnail;
              this._tileCacheSet(fid, { thumbnail: info.thumbnail, type });
            }
            if (info?.file_name) {
          ...
          if (Object.keys(thumbMap).length > 0) {
            ...
            console.log('[SlncTrZ] Persisted', Object.keys(thumbMap).length, 'result thumbnails for', nodeId);
          }
          if (Object.keys(fileNameMap).length > 0) {
            ...
            console.log('[SlncTrZ] Persisted', Object.keys(fileNameMap).length, 'result file_names for', nodeId);
          }
        }
        ...
        if (Object.keys(resultThumbs).length === 0 && data.result?.fileIds?.length > 0 && typeof MessageBridge !== 'undefined') {
          ...
          try {
            ...
            if (Object.keys(thumbMap).length > 0) {
              ...
              console.log('[SlncTrZ] Persisted (fallback)', Object.keys(thumbMap).length, 'thumbnails for', nodeId);
            }
          } catch (e) {
            console.warn('[SlncTrZ] Fallback scan thumbnails failed:', e.message);
          }
        }
        ...
        if (fallbackThumbMap && window.storageManager && this.workflow?.wf_id) {
          ...
          }).catch((err) => console.warn('[SlncTrZ] Persist fallback thumbnails failed:', err?.message));
        }
      };
      ...
      // Bug 32 fix (2026-05-19): Admin update Flow ratios / download_resolutions →
      // re-render node settings form đang mở (nếu node có dropdown affected).
      this._ebHandlers['provider:api_config_updated'] = ({ provider, key }) => {
        if (provider !== 'flow') return;
        if (key !== 'ratios' && key !== 'download_resolutions') return;
        if (!this.selectedNodeId || !this.overlay) return;
        if (this.overlay.classList.contains('hidden')) return;
        try {
          // Re-render settings form để dropdowns đọc fresh PCM data
          this._handleNodeSelected(this.selectedNodeId);
        } catch (e) {
      ...
      console.log('[WorkflowEditor] Bound provider:api_config_updated + provider:models_updated + validation_rules:updated + provider:api_configs_loaded listeners');
    }
  }
  ...
  _updateToolbarLockStates() {
    ...
    for (const { action, featureKey, normalSvg } of buttons) {
      const btn = this.overlay.querySelector(`.slnctrz-wf-tool-btn[data-action="${action}"]`);
      if (!btn) continue;
      const canUse = window.featureGate?.canUse(featureKey) ?? false;
      btn.classList.toggle('slnctrz-wf-tool-btn--locked', !canUse);
    ...
    if (type === 'prompt') {
      ...
      if (providerSel) {
        ...
        if (cgOpt) {
          ...
          cgOpt.textContent = canChatGPT ? 'ChatGPT' : 'ChatGPT (Pro)';
        }
        if (gmOpt) {
          gmOpt.disabled = !canGemini;
          gmOpt.textContent = canGemini ? 'Gemini' : 'Gemini (Pro)';
        }
      }
    }
  }
  ...
  _syncExecutionUI() {
    ...
    const toolbarPlayBtn = this.overlay.querySelector('.slnctrz-wf-tool-btn[data-action="run-workflow"]');
    const toolbarStopBtn = this.overlay.querySelector('.slnctrz-wf-tool-btn[data-action="stop-workflow"]');
      ...
      <div class="workflow-editor">
        <div class="workflow-editor-header">
          ...
          <div class="workflow-editor-actions">
            ...
            ${window.featureGate?.canManageWorkflowTemplates() && !this.isTemplateMode && this.workflow?.wf_id && !this.isReadOnly() ? `
            ...
            ` : ''}
            <button class="btn btn-primary ${this.isReadOnly() ? 'hidden' : ''}" id="saveWorkflowBtn">${this.isTemplateMode ? (this.templateId ? (window.I18n?.t('workflow.updateTemplate') || 'Cập nhật Template') : (window.I18n?.t('workflow.saveTemplateBtn') || 'Lưu Template')) : (this.mode === 'create' ? (window.I18n?.t('workflow.createBtn') || 'Tạo mới') : (window.I18n?.t('workflow.saveBtn') || 'Lưu'))}</button>
          </div>
        </div>
        <div class="workflow-editor-body">
          <div class="workflow-editor-center">
            <div class="slnctrz-wf-toolbar">
                <button class="slnctrz-wf-tool-btn ${this.isReadOnly() ? 'hidden' : ''}" data-action="add-node" title="${window.I18n?.t('workflow.addNodeShortcut') || 'Thêm node (N)'}" data-tooltip="${window.I18n?.t('workflow.addNodeShortcut') || 'Thêm node (N)'}" data-tooltip-pos="right">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </button>
                <button class="slnctrz-wf-tool-btn ${(this.mode === 'create' || this.isTemplateMode || this.isReadOnly()) ? 'hidden' : ''}" data-action="run-workflow" title="${window.I18n?.t('workflow.runShortcut') || 'Chạy (Ctrl+Enter)'}" data-tooltip="${window.I18n?.t('workflow.runShortcut') || 'Chạy (Ctrl+Enter)'}" data-tooltip-pos="right">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
                </button>
                <button class="slnctrz-wf-tool-btn hidden" data-action="stop-workflow" title="${window.I18n?.t('workflow.stopBtn') || 'Dừng'}" data-tooltip="${window.I18n?.t('workflow.stopBtn') || 'Dừng'}" data-tooltip-pos="right">
                ...
                <div class="slnctrz-wf-tool-divider ${this.isReadOnly() ? 'hidden' : ''}"></div>
                <button class="slnctrz-wf-tool-btn ${this.isReadOnly() ? 'hidden' : ''}" data-action="undo" id="wfUndoBtn" disabled title="${window.I18n?.t('workflow.undoShortcut') || 'Hoàn tác (Ctrl+Z)'}" data-tooltip="${window.I18n?.t('workflow.undoShortcut') || 'Hoàn tác (Ctrl+Z)'}" data-tooltip-pos="right">
                ...
                <button class="slnctrz-wf-tool-btn ${this.isReadOnly() ? 'hidden' : ''}" data-action="redo" id="wfRedoBtn" disabled title="${window.I18n?.t('workflow.redoShortcut') || 'Làm lại (Ctrl+Shift+Z)'}" data-tooltip="${window.I18n?.t('workflow.redoShortcut') || 'Làm lại (Ctrl+Shift+Z)'}" data-tooltip-pos="right">
                ...
                <div class="slnctrz-wf-tool-divider"></div>
                <button class="slnctrz-wf-tool-btn ${(this.isTemplateMode || this.isReadOnly()) ? 'hidden' : ''}" data-action="toggle-log" title="${window.I18n?.t('workflow.logAndProgress') || 'Log & tiến độ'}" data-tooltip="${window.I18n?.t('workflow.logAndProgress') || 'Log & tiến độ'}" data-tooltip-pos="right">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                </button>
                <button class="slnctrz-wf-tool-btn" data-action="fit-screen" title="${window.I18n?.t('workflow.fitScreen') || 'Vừa màn hình (F)'}" data-tooltip="${window.I18n?.t('workflow.fitScreen') || 'Vừa màn hình (F)'}" data-tooltip-pos="right">
                ...
                <button class="slnctrz-wf-tool-btn ${this.isReadOnly() ? 'hidden' : ''}" data-action="auto-layout" title="${window.I18n?.t('workflow.autoLayout') || 'Sắp xếp lại nodes'}" data-tooltip="${window.I18n?.t('workflow.autoLayout') || 'Sắp xếp lại nodes'}" data-tooltip-pos="right">
  ...
  /**
   * Refresh prompt source hint trong right sidebar form (chỉ cho node đang được mở form).
   * Hint hiển thị khi port "text" của node đang chỉnh sửa có edge connect từ Prompt node upstream.
   */
  _refreshAllPromptSourceBadges() {
    ...
    try {
      ...
      const formBody = overlay.querySelector('#nodeFormBody');
      if (!formBody) return;
      let banner = formBody.querySelector('[data-prompt-source-banner]');
      ...
      // (legacy workflows có thể lưu connections vào input_1 thay vì port "text")
      ...
      if (!banner) {
        ...
        banner.className = 'wf-form-prompt-source-banner';
        banner.setAttribute('data-prompt-source-banner', '');
        formBody.insertBefore(banner, formBody.firstChild);
      }
      const labelText = window.I18n?.t('workflow.promptFromUpstream') || 'Prompt từ';
      banner.innerHTML = `
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-top:2px;"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
        <div style="flex:1;min-width:0;">
          <div>${labelText} <strong title="${this.escapeAttr(upstreamName)}">${this.escapeHtml(upstreamName)}</strong></div>
        </div>
      `;
    } catch (e) { /* ignore */ }
  }
    ...
    for (const node of this.workflow.nodes) {
      ...
      if (node.result_thumbnails && typeof node.result_thumbnails === 'object') {
        for (const [fileId, thumbVal] of Object.entries(node.result_thumbnails)) {
          if (fileId && thumbVal && !this._tileCache.has(fileId)) {
            // Handle both formats: string (URL) or object { thumbnail, type }
            if (typeof thumbVal === 'object' && thumbVal.thumbnail) {
              this._tileCacheSet(fileId, { thumbnail: thumbVal.thumbnail, type: thumbVal.type || 'image' });
      ...
      if (node.ref_thumbnails && typeof node.ref_thumbnails === 'object') {
        const activeRefIds = node.ref_file_ids
          ...
          : null;
        for (const [fileId, thumbUrl] of Object.entries(node.ref_thumbnails)) {
          if (fileId && thumbUrl && !this._tileCache.has(fileId)) {
            if (!activeRefIds || activeRefIds.has(fileId)) {
              this._tileCacheSet(fileId, { thumbnail: thumbUrl, type: 'image' });
            }
          }
        }
      }
  ...
  _backgroundThumbnailScan(retryCount = 0) {
    ...
    console.log('[SlncTrZ] Background scan:', missingArr.length, 'missing thumbnails');
    ...
    const timerId = setTimeout(() => {
      MessageBridge.getThumbnailsByIds(missingArr).then(result => {
        ...
        if (found > 0) {
          console.log('[SlncTrZ] Background scan found', found, '/', missingArr.length, 'thumbnails');
        ...
        if (stillMissing.length > 0 && retryCount === 0) {
          console.log('[SlncTrZ] Background scan: còn', stillMissing.length, 'missing, đang chuẩn bị Flow...');
          MessageBridge.prepareFlowForScan().then(() => {
            ...
            if (retryFound > 0) {
              console.log('[SlncTrZ] Background scan retry found', retryFound, 'more thumbnails');
              this._directRenderFromCache(nodesMissingPreview);
              this._deferredThumbnailSave();
            }
            this._hideBackgroundScanLoading();
          }).catch(err => {
            console.warn('[SlncTrZ] Background scan retry failed:', err.message);
            this._hideBackgroundScanLoading();
          });
        } else {
          this._hideBackgroundScanLoading();
          if (stillMissing.length > 0) {
            console.log('[SlncTrZ] Background scan done,', stillMissing.length, 'thumbnails not found on Flow page');
          }
        }
      }).catch(err => {
        console.warn('[SlncTrZ] Background thumbnail scan failed:', err.message);
        this._hideBackgroundScanLoading();
      });
    }, delay);
  ...
  _correctStaleIds() {
    ...
    MessageBridge.correctStaleFileIds(idToUrlMap, fileNameMap).then(result => {
      ...
      if (crossProjectIds.length > 0) {
        console.log('[SlncTrZ] Cross-project detected:', crossProjectIds.length, 'IDs');
      ...
      console.log('[SlncTrZ] Corrected', Object.keys(corrections).length, 'stale tile IDs');
      ...
      console.warn('[SlncTrZ] correctStaleIds failed:', err.message);
    });
  }
  ...
  _updateCrossProjectNodePreviews(crossProjectIds) {
    ...
    for (const node of this.workflow.nodes) {
      ...
      console.log(`[SlncTrZ] Cross-project warning added to node ${node.node_id}: ref=${hasRefCross}, result=${hasResultCross}`);
    }
  }
  ...
  _updatePlayButtonState() {
    ...
    const toolbarRunBtn = this.overlay.querySelector('.slnctrz-wf-tool-btn[data-action="run-workflow"]');
  ...
  bindEvents() {
    ...
    const toolbar = this.overlay.querySelector('.slnctrz-wf-toolbar');
    toolbar?.addEventListener('click', (e) => {
      const btn = e.target.closest('.slnctrz-wf-tool-btn');
      if (!btn) return;
    ...
    window.eventBus?.on('node:download', (data) => {
      if (!data.nodeId) return;
      const nodeData = this.diagramCanvas?.editor?.getNodeFromId(data.nodeId);
      const fileIds = (nodeData?.data?.result_file_ids || '').split(',').filter(Boolean);
      if (fileIds.length === 0) return;
      const label = nodeData?.data?.prompt || nodeData?.data?.node_name || 'flow';
      const fileNames = nodeData?.data?.result_file_names || {};
      const wfName = this.workflow?.wf_name || null;
      for (const fileId of fileIds) {
        const fileName = fileNames[fileId] || null;
        if (typeof MessageBridge !== 'undefined') {
          MessageBridge.downloadTileMedia(fileId, label, wfName, fileName).catch(() => {});
        } else if (typeof downloadTileMedia === 'function') {
          downloadTileMedia(fileId, label, wfName, fileName).catch(() => {});
        }
      }
    });
    ...
    if (diagramContainer) {
      ...
      diagramContainer.addEventListener('drop', async (e) => {
        ...
        if (nodeType && this.diagramCanvas) {
          ...
          const nodeId = this.diagramCanvas.addNode(nodeType, posX, posY, {
            ...NodeTemplates.getDefaults(nodeType, afSettings),
            node_name: NodeTemplates.getType(nodeType).name,
            node_type: nodeType
          });
          if (nodeId) {
            this._hasUnsavedChanges = true;
            // Phase WK-1.5.3: refresh warning badges sau khi thêm node mới
            try { this._refreshAllNodeWarningBadges(); } catch (err) {}
  ...
  _renderPromptSourceRadio(data, nodeId) {
    ...
    // Badge hiển thị khi đang dùng upstream Prompt node
    const badgeHtml = !useOwnPrompt
      ? (upstreamPromptNode
          ? `<span class="prompt-source-badge">
              ${promptSourceIcon}
              ${window.I18n?.t('workflow.fromPromptNode', { name: this.escapeHtml(upstreamName) }) || `Từ: ${this.escapeHtml(upstreamName)}`}
            </span>`
          : `<span class="prompt-source-warning">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              ${window.I18n?.t('workflow.noUpstreamPrompt') || 'Chưa connect upstream Prompt node'}
            </span>`)
      : '';

    // Hint dynamic theo trạng thái toggle
    const hintOn = window.I18n?.t('workflow.promptSourceOwnHint') || 'Dùng prompt bên dưới để gen ảnh.';
    const hintOff = window.I18n?.t('workflow.promptSourceUpstreamHint') || 'Nhận prompt từ Prompt node phía trước (kéo edge vào port "text").';
    ...
    return `
      <div class="form-group prompt-source-group">
        ...
        <p class="prompt-source-hint" style="font-size: 11px; color: var(--muted-foreground); margin-top: 4px;">${useOwnPrompt ? hintOn : hintOff}</p>
        ${badgeHtml}
      </div>`;
  }
  ...
  /**
   * Render provider login reminder banner cho ChatGPT/Grok nodes
   * Nhắc user mở trang provider và đăng nhập để sử dụng tính năng
   * @param {string} provider - 'chatgpt' | 'grok'
   * @returns {string} HTML
   */
  _renderProviderLoginReminder(provider) {
    const providerLabels = { chatgpt: 'ChatGPT', grok: 'Grok' };
    const label = providerLabels[provider] || provider;
    const reminderText = window.I18n?.t('workflow.providerReminderText', { provider: label })
      || `Open ${label} page and login to use this feature`;
    const buttonText = window.I18n?.t('workflow.providerReminderButton', { provider: label })
      || `Open ${label}`;
    const providerId = provider === 'chatgpt' ? 'chatgptLoginReminder' : 'grokLoginReminder';
    return `
      <div class="form-group node-provider-reminder" id="${providerId}" data-provider="${provider}">
        <div class="provider-reminder-box">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;">
            <circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line>
          </svg>
          <span class="provider-reminder-text">${reminderText}</span>
  ...
  _renderNodeFormByType(nodeType, data, nodeId) {
    ...
    if (nodeType === 'scheduler') {
      return `
        <div class="form-group">
          <label for="nodeScheduleType">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align: -1px; margin-right: 4px;"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
            ${window.I18n?.t('workflow.scheduleType') || 'Loại lịch'}
          </label>
          <select id="nodeScheduleType" style="width:100%">
            <option value="interval" ${data.schedule_type === 'interval' ? 'selected' : ''}>${window.I18n?.t('workflow.scheduleInterval') || 'Lặp lại mỗi N phút'}</option>
            <option value="hourly" ${data.schedule_type === 'hourly' ? 'selected' : ''}>${window.I18n?.t('workflow.scheduleHourly') || 'Mỗi giờ'}</option>
            <option value="daily" ${data.schedule_type === 'daily' ? 'selected' : ''}>${window.I18n?.t('workflow.scheduleDaily') || 'Hàng ngày'}</option>
            <option value="cron" ${data.schedule_type === 'cron' ? 'selected' : ''}>${window.I18n?.t('workflow.scheduleCron') || 'Cron expression'}</option>
          </select>
        </div>
        <div class="form-group" id="nodeScheduleIntervalGroup" style="${data.schedule_type !== 'hourly' && data.schedule_type !== 'cron' ? '' : 'display:none'}">
          <label for="nodeScheduleInterval">${window.I18n?.t('workflow.scheduleIntervalMin') || 'Số phút'}</label>
          <input type="number" id="nodeScheduleInterval" min="1" max="1440" value="${data.schedule_interval || 15}" />
        </div>
        <div class="form-group" id="nodeScheduleDailyGroup" style="${data.schedule_type === 'daily' ? '' : 'display:none'}">
          <label for="nodeScheduleDailyTime">${window.I18n?.t('workflow.scheduleDailyTime') || 'Giờ chạy'}</label>
          <input type="time" id="nodeScheduleDailyTime" value="${data.schedule_daily_time || '08:00'}" />
        </div>
        <div class="form-group" id="nodeScheduleCronGroup" style="${data.schedule_type === 'cron' ? '' : 'display:none'}">
          <label for="nodeScheduleCronExpr">${window.I18n?.t('workflow.scheduleCronExpr') || 'Cron expression'}</label>
          <input type="text" id="nodeScheduleCronExpr" value="${data.schedule_cron_expr || '*/15 * * * *'}" placeholder="*/15 * * * *" />
          <small style="opacity:0.5">${window.I18n?.t('workflow.scheduleCronHint') || 'VD: */15 * * * * (15 phút), 0 * * * * (mỗi giờ), 0 8 * * * (8h sáng)'}</small>
        </div>
        <div class="form-group">
          <label>
            <input type="checkbox" id="nodeScheduleActive" ${data.schedule_active ? 'checked' : ''} />
            <span>${window.I18n?.t('workflow.scheduleActive') || 'Kích hoạt lịch trình'}</span>
          </label>
        </div>`;
    }
    if (nodeType === 'image') {
      ...
      if (isTemplateCtx) {
        ...
        return `${nameField}${refField}${enabledField}`;
      }
    ...
    if (nodeType === 'prompt') {
      ...
      // Quota hint label theo provider hiện tại
      const quotaActionLabel = provider === 'gemini' ? 'gemini_run' : 'chatgpt_run';
      return `${nameField}
        <div class="form-group">
          ...
          <textarea id="promptNodeText" style="height: 120px;" placeholder="${window.I18n?.t('workflow.promptNodePlaceholder') || 'VD: A cute cat playing with yarn, cinematic lighting'}">${this.escapeHtml(promptText)}</textarea>
        </div>
        <div class="form-group">
        ...
        <div class="form-group prompt-node-provider-section${providerHiddenClass}" id="promptNodeProviderSection">
          ...
          <div class="input-group select-group compact-select">
            <select id="promptNodeProvider">
              <option value="chatgpt" ${provider === 'chatgpt' ? 'selected' : ''} ${!canChatGPT ? 'disabled' : ''}>ChatGPT${!canChatGPT ? ' (Pro)' : ''}</option>
              <option value="gemini" ${provider === 'gemini' ? 'selected' : ''} ${!canGemini ? 'disabled' : ''}>Gemini${!canGemini ? ' (Pro)' : ''}</option>
            </select>
            <svg class="select-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </div>
          <p style="font-size: 11px; color: var(--warning); margin-top: 4px;">
            ...
            ${window.I18n?.t('workflow.promptProviderHint') || 'Khuyên dùng ChatGPT để có kết quả enhance tốt hơn. Gemini có thể tạo ảnh thay vì trả prompt.'}
          </p>
        </div>
        <div class="form-group prompt-node-provider-section${providerHiddenClass}" id="promptNodeTimeoutSection">
          ...
          <p style="font-size: 11px; color: var(--muted-foreground); margin-top: 4px;" id="promptNodeQuotaHint">${window.I18n?.t('workflow.promptQuotaHint', { action: quotaActionLabel }) || `Tốn 1 lượt ${quotaActionLabel} mỗi lần chạy`}</p>
        </div>
    ...
    if (nodeType === 'chatgpt') {
      ...
      const chatgptBrandHeader = `
        <div class="node-brand-header node-brand-header--chatgpt">
          ...
          <span class="node-brand-name">ChatGPT</span>
        </div>`;
      return `${chatgptBrandHeader}${nameField}
        ...
        <div class="form-group" id="chatgptImagePromptGroup">
          <label for="chatgptImagePrompt">
          ...
          <textarea id="chatgptImagePrompt" style="height: 120px;" placeholder="${window.I18n?.t('workflow.chatgptPromptPlaceholder') || 'VD: A cute cat playing in the garden...'}">${this.escapeHtml(cgPrompt)}</textarea>
        </div>
        <div class="form-group">
    ...
    if (nodeType === 'grok') {
      ...
      const grokBrandHeader = `
        <div class="node-brand-header node-brand-header--grok">
          ...
          <span class="node-brand-name">Grok</span>
        </div>`;

      return `${grokBrandHeader}${nameField}
        ...
        <div class="form-group" id="grokNodePromptGroup">
          ...
          <textarea id="grokNodePrompt" style="height: 100px;" placeholder="${window.I18n?.t('workflow.grokNodePlaceholder') || 'VD: A futuristic robot with neon lights, cinematic'}">${this.escapeHtml(grokPrompt)}</textarea>
        </div>
        <div class="form-group">
          <label>
    ...
    const googleBrandHeader = `
      <div class="node-brand-header node-brand-header--google">
        ...
        <span class="node-brand-name">Google Flow</span>
      </div>`;
    return `${googleBrandHeader}${nameField}
      ${genPromptSourceHtml}
      ${!canUseGenerate ? `
      ...
      <div class="form-group" id="nodePromptGroup">
        ...
        <textarea id="nodePrompt" style="height: 200px;">${this.escapeHtml(data.prompt || '')}</textarea>
      </div>
      ...
      <div class="gen-compact-bar">
        <div class="gen-compact-item" id="nodeImageModelGroup">
          <div class="input-group select-group compact-select">
            <select id="nodeModel">
              ${(window.ModelRegistry?.getModelsSync('flow', 'image') || []).map(m => `<option value="${this.escapeAttr(m.value)}" ${data.model === m.value ? 'selected' : ''}>${this.escapeHtml(m.name)}</option>`).join('')}
            </select>
            <svg class="select-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </div>
        </div>
        <div class="gen-compact-item hidden" id="nodeVideoModelGroup">
          <div class="input-group select-group compact-select">
            <select id="nodeVideoModel">
              ${(window.ModelRegistry?.getModelsSync('flow', 'video') || []).map(m => `<option value="${this.escapeAttr(m.value)}" ${data.model === m.value ? 'selected' : ''}>${this.escapeHtml(m.name.replace(/^Veo 3\.1 - /, 'Veo 3.1 '))}</option>`).join('')}
            </select>
            <svg class="select-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </div>
        </div>
        <div class="gen-compact-item hidden" id="nodeVideoInputTypeGroup">
          <div class="input-group select-group compact-select">
            <select id="nodeVideoInputType">
              ...
              <option value="Ingredients" ${data.video_input_type === 'Ingredients' ? 'selected' : ''}>Ingredients</option>
            </select>
            <svg class="select-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
          </div>
        </div>
          ...
          <div class="input-group select-group compact-select">
            <select id="nodeRatio">
              ${(() => {
                ...
                const _genRatios = (window.ProviderConfigManager?.getRatiosSync?.('flow', _genIsVideo ? 'video' : 'image'))
        ...
        <div class="gen-compact-item">
          <div class="input-group compact-qty">
            ...
            <input type="number" id="nodeQuantity" min="${window.ValidationRules?.getInt?.('quantity_min', 1) || 1}" max="${window.ValidationRules?.getInt?.('quantity_max', 4) || 4}" value="${data.quantity || 1}" />
    ...
    if (status === 'completed' && fileIds.length > 0) {
      ...
      const renderThumb = (id) => {
        ...
        const badgeHtml = hasOrig
          ...
          : '';
        return `<div class="node-result-thumb ${ratioClass}${hasOrig ? ' has-provider-url' : ''}" data-file-id="${this.escapeAttr(id)}" data-provider="${provider}"><div class="slnctrz-loading-spinner" style="width:16px;height:16px;"></div>${badgeHtml}</div>`;
      };
      ...
      html += `
        <div class="form-group">
          <div style="display: flex; align-items: center; gap: 8px; padding: 8px 0;">
            <div class="slnctrz-loading-spinner" style="width:16px;height:16px;"></div>
    ...
    for (let i = 0; i < fileIds.length && i < thumbEls.length; i++) {
      ...
      if (isCrossProject) {
        ...
        thumbEl.innerHTML = isVideo
          ? `<video src="${mediaSrc}" muted loop autoplay playsinline style="width:100%;height:100%;object-fit:cover;border-radius:6px;"></video>`
    ...
    if (!hasAny && typeof MessageBridge !== 'undefined') {
      ...
      if (missingIds.length > 0) {
        MessageBridge.getThumbnailsByIds(missingIds).then(result => {
          ...
          for (const [fid, info] of Object.entries(results)) {
            if (info?.thumbnail) {
              this._tileCacheSet(fid, { thumbnail: info.thumbnail, type: info.type || 'image', file_name: info.file_name });
            }
          }
            ...
            if (cached) {
              ...
              if (isCrossProject) {
                ...
                thumbEls[i].innerHTML = isVid
                  ? `<video src="${cached.thumbnail}" muted loop autoplay playsinline style="width:100%;height:100%;object-fit:cover;border-radius:6px;"></video>`
  ...
  _bindNodeFormEvents(nodeType, data, nodeId) {
    ...
    if (providerReminderBtn) {
      providerReminderBtn.addEventListener('click', (e) => {
        ...
        chrome.runtime.sendMessage({ action: 'openProviderTab', provider }, (resp) => {
          if (resp?.ok) {
    ...
    if (['generate', 'chatgpt', 'grok'].includes(nodeType)) {
      ...
      if (toggle) {
        ...
        const promptGroupId =
          (nodeType === 'chatgpt') ? '#chatgptImagePromptGroup'
        ...
        const hintEl = this.overlay?.querySelector('.prompt-source-hint');
        const hintOn = window.I18n?.t('workflow.promptSourceOwnHint') || 'Dùng prompt bên dưới để gen ảnh.';
        const hintOff = window.I18n?.t('workflow.promptSourceUpstreamHint') || 'Nhận prompt từ Prompt node phía trước (kéo edge vào port "text").';
        ...
        const updateState = () => {
          ...
          if (hintEl) {
            hintEl.textContent = useOwnPrompt ? hintOn : hintOff;
          }
        };
    ...
    // === PROMPT NODE (Phase CG-8) ===
    if (nodeType === 'prompt') {
      // Toggle Enhance ON/OFF → show/hide provider+timeout sections + update hint
      ...
      const quotaHint = this.overlay?.querySelector('#promptNodeQuotaHint');
      const updateQuotaHint = () => {
        if (!quotaHint) return;
        const provider = providerSelect?.value || 'chatgpt';
        const action = provider === 'gemini' ? 'gemini_run' : 'chatgpt_run';
        const tpl = window.I18n?.t('workflow.promptQuotaHint', { action });
        // I18n.t trả về key nếu chưa có translation → dùng fallback Vi.
        quotaHint.textContent = (tpl && !tpl.startsWith('workflow.'))
          ? tpl
          : `Tốn 1 lượt ${action} mỗi lần chạy`;
      };
      ...
      providerSelect?.addEventListener('change', updateQuotaHint);
      // Initial state đã set qua HTML render — chỉ refresh hint cho đồng bộ.
      updateQuotaHint();
            ...
            onConfirm: async (images) => {
              ...
              for (const img of flowImages) {
                if (img.fileId && img.thumbnail) {
                  this._tileCacheSet(img.fileId, { thumbnail: img.thumbnail, file_name: img.file_name || '', type: 'image' });
                }
              }
    ...
    this.overlay?.querySelector('#nodeQtyMinus')?.addEventListener('click', () => {
      const val = parseInt(nodeQtyInput?.value) || 1;
      if (val > 1 && nodeQtyInput) nodeQtyInput.value = val - 1;
    });
    this.overlay?.querySelector('#nodeQtyPlus')?.addEventListener('click', () => {
      const val = parseInt(nodeQtyInput?.value) || 1;
      if (val < 4 && nodeQtyInput) nodeQtyInput.value = val + 1;
    });
    ...
    pickerBtn?.addEventListener('click', () => {
      ...
      if (window.imagePickerModal) {
        ...
        window.imagePickerModal.open({
          existingFileIds: existingIds,
          mediaFilter: 'image',
          maxSelections: ImagePickerModal.resolveMaxSelections({
            provider: 'flow',
  ...
  _captureFormSnapshot() {
    ...
    const fields = ['nodeName', 'nodePrompt', 'nodeMediaType', 'nodeModel', 'nodeVideoModel',
      ...
      'chatgptImagePrompt', 'chatgptImageRatio', 'chatgptImageRefFileIds', 'chatgptImageMode',
      ...
      'nodeScheduleType', 'nodeScheduleInterval', 'nodeScheduleDailyTime', 'nodeScheduleCronExpr', 'nodeScheduleActive'];
    const snapshot = {};
    fields.forEach(id => {
  ...
  _reselectNode(nodeId) {
    ...
    try {
      ...
      console.warn('[SlncTrZ] Re-select node failed:', e);
    }
  }
  ...
  _updateNodeRatioOptions() {
    ...
    const ratios = (window.ProviderConfigManager?.getRatiosSync?.('flow', mode)) || fallback;
  ...
  /**
   * Extract file_name (UUID) from URL containing getMediaUrlRedirect
  ...
  _truncateRefFileIds(fileIdsStr, containerSelector) {
    ...
    if (ids.length > refLimit) {
      console.log(`[WorkflowEditor] Ref images vượt giới hạn (${ids.length}/${refLimit}), chỉ lưu ${refLimit} ảnh đầu tiên`);
      return ids.slice(0, refLimit).join(', ');
    }
    return fileIdsStr;
  }
  ...
  _syncUploadKeyToAllNodes(data) {
    ...
    const editorData = this.diagramCanvas.editor.export();
    const homeData = editorData?.drawflow?.Home?.data || {};
    let updatedCount = 0;

    for (const [drawflowId, nodeInfo] of Object.entries(homeData)) {
      const nodeData = nodeInfo?.data;
      if (!nodeData?.ref_file_ids) continue;

      const ids = nodeData.ref_file_ids.split(',').map(s => s.trim()).filter(Boolean);
      const idx = ids.indexOf(key);
      if (idx === -1) continue;

      // Replace upload_xxx với tile_id
      ids[idx] = tile_id;
      nodeData.ref_file_ids = ids.join(', ');

      // Update ref_thumbnails
      if (thumbnail_url) {
        if (!nodeData.ref_thumbnails) nodeData.ref_thumbnails = {};
        nodeData.ref_thumbnails[tile_id] = thumbnail_url;
        delete nodeData.ref_thumbnails[key];
      }

      // Update ref_file_names
      if (file_name) {
        if (!nodeData.ref_file_names) nodeData.ref_file_names = {};
        nodeData.ref_file_names[tile_id] = file_name;
        delete nodeData.ref_file_names[key];
      }

      // Update _tileCache
      this._tileCacheSet(tile_id, {
        thumbnail: thumbnail_url || this._tileCache.get(key)?.thumbnail || '',
  ...
  async hideNodeForm({ skipUploadCheck = false } = {}) {
    ...
    if (applyToNodeId && panel && !panel.classList.contains('hidden') && !this.isReadOnly()) {
  ...
  _applyNodeFormData(targetNodeId = null) {
    ...
    if (nodeType === 'note') {
    ...
    } else if (nodeType === 'scheduler') {
      data.schedule_type = this.overlay?.querySelector('#nodeScheduleType')?.value || 'interval';
      data.schedule_interval = parseInt(this.overlay?.querySelector('#nodeScheduleInterval')?.value) || 15;
      data.schedule_daily_time = this.overlay?.querySelector('#nodeScheduleDailyTime')?.value || '08:00';
      data.schedule_cron_expr = this.overlay?.querySelector('#nodeScheduleCronExpr')?.value || '*/15 * * * *';
      data.schedule_active = !!this.overlay?.querySelector('#nodeScheduleActive')?.checked;
      ...
      data.prompt = this.overlay?.querySelector('#chatgptImagePrompt')?.value || '';
      ...
      data.model = mediaType === 'Video'
        ? (this.overlay?.querySelector('#nodeVideoModel')?.value || window.ModelRegistry?.getDefault('flow', 'video') || 'Veo 3.1 - Fast')
        : (this.overlay?.querySelector('#nodeModel')?.value || window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro');
      // Preserve existing ratio if form element is empty/missing
    ...
    if (typeof data.ref_file_ids === 'string' && !this.isTemplateMode) {
      ...
      // Sync ref_thumbnails: chỉ giữ các key có trong ref_file_ids
      if (node.data?.ref_thumbnails) {
        const syncedThumbs = {};
        for (const key of Object.keys(node.data.ref_thumbnails)) {
          if (currentRefIds.has(key)) {
            syncedThumbs[key] = node.data.ref_thumbnails[key];
          }
        }
        data.ref_thumbnails = syncedThumbs;
      }
      // Sync ref_file_names: chỉ giữ các key có trong ref_file_ids
      if (node.data?.ref_file_names) {
        const syncedNames = {};
        for (const key of Object.keys(node.data.ref_file_names)) {
          if (currentRefIds.has(key)) {
            syncedNames[key] = node.data.ref_file_names[key];
          }
        }
        data.ref_file_names = syncedNames;
      }
    }
  ...
  _restoreNodePreviewAfterUpdate(nodeId, nodeType) {
    ...
    // Emit để undo history schedule snapshot (debounced 400ms)
    try { window.eventBus?.emit('node:data_changed', { nodeId: this.selectedNodeId }); } catch (e) {}
  }
  ...
  async saveNode() {
    ...
    try {
      ...
      console.error('[SlncTrZ] saveNode failed:', error);
  ...
  async saveWorkflow() {
    ...
    if (this.mode === 'create') {
      // For logged-in users: check FeatureGate
      if (window.authManager?.isLoggedIn() && window.featureGate) {
        const quota = window.featureGate.checkQuota('workflows_max');
        console.log('[SlncTrZ] Workflow quota check:', quota);
        // Only block if limit is valid (> 0) and used >= limit
      ...
      // For trial users (not logged in): check FeatureGate with async method
      else if (window.featureGate) {
        const canCreate = await window.featureGate.canCreateWorkflowAsync();
        if (!canCreate) {
          window.featureGate.showLoginPrompt(window.I18n?.t('workflow.trialWorkflowLimit') || 'Bạn đã tạo tối đa số workflow cho phép trong bản dùng thử.');
          return;
        }
      }
    }
    ...
    if (this._isSaving) {
      ...
      console.log('[WorkflowEditor] saveWorkflow() queued — another save in progress');
    ...
    try {
      ...
      if (saveBtn) {
        ...
        saveBtn.innerHTML = `<span class="slnctrz-loading-spinner" style="width:14px;height:14px;border-width:2px;display:inline-block;vertical-align:-2px;margin-right:4px;"></span>${window.I18n?.t('common.saving') || 'Saving...'}`;
      }
      ...
      this._updatePlayButtonState();
      const workflowName = this.overlay?.querySelector('#workflowName')?.value?.trim();
      ...
      if (!window.storageManager) {
        console.error('[SlncTrZ] storageManager chưa khởi tạo');
          ...
          const fixedIds = ids.map(id => {
            if (id.startsWith('upload_')) {
              ...
              if (realTileId && typeof realTileId === 'string' && !realTileId.startsWith('upload_')) {
                console.log('[SlncTrZ] saveWorkflow: replaced upload key', id, '->', realTileId);
                return realTileId;
              }
            }
            return id;
      ...
      await window.storageManager.saveWorkflowFull(workflowData, cleanNodes, edges);
      ...
      if (this.mode === 'create') {
        ...
        this.overlay?.querySelector('.slnctrz-wf-tool-btn[data-action="run-workflow"]')?.classList.remove('hidden');
        this.overlay?.querySelector('.slnctrz-wf-tool-btn[data-action="export-workflow"]')?.classList.remove('hidden');
      ...
      console.error('[SlncTrZ] Save FAILED:', error);
      ...
      if (this._pendingSaveRequest) {
        this._pendingSaveRequest = false;
        console.log('[WorkflowEditor] Processing queued save request');
        // Use setTimeout to avoid stack overflow và cho UI update
        setTimeout(() => this.saveWorkflow().catch(e => console.warn('[WorkflowEditor] Queued save failed:', e)), 50);
      }
    }
  }
  ...
  _convertNodesToTemplateFormat(nodes) {
    return nodes.map(node => {
      ...
      const copyFields = [
        ...
        // Scheduler fields
        'schedule_type', 'schedule_interval', 'schedule_daily_time', 'schedule_cron_expr', 'schedule_active',
        // Telegram settings
  ...
  async exportWorkflow() {
    ...
    try {
      ...
      console.log('[SlncTrZ] Workflow exported:', filename);
    } catch (error) {
      console.error('[SlncTrZ] Export failed:', error);
      const dialog = window.customDialog || window.CustomDialog;
  ...
  async _runSingleNode(drawflowId) {
    ...
    if (!actualNodeId) {
      console.error('[SlncTrZ] Cannot resolve node_id from drawflow ID:', drawflowId);
      return;
    }
    ...
    if (!this.workflow?.nodes?.find(n => n.node_id === actualNodeId)) {
      console.error('[SlncTrZ] Node not found in saved workflow after save:', actualNodeId);
    ...
    try {
      ...
      console.warn('[SlncTrZ] Reset node status failed (non-critical):', e.message);
    }
  ...
  async _resetWorkflowFromEditor() {
    ...
    try {
      ...
      if (saveBtn) {
        saveBtn.disabled = false;
        if (saveBtn.innerHTML.includes('slnctrz-loading-spinner')) {
  ...
  _clearNodePreview(nodeId) {
    ...
    previewContainer.classList.remove('image-ref', 'hidden');
    ...
    if (!hasDom && !allCached && typeof MessageBridge !== 'undefined') {
      ...
      MessageBridge.getThumbnailsByIds(uncachedIds).then(result => {
        for (const [fid, info] of Object.entries(result?.results || {})) {
          ...
          this._tileCacheSet(fid, { thumbnail: info.thumbnail, type: info.type || 'image', ...(newFn && { file_name: newFn }) });
        }
        renderThumbs();
      }).catch(() => renderThumbs());
    } else {
  ...
  _renderNodePreviewInner(previewContainer, fileIds, attempt) {
    ...
    for (const tileId of fileIds.slice(0, 4)) {
      let mediaSrc = '';
      let isVideo = false;
    ...
    if (fileIds.length > 4) {
      const more = document.createElement('div');
      more.className = 'df-preview-more';
      more.textContent = `+${fileIds.length - 4}`;
  ...
  _deferredThumbnailSave() {
    ...
    this._deferredSaveTimer = setTimeout(() => {
      this._deferredSaveTimer = null;
      this.saveWorkflow()
        .catch(err => console.warn('[SlncTrZ] Deferred thumbnail save failed:', err))
  ...
  async _scanRefFileNames(refIds, drawflowId) {
    ...
    try {
      ...
      console.warn('[SlncTrZ] _scanRefFileNames error:', e);
    }
  }
  ...
  async _cacheNodeRefImageBlobs(nodeData) {
    ...
    for (const id of refIds) {
      ...
      try {
        ...
        let resp;
        if (fetchUrl.includes('getMediaUrlRedirect')) {
        ...
        if (resp?.success && resp.base64) {
          ...
          console.log(`[SlncTrZ] Proactive cached blob cho ref ${id.substring(0, 8)}`);
        }
      } catch (e) {
        // Không block — fire-and-forget
        console.warn(`[SlncTrZ] Failed to cache blob cho ref ${id.substring(0, 8)}:`, e.message);
      }
    }
  }
  ...
  _refreshExpiredNodeThumbnail(previewContainer, fileIds) {
    ...
    this._expiredNodeTimer = setTimeout(() => {
      if (typeof MessageBridge === 'undefined') return;
      const expiredIds = fileIds.filter(id => !id.startsWith('upload_'));
      if (expiredIds.length === 0) return;
      MessageBridge.getThumbnailsByIds(expiredIds).then(result => {
        const results = result?.results || {};
        for (const [fid, info] of Object.entries(results)) {
          if (info?.thumbnail) {
            this._tileCacheSet(fid, { thumbnail: info.thumbnail, type: info.type || 'image' });
          }
        }
        // Re-render preview with fresh URLs
  ...
  async _checkRefFilesExist(nodeDataList) {
    ...
    try {
      ...
      console.warn('[SlncTrZ] Check tiles exist failed:', e.message);
      return null;
    }
  }
   ...
   */
  async _downloadNodeFiles({ source = 'original' } = {}) {
    if (!this.selectedNodeId || !this.diagramCanvas?.editor) return;
    ...
    if (source === 'original') {
      // Download chỉ tiles có provider URL gốc. Tile không có URL → bỏ qua + log warning.
      const providerTiles = fileIds.filter(id => providerUrls[id]?.url);
      if (providerTiles.length === 0) {
        window.showNotification?.('Node này chưa có bản gốc provider. Hãy chạy lại workflow để extension lưu URL gốc.', 'warning', 3000);
        return;
      }
      ...
      console.log('[SlncTrZ] Manual download (Original) — tiles:', providerTiles.length, providers);
      ...
      for (const fileId of providerTiles) {
        try {
          ...
          console.warn('[SlncTrZ] Provider download failed:', fileId, e);
        }
      }
      return;
    }

    // source='flow' — download qua Flow context menu (chọn 1k/2k/720p/1080p).
    window.showNotification?.(`Đang tải ${fileIds.length} file qua Google Flow...`, 'info', 2000);
    console.log('[SlncTrZ] Manual download (Flow) — tiles:', fileIds.length);
    ...
    for (const fileId of fileIds) {
      try {
        ...
        console.warn('[SlncTrZ] Download failed:', fileId, e);
      }
    }
  }

  /**
   * Dual URL — download tile từ URL provider gốc (Grok/ChatGPT) chất lượng 100%.
   * Fetch qua background.js executeScript với cookie session của tab provider → base64 → blob → chrome.downloads.
   * Pattern giống auto-download trong _executeChatGPTImageNode + _executeGrokImageNode.
   */
  async _downloadProviderTile(fileId, providerData, promptText, index, nodeData, fileName) {
    ...
    if (!url || !window.MessageBridge) {
      console.warn('[SlncTrZ] _downloadProviderTile: missing url/MessageBridge', { fileId, provider });
      return;
    }

    try {
      // Fetch URL gốc qua provider tab cookie session
      const fetchFn = provider === 'chatgpt'
        ...
        : provider === 'grok'
          ...
          : null;
      if (!fetchFn) {
        console.warn('[SlncTrZ] _downloadProviderTile: no fetcher for provider', provider);
        return;
      }

      const fetchResp = await fetchFn(url, tab_id);
      if (!fetchResp?.success || !fetchResp.base64) {
        console.warn('[SlncTrZ] _downloadProviderTile fetch fail:', provider, fetchResp?.error);
        // Fallback: download qua Flow tile (nếu có) — provider URL có thể đã expire
        if (typeof MessageBridge !== 'undefined') {
          const isVideo = media_type === 'video';
          const res = isVideo ? (nodeData.video_download_resolution || '720p') : (nodeData.download_resolution || '1k');
          await MessageBridge.downloadTileMedia(fileId, promptText, null, fileName, res);
        }
        return;
      }

      // Convert base64 → blob → blobUrl → chrome.downloads
      ...
      if (!dlResp?.success) {
        console.warn(`[SlncTrZ] ${provider} manual download fail:`, dlResp?.error);
      } else {
        console.log(`[SlncTrZ] ${provider} manual download OK:`, filename);
      }
    } catch (err) {
      console.error('[SlncTrZ] _downloadProviderTile exception:', err);
    }
  }
  ...
  async _showNodePicker(posX, posY, sourceNodeId = null, portContext = null) {
    ...
    // Hiển thị tất cả node có trong local types.
    ...
    const isAllowedNode = ([typeKey, _cfg]) => !!NodeTemplates.types[typeKey];
    ...
    picker.className = 'slnctrz-node-picker';
    const closeBtnHtml = `<button type="button" class="slnctrz-node-picker-close" title="${window.I18n?.t('workflow.kbdClose') || 'Close'}" aria-label="${window.I18n?.t('workflow.kbdClose') || 'Close'}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`;
    const headerHtml = portContext ? `
      <div class="slnctrz-node-picker-context-hint">
        <span class="slnctrz-node-picker-context-text">${window.I18n?.t('workflow.suggestForPort') || 'Gợi ý cho port'} <span class="slnctrz-node-picker-port-tag" data-port-type="${portContext.portType}">${this.escapeHtml(portContext.portLabel || portContext.portName)}</span></span>
        ${closeBtnHtml}
      </div>
    ` : `<div class="slnctrz-node-picker-context-hint slnctrz-node-picker-context-hint--no-text">${closeBtnHtml}</div>`;
    picker.innerHTML = `
      ${headerHtml}
      <div class="slnctrz-node-picker-search">
        <input type="text" placeholder="${window.I18n?.t('workflow.searchNode') || 'Tìm node...'}" class="slnctrz-node-picker-input" autofocus>
      </div>
      <div class="slnctrz-node-picker-list">
        ${Object.entries(nodeTypes)
          ...
          .map(([key, config]) => {
            ...
            return `
            <button class="slnctrz-node-picker-item ${config.comingSoon ? 'slnctrz-node-picker-disabled' : ''}" data-type="${key}" ${config.comingSoon ? 'disabled' : ''}>
              <div class="df-node-icon ${config.color}">${iconSvg}</div>
              <div class="slnctrz-node-picker-info">
                <div class="slnctrz-node-picker-name">${config.name}${premiumBadge}${config.comingSoon ? ` <span style="font-size:10px;color:var(--warning,#f59e0b);margin-left:4px;">(${window.I18n?.t('workflow.comingSoon') || 'Sắp ra mắt'})</span>` : ''}</div>
                <div class="slnctrz-node-picker-desc">${config.description}</div>
              </div>
            </button>
          `;}).join('')}
      </div>
      <div class="slnctrz-node-picker-footer">
    ...
    const input = picker.querySelector('.slnctrz-node-picker-input');
    setTimeout(() => input?.focus(), 50);

    // Filter on type
    input?.addEventListener('input', () => {
      const query = input.value.toLowerCase();
      picker.querySelectorAll('.slnctrz-node-picker-item').forEach(item => {
        const name = item.querySelector('.slnctrz-node-picker-name')?.textContent.toLowerCase() || '';
        const desc = item.querySelector('.slnctrz-node-picker-desc')?.textContent.toLowerCase() || '';
    ...
    picker.querySelector('.slnctrz-node-picker-close')?.addEventListener('click', (e) => {
      e.stopPropagation();
      e.preventDefault();
      this._hideNodePicker();
    });
    ...
    picker.addEventListener('click', async (e) => {
      const item = e.target.closest('.slnctrz-node-picker-item');
      if (!item) return;
    ...
    // Keyboard nav
    let selectedIdx = 0;
    const items = () => [...picker.querySelectorAll('.slnctrz-node-picker-item:not([style*="display: none"])')];
    const highlight = () => {
  ...
  _ensureNodeCornerGears() {
    ...
    nodeRoots.forEach((nodeEl) => {
      if (nodeEl.querySelector(':scope > .df-node-corner-gear')) return;
  ...
  _showInlineSettingDropdown(anchorEl, drawflowId, setting) {
    ...
    // Bug 41 fix (2026-05-13): Quantity range source từ ValidationRules (admin tweak qua
     // /admin/validation-rules → SSE validation_rules:updated → cache invalidate).
     const _qMin = window.ValidationRules?.getInt?.('quantity_min', 1) || 1;
     const _qMax = window.ValidationRules?.getInt?.('quantity_max', 4) || 4;
     const _qty = [];
    ...
    const optionsMap = {
      ...
      ratio: (() => {
        ...
        const _flowRatios = (window.ProviderConfigManager?.getRatiosSync?.('flow', isVideo ? 'video' : 'image'))
    ...
    dropdown.addEventListener('click', async (e) => {
      ...
      try {
        ...
        // Group C: model lists từ ModelRegistry; ratios từ ProviderConfigManager.getRatiosSync.
        if (setting === 'mediaType') {
          const VIDEO_MODELS = (window.ModelRegistry?.getValuesList('flow', 'video')) || ['Veo 3.1 - Lite', 'Veo 3.1 - Fast', 'Veo 3.1 - Quality'];
          const IMAGE_MODELS = (window.ModelRegistry?.getValuesList('flow', 'image')) || ['Nano Banana Pro', 'Nano Banana 2'];
          const VIDEO_RATIOS = (window.ProviderConfigManager?.getRatiosSync('flow', 'video')) || ['16:9', '9:16'];

          if (newValue === 'Video') {
            // Reset model CHỈ KHI current model không phải Veo (incompatible với Video)
            if (!VIDEO_MODELS.includes(updated.model)) {
              updated.model = window.ModelRegistry?.getDefault('flow', 'video') || 'Veo 3.1 - Lite';
            }
            if (!updated.video_input_type) updated.video_input_type = 'Frames';
            // Video chỉ hỗ trợ 16:9 và 9:16 — fix ratio nếu incompatible
            if (!VIDEO_RATIOS.includes(updated.ratio)) {
              updated.ratio = '16:9';
            }
          } else if (newValue === 'Image') {
            if (!IMAGE_MODELS.includes(updated.model)) {
              updated.model = window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana Pro';
            }
            // Bỏ video-specific fields để không bị lưu rác trong DB
  ...
  async _createNodeFromPicker(type, posX, posY, sourceNodeId) {
    ...
    const newId = this.diagramCanvas.addNode(type, posX, posY, {
      ...NodeTemplates.getDefaults(type, afSettings),
      node_name: NodeTemplates.getType(type).name || type,
      node_type: type
    });
    if (newId) {
      this._hasUnsavedChanges = true;
  ...
  _bindKeyboardShortcuts() {
    this._keyHandler = (e) => {
      ...
      // Block Ctrl+S (save shortcut) ở read-only
      if (e.ctrlKey && (e.key === 's' || e.key === 'S') && readOnly) {
        e.preventDefault();
      }
    };
  ...
  _showWorkflowSettings() {
    ...
    dialog.className = 'slnctrz-wf-settings-overlay';
    dialog.innerHTML = `
      <div class="slnctrz-wf-settings-dialog">
        <div class="slnctrz-wf-settings-header">
          <h3>${window.I18n?.t('workflow.settings') || 'Workflow Settings'}</h3>
          <button class="slnctrz-wf-settings-close" title="${window.I18n?.t('common.close') || 'Close'}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <div class="slnctrz-wf-settings-body">
          <div class="slnctrz-wf-settings-group">
          ...
          <div class="slnctrz-wf-settings-divider">${window.I18n?.t('workflow.execution') || 'Thực thi'}</div>
          <div class="slnctrz-wf-settings-group slnctrz-wf-settings-row">
            <label>${window.I18n?.t('workflow.delayBetweenNodes') || 'Chờ giữa các node'}</label>
            <div class="slnctrz-wf-settings-input-group">
        ...
        <div class="slnctrz-wf-settings-footer">
    ...
    chrome.storage.local.get('af_entitlements', (result) => {
      ...
      if (retryInput && !canUseRetry) {
        ...
        const group = retryInput.closest('.slnctrz-wf-settings-group');
    ...
    dialog.querySelector('.slnctrz-wf-settings-close')?.addEventListener('click', () => dialog.remove());
  ...
  async _showTemplateSettingsModal() {
    ...
    const dialog = document.createElement('div');
    dialog.className = 'slnctrz-wf-settings-overlay template-settings-modal';
    dialog.innerHTML = `
      <div class="slnctrz-wf-settings-dialog">
        <div class="slnctrz-wf-settings-header">
          <h3>
          ...
          <button class="slnctrz-wf-settings-close" title="${t('common.close', 'Đóng')}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
        <div class="slnctrz-wf-settings-body">
          ...
          <div class="slnctrz-wf-settings-group">
            ...
            <span class="slnctrz-wf-settings-hint">${t('workflow.saveTemplate.thumbnailSizeHint', 'Khuyến nghị: 640×360px hoặc 1280×720px (tỉ lệ 16:9)')}</span>
          </div>
          ...
          <div class="slnctrz-wf-settings-group">
            ...
            <span class="slnctrz-wf-settings-hint">${t('workflow.saveTemplate.videoUrlHint', 'Link video YouTube demo template')}</span>
          </div>

          <div class="slnctrz-wf-settings-divider">${t('workflow.templateOptions', 'Tùy chọn')}</div>
  ...
  _renderSharedUsersAvatars(shares) {
    ...
    return `<div class="wf-share-avatars wf-share-avatars--editor">${avatarsHtml}</div>`;
  }
  ...
  _dispatchToolbarAction(action, opts = {}) {
    ...
    if (action === 'add-node') {
      ...
      if (window.workflowExecutor?.isRunning) window.workflowExecutor.stop();
  ...
  _showCanvasContextMenu(clientX, clientY, canvasX, canvasY) {
    ...
    menu.addEventListener('click', async (e) => {
      ...
      // Pass canvas coords for add-node placement at right-click position
      const opts = action === 'add-node' && this._contextMenuCanvasPos
  ...
  _updateUndoRedoButtons() {
    ...
    if (undoBtn) {
      ...
      undoBtn.classList.toggle('slnctrz-wf-tool-btn--disabled', !canUndo);
    }
    if (redoBtn) {
      const canRedo = this.history?.canRedo();
      redoBtn.disabled = !canRedo;
      redoBtn.classList.toggle('slnctrz-wf-tool-btn--disabled', !canRedo);
    }
  }
