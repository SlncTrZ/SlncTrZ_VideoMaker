...
class GenTab {
  ...
  // ─── State ──────────────────────────────────────────────
  static savedTemplates = [];   // Array of { title, prompt, category }
  ...
  static templateSheetUrl = null;
  static fetchTemplatesBtn = null;
  static templateLoadStatus = null;
  static promptTemplatesSection = null;
  static templateSelect = null;
  static templatePreview = null;
  static insertTemplateBtn = null;
  static replaceTemplateBtn = null;
  ...
  static init() {
    console.log('[SlncTrZ] GenTab.init()');
    ...
    GenTab.templateSheetUrl = document.getElementById('templateSheetUrl');
    GenTab.fetchTemplatesBtn = document.getElementById('fetchTemplatesBtn');
    GenTab.templateLoadStatus = document.getElementById('templateLoadStatus');
    GenTab.promptTemplatesSection = document.getElementById('promptTemplatesSection');
    GenTab.templateSelect = document.getElementById('templateSelect');
    GenTab.templatePreview = document.getElementById('templatePreview');
    GenTab.insertTemplateBtn = document.getElementById('insertTemplateBtn');
    GenTab.replaceTemplateBtn = document.getElementById('replaceTemplateBtn');
    ...
    if (openImagePickerBtn) {
      openImagePickerBtn.addEventListener('click', () => {
        ...
        if (window.imagePickerModal) {
          ...
          // Post-audit fix: resolve maxSelections theo provider + mediaType hiện tại của Gen tab.
          const currentProvider = (GenTab.providerSelect?.value || 'flow').toLowerCase();
          const mediaTypeEl = document.getElementById('genTabMediaType');
          const currentMode = (mediaTypeEl?.value || 'Image').toLowerCase();
          const videoInputType = (document.getElementById('genTabVideoInputType')?.value || '').toLowerCase();
          const isFrames = currentMode === 'video' && videoInputType === 'frames';
    ...
    if (GenTab.quantitySelect) GenTab.quantitySelect.addEventListener('change', GenTab.updatePromptCount);
    if (GenTab.multiPromptCheck) {
      GenTab.multiPromptCheck.addEventListener('change', () => {
        ...
        // Show/hide run mode button (only relevant for multi-prompt)
        const runModeRow = document.querySelector('.gen-run-mode-row');
        if (runModeRow) {
          runModeRow.classList.toggle('hidden', !isMulti);
          // BUG FIX: Update run mode button styling when showing
          if (isMulti) {
            GenTab._updateRunModeUI();
          }
        }
    ...
    if (GenTab.startBtn) GenTab.startBtn.addEventListener('click', async () => {
      ...
      if (window.ExecutionGate) {
        try {
          const gate = await ExecutionGate.request('generate', promptCount, { owner: 'prompts', label: 'Auto Gen' });
          if (!gate.allowed) {
          ...
          console.error('[GenTab] ExecutionGate request failed, proceeding:', e.message);
        }
      }
        ...
        PromptQueue.getInstance().submitJob({
          ...
          if (window.eventBus) {
            ...
            window.eventBus.emit('generation:complete', {
              message: `Hoàn tất ${result.completed} ảnh/video${result.failed > 0 ? `, ${result.failed} thất bại` : ''}`,
      ...
      // Fallback: dùng MessageBridge hoặc runAutoPrompt trực tiếp (hành vi cũ)
        ...
        window.MessageBridge.runAutoPrompt(payload).then(result => {
          ...
          // SS-Phase F: Track prompt_total cho legacy Flow path (pipeline OFF). Pipeline ON đã được
          // EditorExecutor track riêng từng QueueItem → 2 path không double count nhau.
          try {
            const legacyPromptCount = payload.prompts?.length || 0;
            if (legacyPromptCount > 0 && window.EditorExecutor?._incrementDailyStat) {
              window.EditorExecutor._incrementDailyStat('prompt_total', legacyPromptCount);
            }
          } catch (_) { /* noop */ }
          // Emit tracker completed
          if (window.eventBus) {
            ...
            // Emit generation:complete for NotificationManager
            window.eventBus.emit('generation:complete', {
              message: `Hoàn tất ${payload.prompts?.length || 0} prompt(s)`,
              prompt: payload.prompts?.[0] || '',
              resultCount: legacyResultIds.length || payload.prompts?.length || 0
            });
          }
    ...
    // ─── Templates feature ────────────────────────────────
    if (GenTab.templateSelect) {
      GenTab.templateSelect.addEventListener('change', () => {
        const idx = parseInt(GenTab.templateSelect.value);
        const tpl = GenTab.savedTemplates[idx];
        if (tpl && GenTab.templatePreview) {
          GenTab.templatePreview.textContent = tpl.prompt;
          GenTab.templatePreview.classList.remove('hidden');
          if (GenTab.insertTemplateBtn) GenTab.insertTemplateBtn.disabled = false;
          if (GenTab.replaceTemplateBtn) GenTab.replaceTemplateBtn.disabled = false;
        } else {
          if (GenTab.templatePreview) GenTab.templatePreview.classList.add('hidden');
          if (GenTab.insertTemplateBtn) GenTab.insertTemplateBtn.disabled = true;
          if (GenTab.replaceTemplateBtn) GenTab.replaceTemplateBtn.disabled = true;
        }
      });
    }

    if (GenTab.insertTemplateBtn) {
      GenTab.insertTemplateBtn.addEventListener('click', () => {
        const idx = parseInt(GenTab.templateSelect?.value);
        const tpl = GenTab.savedTemplates[idx];
        if (!tpl || !GenTab.promptsArea) return;
        const current = GenTab.promptsArea.value.trim();
        GenTab.promptsArea.value = current ? `${current}\n${tpl.prompt}` : tpl.prompt;
        GenTab.promptsArea.dispatchEvent(new Event('input', { bubbles: true }));
      });
    }

    if (GenTab.replaceTemplateBtn) {
      GenTab.replaceTemplateBtn.addEventListener('click', () => {
        const idx = parseInt(GenTab.templateSelect?.value);
        const tpl = GenTab.savedTemplates[idx];
        if (!tpl || !GenTab.promptsArea) return;
        GenTab.promptsArea.value = tpl.prompt;
        GenTab.promptsArea.dispatchEvent(new Event('input', { bubbles: true }));
      });
    }

    if (GenTab.fetchTemplatesBtn) {
      GenTab.fetchTemplatesBtn.addEventListener('click', async () => {
        const url = GenTab.templateSheetUrl?.value?.trim();
        if (!url) { if (window.customDialog) window.customDialog.alert('Vui lòng nhập link Google Sheet Templates!', { title: 'Thiếu link', type: 'warning' }); return; }

        const sheetIdMatch = url.match(/\/d\/([a-zA-Z0-9_-]+)/);
        if (!sheetIdMatch) { if (window.customDialog) window.customDialog.alert('Link Google Sheet không hợp lệ!', { title: 'Link sai', type: 'warning' }); return; }

        const sheetId = sheetIdMatch[1];
        GenTab.fetchTemplatesBtn.textContent = 'Đang tải...';
        GenTab.fetchTemplatesBtn.disabled = true;
        ...
        try {
          ...
          // Expected columns: A=title, B=prompt, C=category (optional)
          for (let i = 1; i < lines.length; i++) {
            const cols = GenTab.parseCSVLine(lines[i]);
            const title = (cols[0] || '').replace(/^"|"$/g, '').trim();
            const prompt = (cols[1] || '').replace(/^"|"$/g, '').trim();
            const category = (cols[2] || '').replace(/^"|"$/g, '').trim();
            if (title && prompt) {
              templates.push({ title, prompt, category });
            }
          }

          GenTab.savedTemplates = templates;
          GenTab.renderTemplateSelect();
          GenTab.saveState();

          if (GenTab.templateLoadStatus) {
            GenTab.templateLoadStatus.textContent = `Đã tải ${templates.length} templates`;
            GenTab.templateLoadStatus.classList.remove('hidden');
            GenTab.templateLoadStatus.style.color = 'var(--success)';
          }
        } catch (err) {
          if (GenTab.templateLoadStatus) {
            GenTab.templateLoadStatus.textContent = `Lỗi: ${err.message}`;
            GenTab.templateLoadStatus.classList.remove('hidden');
            GenTab.templateLoadStatus.style.color = 'var(--destructive)';
          }
        }

        GenTab.fetchTemplatesBtn.textContent = 'Tải Templates';
        GenTab.fetchTemplatesBtn.disabled = false;
      });
    }
    ...
    console.log('[SlncTrZ] GenTab initialized');
  }
  ...
  static _initProviderSelector() {
    ...
    chrome.storage.local.get(['af_settings'], (res) => {
      ...
      try {
        if (window.ProviderRegistry && typeof ProviderRegistry.get === 'function') {
          if (!ProviderRegistry.get(initial)) initial = 'flow';
        }
      } catch (e) {
        initial = 'flow';
      }
    ...
    if (window.eventBus && !GenTab._providerGateBound) {
      ...
      // Admin update ratios / download_resolutions / max_ref_images qua /admin/providers → re-render
   ...
   * không xác định. Show warning + suggest user đóng bớt 1 tab.
   * Fire-and-forget: query qua chrome.tabs background handler.
   */
  static _checkDuplicateProviderTabs(providerKey) {
    ...
    try {
      chrome.runtime.sendMessage({ action: 'queryProviderTabs', provider: providerKey }, (resp) => {
        ...
        if (count > 1) {
          const providerName = providerKey === 'chatgpt' ? 'ChatGPT' : (providerKey === 'grok' ? 'Grok' : 'Flow');
      ...
      try {
        ...
        if (currentGate?.locked && currentGate.reason === 'feature') {
          ...
          if (targetProvider) {
            ...
            if (typeof window.showNotification === 'function') {
              try {
                const providerNames = { flow: 'Google Flow', chatgpt: 'ChatGPT', grok: 'Grok' };
                const msg = window.I18n?.t?.('gen.autoSwitchProvider', { from: providerNames[currentProvider], to: providerNames[targetProvider] })
                  || `Đã chuyển sang ${providerNames[targetProvider]} do mất quyền ${providerNames[currentProvider]}`;
    ...
    if (providerKey === 'flow') {
      ...
      if (lastProvider === 'chatgpt' || lastProvider === 'grok') {
        ...
        chrome.storage.local.get(['af_settings'], (res) => {
          ...
          const validFlowRatios = window.ProviderConfigManager?.getRatiosSync?.('flow', 'image')
  ...
  static async _updateProviderStatus(providerKey) {
    ...
    try {
      ...
      if (result?.ready) {
        ...
        badge.textContent = window.I18n?.t('gen.providerStatusReady') || 'Sẵn sàng';
      } else if (result?.error === 'NOT_LOGGED_IN' || result?.error === 'NO_TAB') {
        badge.classList.add('is-warning');
        badge.textContent = window.I18n?.t('gen.providerStatusLogin') || 'Chưa đăng nhập';
      } else {
        badge.classList.add('is-blocked');
        badge.textContent = window.I18n?.t('gen.providerStatusBlocked') || 'Bị chặn';
      }
    } catch (err) {
  ...
  static async _submitViaChatGPT(params) {
    ...
    //    Cap maxRefImages TRƯỚC khi fetch để tiết kiệm network calls.
    const maxRef = adapter.capabilities?.maxRefImages || 4;
    const fileIdsCapped = fileIds.slice(0, maxRef);
    if (fileIds.length > maxRef) {
      sendLog(`ChatGPT: vượt giới hạn ${maxRef} ảnh ref — chỉ gửi ${maxRef} ảnh đầu`, 'warn');
    }

    const refImagesCapped = [];
    for (const fid of fileIdsCapped) {
    ...
    const downloadTemplate = settingsObj.downloadTemplate || '[Date]_[Project]_[Prompt]_[Index]';
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      try {
        const result = await adapter.submit({
          prompt,
          refFileIds: refImagesCapped,
          settings: {
            ratio: settings.ratio || settingsObj.chatgptDefaultRatio || 'story',
            imageMode: true,
            fallbackPrefix: settingsObj.chatgptFallbackPrefix || 'Generate an image of: '
          },
          taskName: null
        });
            ...
            for (let urlIdx = 0; urlIdx < result.imageUrls.length; urlIdx++) {
              ...
              try {
                ...
                if (fetchResp?.success && fetchResp.base64) {
                  ...
                  // Revoke blob URL sau khi gửi yêu cầu (chrome đã queue download)
    ...
    if (window.eventBus) {
      ...
      window.eventBus.emit('generation:complete', {
        message: `Hoàn tất ChatGPT: ${completed} thành công, ${failed} thất bại`,
        prompt: prompts[0] || '',
  ...
  static async _submitViaGrok(params) {
    ...
    //    Cap maxRefImages TRƯỚC fetch để tiết kiệm network.
    const maxRef = adapter.capabilities?.maxRefImages || 4;
    const fileIdsCapped = fileIds.slice(0, maxRef);
    if (fileIds.length > maxRef) {
    ...
    if (window.eventBus) {
      ...
      window.eventBus.emit('generation:complete', {
        message: `Hoàn tất Grok: ${completed} thành công, ${failed} thất bại`,
        prompt: prompts[0] || '',
        resultCount: completed,
        failedCount: failed
      });
  ...
  static updateRatioOptions() {
    ...
    const options = ratios.map(value => ({ value, label: `${GenTab._ratioIcon(value)} ${value}` }));
  ...
  static getRefLimit() {
    ...
    const resolved = (typeof ImagePickerModal !== 'undefined' && ImagePickerModal.resolveMaxSelections)
      ? ImagePickerModal.resolveMaxSelections({ provider, mode, isFrames })
      : null;
  ...
  static importTxtFile(file) {
    ...
    reader.onload = (e) => {
      ...
      if (lines.length === 0) {
        console.log('[SlncTrZ] File trống hoặc chỉ chứa comment');
        if (window.customDialog) {
      ...
      GenTab.updatePromptCount();
      GenTab.saveState();
      console.log(`[SlncTrZ] Đã import ${lines.length} prompt(s) từ file "${file.name}"`);
    ...
    reader.onerror = () => {
      console.error('[SlncTrZ] Lỗi đọc file:', reader.error);
      if (window.customDialog) {
  ...
  static loadState(state) {
    // Restore prompt text + fileIds (user work-in-progress)
    if (state.prompts && GenTab.promptsArea) GenTab.promptsArea.value = state.prompts;
    if (state.fileIds && GenTab.fileIdsInput) GenTab.fileIdsInput.value = state.fileIds;
    ...
    // S5: Restore ref image names FIRST (before _updateRefModeUI calls _refreshMentionHelper)
    if (state.refImageNames) {
      GenTab.refImageNames = state.refImageNames;
    }

    // S5.1: Cleanup orphan names (names for file IDs not in current ref images)
    ...
    if (state.savedTemplates) {
      GenTab.savedTemplates = state.savedTemplates;
      GenTab.renderTemplateSelect();
    }
    if (state.templateSheetUrl && GenTab.templateSheetUrl) {
      GenTab.templateSheetUrl.value = state.templateSheetUrl;
    }
  ...
  // Hardcoded fallback addons when API fails
  static _FALLBACK_ADDONS = [
    { id: 'fallback_1', name: 'Anime', content: 'anime style, vibrant colors, cel shading', category: 'Nghệ thuật' },
    { id: 'fallback_2', name: 'Realistic', content: 'photorealistic, highly detailed, 8k resolution', category: 'Nghệ thuật' },
    { id: 'fallback_3', name: 'Cinematic', content: 'cinematic lighting, dramatic atmosphere, film grain', category: 'Nghệ thuật' },
    { id: 'fallback_4', name: 'Watercolor', content: 'watercolor painting, soft colors, artistic brushstrokes', category: 'Nghệ thuật' },
    { id: 'fallback_5', name: 'Minimalist', content: 'minimalist design, clean lines, simple composition', category: 'Thiết kế' }
  ];
  ...
  static async _loadAddonPrompts() {
    ...
    if (fetched) {
      ...
      console.log('[SlncTrZ] Using fallback addon prompts');
      GenTab.addonPrompts = GenTab._FALLBACK_ADDONS;
    }
    GenTab._renderAddonPromptSelect();
  }
  ...
  static async _fetchAddonPromptsFromApi() {
    return new Promise((resolve) => {
      if (!window.authManager?.apiBaseUrl) { resolve(null); return; }
      ...
      chrome.storage.local.get(['af_auth'], (authResult) => {
        ...
        chrome.runtime.sendMessage({
          ...
          if (chrome.runtime.lastError) {
            console.warn('[SlncTrZ] Addon prompts API error:', chrome.runtime.lastError.message);
            resolve(null);
            return;
          }
          ...
          if (response?.success && Array.isArray(response.data)) {
            ...
            console.warn('[SlncTrZ] Addon prompts API failed:', response?.error?.message);
            resolve(null);
          }
        });
      });
  ...
  static _saveStateImmediate() {
    ...
    // KHÔNG lưu thumbnailCache, fileNameCache (session-only, base64 rất lớn)
    const state = {
      prompts: GenTab.promptsArea.value,
      fileIds: GenTab.fileIdsInput.value,
      ...
      refImageNames: GenTab.refImageNames || {},
      // Addon prompt
  ...
  static async performFlowImageUpload(filesArray, executeBtnElement) {
    ...
    const existingTiles = Array.from(document.querySelectorAll('div[data-tile-id]')).map(el => el.getAttribute('data-tile-id'));
    ...
    while(waitTime < maxWait) {
      ...
      const currentTiles = Array.from(document.querySelectorAll('div[data-tile-id]'));
  ...
  static renderTemplateSelect() {
    if (!GenTab.templateSelect) return;
    GenTab.templateSelect.innerHTML = '<option value="">-- Chọn template --</option>';

    if (GenTab.savedTemplates.length === 0) {
      if (GenTab.promptTemplatesSection) GenTab.promptTemplatesSection.style.display = 'none';
      return;
    }

    if (GenTab.promptTemplatesSection) GenTab.promptTemplatesSection.style.display = 'block';

    // Group by category
    const categories = {};
    GenTab.savedTemplates.forEach((t, i) => {
      const cat = t.category || 'Chung';
      if (!categories[cat]) categories[cat] = [];
      categories[cat].push({ ...t, index: i });
    });
    ...
    Object.entries(categories).forEach(([cat, items]) => {
      ...
      GenTab.templateSelect.appendChild(optgroup);
    });
  }
  ...
  static _updateRefModeUI() {
    ...
    // Ẩn/hiện mention helper (chỉ cho mention mode)
    if (mentionHelper) {
      mentionHelper.classList.toggle('hidden', mode !== 'mention');
      if (mode === 'mention') {
        GenTab._refreshMentionHelper();
  ...
  static _editRefImageName(fileId, currentName) {
    ...
    title.textContent = 'Tên ảnh (dùng cho @mention)';
  ...
  static _showConfirmModal(promptCount) {
    return new Promise((resolve) => {
      ...
      if (isFlow) {
        if (confirmQty && GenTab.quantitySelect) confirmQty.value = GenTab.quantitySelect.value;
        ...
        if (confirmRes) {
          if (isVideo) {
            confirmRes.innerHTML = '<option value="">Không</option><option value="720p">720p</option><option value="1080p">1080p</option><option value="4k">4K (Ultra)</option>';
          } else {
            confirmRes.innerHTML = '<option value="">Không</option><option value="1k">1K</option><option value="2k">2K (Pro)</option><option value="4k">4K (Ultra)</option>';
          }
          if (autoDownloadToggle?.checked) {
