...
class MyPromptsTab {
  ...
  static _renderList() {
    ...
    if (hasMore) {
      ...
      cardsHtml += '<div class="slnctrz-load-more-row">' +
        '<button class="slnctrz-load-more-btn" id="myPromptsLoadMoreBtn">' +
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>' +
          loadMoreLabel +
          '<span class="slnctrz-load-more-count">' + (paginationInfo.loaded || 0) + ' / ' + (paginationInfo.total || 0) + '</span>' +
        '</button>' +
      '</div>';
    }
