...
(function() {
  ...
  function log(...args) {
    if (DEBUG) console.log('[SlncTrZ]', ...args);
  }
  ...
  function sidebarLog(msg, level = 'info') {
    ...
    const logTabBtn = document.querySelector('.slnctrz-tab[data-tab="tab-logs"]');
  ...
  window.showNotification = function(message, type = 'success', duration = 2500) {
    // Remove existing
    const existing = document.querySelector('.slnctrz-notification');
    if (existing) {
      clearTimeout(_notifTimer);
      existing.remove();
    }
    const el = document.createElement('div');
    el.className = `slnctrz-notification ${type}`;
    el.innerHTML = `${_notifIcons[type] || _notifIcons.info}<span>${message}</span>`;
    document.body.appendChild(el);
    _notifTimer = setTimeout(() => {
      el.classList.add('slnctrz-notif-out');
  ...
  function _showCloudflareToast(msg) {
    const existing = document.getElementById('slnctrz-cloudflare-toast');
    ...
    if (msg.phase === 'resolved') {
      ...
      if (existing) {
        existing.classList.add('slnctrz-notif-out');
    ...
    if (!el) {
      ...
      el.id = 'slnctrz-cloudflare-toast';
      el.className = 'slnctrz-cloudflare-toast';
      document.body.appendChild(el);
    }
    el.classList.toggle('is-urgent', isUrgent);
    el.innerHTML = `
      <div class="slnctrz-cf-toast-icon">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
      </div>
      <div class="slnctrz-cf-toast-body">
        <div class="slnctrz-cf-toast-title">${titleText}</div>
        <div class="slnctrz-cf-toast-subtitle">${subtitleText}</div>
      </div>
      <button type="button" class="slnctrz-cf-toast-btn" data-action="focus-tab">
        ${openTabText}
      </button>
    `;
  ...
  function _showConnectingOverlay() {
    const overlay = document.getElementById('slnctrz-connecting-overlay');
    if (overlay) {
  ...
  async function _pollProviderLogin(provider, tabId) {
    ...
    if (state.active) {
      console.log(`[SlncTrZ] Already polling ${provider} login`);
      return;
    }
    ...
    console.log(`[SlncTrZ] Start polling ${provider} login status, tabId=${tabId}`);

    const poll = async () => {
      // Timeout check
      if (Date.now() - startTime > TIMEOUT) {
        console.log(`[SlncTrZ] ${provider} login polling timeout`);
      ...
      try {
        ...
        if (!tab) {
          console.log(`[SlncTrZ] ${provider} tab closed, stop polling`);
          _stopLoginPolling(provider);
          return;
        }
        ...
        if (resp?.success && resp?.ready) {
          console.log(`[SlncTrZ] ${provider} login detected!`);
          _stopLoginPolling(provider);
        ...
        console.warn(`[SlncTrZ] ${provider} login poll error:`, err.message);
      }
    };
  ...
  async function _checkQueueAndReloadIfEmpty(caller = 'workflow') {
    try {
      ...
      if (!queue) {
        console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): PromptQueue not available, skip`);
      ...
      console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): activeJobs=${activeJobs}, pendingItems=${pendingItems}, isRunning=${isRunning}`);

      // Nếu queue đang có tác vụ → không reload, để tiếp tục bình thường
      if (activeJobs > 0 || pendingItems > 0 || isRunning) {
        console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): queue not empty, skip reload`);
        return true;
      }

      // Queue trống → reload Flow page
      console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): queue empty, reloading Flow page...`);
      ...
      if (window.MessageBridge) {
        ...
        try {
          ...
          console.warn(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): reload message failed:`, e.message);
        ...
        if (!ready) {
          console.warn(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): editor not ready after reload`);
          window.showNotification?.(
          ...
          console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): reload done in ${reloadDuration}ms`);
          window.showNotification?.(
          ...
          console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): ready too fast (${reloadDuration}ms), skip notification`);
        }

        // Extra settle delay cho React render
        await new Promise(r => setTimeout(r, 1500));

        console.log(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}): reload complete`);
      }

      return true;
    } catch (err) {
      console.error(`[SlncTrZ] _checkQueueAndReloadIfEmpty(${caller}) error:`, err.message);
  ...
  async function _saveProjectToList(projectId, projectName) {
    ...
    try {
      ...
      console.warn('[SlncTrZ] _saveProjectToList error:', e.message);
    }
  }
  ...
  async function _removeProjectFromList(projectId) {
    ...
    try {
      ...
      console.warn('[SlncTrZ] _removeProjectFromList error:', e.message);
    }
  }
  ...
  async function _cleanupStaleProjects() {
    try {
      ...
      if (changed) {
        await chrome.storage.local.set({ af_projects: projects });
        console.log('[SlncTrZ] Đã dọn projects cũ (>30 ngày)');
      }
    } catch (e) {
      console.warn('[SlncTrZ] _cleanupStaleProjects error:', e.message);
    }
  }
  ...
  async function _syncProjectsFromFlow() {
    try {
      ...
      console.warn('[SlncTrZ] _syncProjectsFromFlow error:', e.message);
    }
  }
  ...
  async function _toggleProjectDropdown() {
    ...
    if (sorted.length === 0) {
      ...
      dropdown.innerHTML = sorted.map(([id, data]) => {
        ...
        const date = data.last_accessed ? new Date(data.last_accessed).toLocaleDateString('vi-VN') : '';
    ...
    dropdown.querySelectorAll('.project-indicator-item[data-project-id]').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        const _flowBase = window.ProviderConfigManager?.getBaseUrlSync('flow') || 'https://labs.google/fx/tools/flow';
  ...
  async function _createNewProject() {
    ...
    const handleClickResp = (resp) => {
      if (resp?.success) {
        ...
        console.warn('[SlncTrZ] Không tìm thấy nút tạo dự án:', resp?.result?.error || resp?.error);
    ...
    if (homepageTab) {
      ...
      const _flowHome = window.ProviderConfigManager?.getBaseUrlSync('flow') || 'https://labs.google/fx/tools/flow';
      chrome.runtime.sendMessage({
        action: 'navigateToProject',
        url: _flowHome
      }, () => {
    ...
    function _handleProjectResponse(resp, tabId = null, tabTitle = null) {
      ...
      if (resp?.projectId) {
        ...
        if (tabId) {
          ...
          console.log('[SlncTrZ] Target Flow tab set:', tabId, 'Project:', resp.projectId, 'Name:', projectName);
  ...
  async function _showProjectSelectOverlay() {
    ...
    try {
      if (chrome.tabs?.query) {
        ...
        if (tabs.length === 0) {
          console.log('[SlncTrZ] _showProjectSelectOverlay aborted: no Flow tab open');
          return;
        }
        ...
        for (const tab of tabs) {
          ...
          if (projectId) {
            if (window._currentProjectId !== projectId) {
              console.log('[SlncTrZ] _showProjectSelectOverlay aborted: defensive guard found project', projectId);
              window._currentProjectId = projectId;
    ...
    const _asyncScanFlowProjects = window._asyncScanFlowProjects = async (overlayEl, force = false) => {
      ...
      if (window._modalScanInProgress) {
        console.log('[SlncTrZ] scanFlowProjects SKIP: already in progress');
      ...
      if (!force && window._lastModalScanTime && (now - window._lastModalScanTime < MODAL_SCAN_COOLDOWN_MS)) {
        ...
        console.log('[SlncTrZ] scanFlowProjects SKIP: cooldown', remainSec + 's remaining (use resync button to force)');
        _setSyncingUI(overlayEl, false);
        return;
      }
      window._modalScanInProgress = true;
      window._lastModalScanTime = now;
      if (force) console.log('[SlncTrZ] scanFlowProjects FORCE: resync button clicked');
      try {
        ...
        console.warn('[SlncTrZ] async scanFlowProjects failed:', err.message);
        _setSyncingUI(overlayEl, false);
      } finally {
    ...
    listContainer?.addEventListener('click', async (e) => {
      ...
      // Group C/Initiative 7: base URL từ ProviderConfigManager
      const _flowBase2 = window.ProviderConfigManager?.getBaseUrlSync('flow') || 'https://labs.google/fx/tools/flow';
      chrome.runtime.sendMessage({
        action: 'navigateToProject',
  ...
  async function _scanAndUpdateFlowProjects(scanTabId, force = false) {
    ...
    try {
      ...
      if (changed) {
        await chrome.storage.local.set({ af_projects: freshProjects });
        console.log('[SlncTrZ] Auto-scan Flow projects: updated storage with', scanResult.projects.length, 'projects');
        _updateProjectIndicator();
      }
      return true;
    } catch (e) {
      console.warn('[SlncTrZ] Auto-scan Flow projects failed:', e?.message);
      return false;
    }
  }
  ...
  setInterval(async () => {
    ...
    try {
      ...
      if (projectFound && overlayVisible) {
        console.log('[SlncTrZ] Periodic re-sync: hide stale overlay, project=', projectFound.id);
        ...
        console.log('[SlncTrZ] Periodic re-sync: no Flow tab, clear stale project state');
        window._currentProjectId = null;
        window._currentProjectName = null;
        _updateProjectIndicator();
      } else if (projectFound && window._currentProjectId !== projectFound.id) {
        console.log('[SlncTrZ] Periodic re-sync: state out of sync, update to', projectFound.id);
        window._currentProjectId = projectFound.id;
        window._targetFlowTabId = projectFound.tab.id;
        window.MessageBridge?.setTargetTabId?.(projectFound.tab.id);
        _updateProjectIndicator();
  ...
  async function init() {
    ...
    // Bug 42 fix (2026-05-13): PCM có 2 endpoints riêng — dom-selectors (fetch)
    // và api-configs (_fetchApiConfigs). Warm cả 2 để getRatiosSync /
    // getDownloadResolutionsSync trả fresh data ngay từ first render.
    try { window.ProviderConfigManager?._fetchApiConfigs?.().catch(() => {}); } catch (e) { /* ignore */ }

    const overlay = document.getElementById('slnctrz-loading-overlay');
    const loadingText = document.getElementById('slnctrz-loading-text');
...
function hideLoadingOverlay() {
    ...
    const offlineOverlay = document.getElementById('slnctrz-offline-overlay');
    const offlineRetryBtn = document.getElementById('slnctrz-offline-retry-btn');
    ...
    // Check connection with actual network request (more reliable than navigator.onLine)
    ...
    if (offlineRetryBtn) {
      offlineRetryBtn.addEventListener('click', async () => {
        ...
        offlineRetryBtn.innerHTML = `
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="slnctrz-spin">
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      ...
      if (msg.action === 'flowHomepageProjectsChanged') {
        const overlay = document.querySelector('.project-select-overlay');
        if (overlay) {
          console.log('[SlncTrZ] Flow homepage projects changed (count=' + msg.count + ') → re-scan modal');
        ...
        try {
          ...
          if (tab?.url?.startsWith('https://labs.google/fx/')) {
            ...
            if (isProjectTab) {
              ...
              console.log('[SlncTrZ] Tab activated, target Flow tab updated:', activeInfo.tabId);
              // Apply Flow page settings sớm khi switch tab — 1-time per tab session
              window.MessageBridge?.sendToContentScript?.('applyFlowPageSettings', {}).catch(() => {});
            } else {
              console.log('[SlncTrZ] Tab activated is Flow homepage, keeping existing target tab');
            }
    ...
    try {
          ...
          if (ok) {
            try {
              ...
              const response = await chrome.runtime.sendMessage({
                ...
                urlPattern: window.ProviderConfigManager?.getTabQuery('chatgpt') || '*://chatgpt.com/*',
                createUrl: window.ProviderConfigManager?.getCreateUrl('chatgpt') || 'https://chatgpt.com/',
                activate: true
              });
              if (response?.tabId) {
              ...
              console.error('[SlncTrZ] Không mở được tab ChatGPT:', err);
            }
          }
        });
        ...
        window.eventBus.on('grok:login_required', async () => {
          ...
          if (ok) {
            try {
              ...
              const response = await chrome.runtime.sendMessage({
                action: 'openOrActivateTab',
                urlPattern: window.ProviderConfigManager?.getTabQuery('grok') || '*://grok.com/*',
                createUrl: window.ProviderConfigManager?.getProviderUrl('grok', 'imagine') || 'https://grok.com/imagine',
                activate: true
              });
              if (response?.tabId) {
                // Start polling login status sau khi mở/activate tab
              ...
              console.error('[SlncTrZ] Không mở được tab Grok:', err);
            }
          }
        });
      }
      ...
      if (window.authManager) {
        ...
        // setupNotificationBell — removed (NotificationBell class deleted)
      ...
      async function _processPendingAddToGenTab() {
        try {
          ...
          console.log(`[SlncTrZ] Processing ${pending.length} pending addImageToGenTab`);
          let addedCount = 0;
          ...
          console.warn('[SlncTrZ] Error processing pending addToGenTab:', e);
        }
      }
          ...
          if (window.ImmediateUploader) {
            window.ImmediateUploader.uploadAllPending().then((result) => {
              if (result.uploaded > 0) {
                console.log(`[SlncTrZ] Auto-uploaded ${result.uploaded} pending file(s) khi Flow tab active`);
              }
            }).catch(() => {});
          }
      ...
      try {
        ...
        // Bug 42 fix: PCM api-configs endpoint riêng — chứa ratios + download_resolutions
      ...
      document.querySelectorAll('.slnctrz-tab').forEach(btn => {
      ...
      if (activeTabId && document.getElementById(activeTabId)) {
        const tabBtns = document.querySelectorAll('.slnctrz-tab');
      ...
      console.error('[SlncTrZ] Init failed:', error);

      // Show error state with retry button (I-2)
      setLoadingText(window.I18n?.t('app.loadError') || 'Lỗi tải dữ liệu. Thử lại...');
      const loadingContent = overlay?.querySelector('.slnctrz-loading-content');
      if (loadingContent && !loadingContent.querySelector('.slnctrz-loading-retry')) {
        const retryBtn = document.createElement('button');
        retryBtn.className = 'btn btn-primary slnctrz-loading-retry';
        retryBtn.textContent = window.I18n?.t('common.retry') || 'Thử lại';
        retryBtn.style.marginTop = '12px';
        retryBtn.addEventListener('click', () => {
          retryBtn.remove();
          overlay?.classList.remove('slnctrz-loading-overlay--hidden');
          init();
        });
  ...
  function setupTabSwitching() {
    const tabButtons = document.querySelectorAll('.slnctrz-tab');
    const tabPanes = document.querySelectorAll('.tab-pane');
    ...
    tabButtons.forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (tabId === 'tab-gen' && window.featureGate) {
          ...
          window.featureGate.refresh?.().then(() => {
            ...
            if (!freshAllowed && syncAllowed) {
              ...
              initializeTab(tabId).catch((err) => console.warn('[SlncTrZ] initializeTab error:', err));
            }
          }).catch((err) => console.warn('[SlncTrZ] featureGate.refresh error:', err));
          ...
          }).catch((err) => console.warn('[SlncTrZ] isModuleEnabledAsync error:', err));
        }
        ...
        if (activePane) {
          ...
          tabSpinner.className = 'slnctrz-tab-loading';
          tabSpinner.innerHTML = '<div class="slnctrz-loading-spinner"></div>';
    ...
    switch (tabId) {
      ...
      case 'tab-workflow':
        ...
        if (window.WorkflowTab) {
          ...
          if (container && !container.__slnctrzTab) {
            container.__slnctrzTab = new WorkflowTab(container);
            await container.__slnctrzTab.init();
          } else if (container?.__slnctrzTab?.workflowList) {
            container.__slnctrzTab.workflowList.loadWorkflows();
            ...
            container.__slnctrzTab.workflowList.loadSharedWorkflows();
          }
          // Check for local data needing migration
          if (window.storageManager?.mode === 'api') {
  ...
  function _setupLogsSubtabs() {
    const subtabs = document.querySelectorAll('.slnctrz-logs-subtab');
    ...
    subtabs.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (target === 'logs-main') {
          mainContent.classList.remove('slnctrz-logs-subtab-content--hidden');
          queueContent.classList.add('slnctrz-logs-subtab-content--hidden');
          if (window.QueueMonitor) {
            QueueMonitor.getInstance()?.setVisible(false);
          }
        } else if (target === 'logs-queue') {
          mainContent.classList.add('slnctrz-logs-subtab-content--hidden');
          queueContent.classList.remove('slnctrz-logs-subtab-content--hidden');
          if (window.QueueMonitor) {
  ...
  function refreshSubtabOverlays() {
    ...
    if (workflowTab?.__slnctrzTab?._applySubtabFeatureGate) {
      const currentSubtab = workflowTab.__slnctrzTab._currentSubtab || 'templates';
      workflowTab.__slnctrzTab._applySubtabFeatureGate(currentSubtab);
    }
  }
  ...
  function setupTaskExecutor() {
    ...
    window.eventBus.on('task:run', async (data) => {
      ...
      if (window.ExecutionGate) {
        try {
          const gate = await ExecutionGate.request('task_run', taskPromptCount, { owner: 'task', label: task.task_name || task.task_id });
          if (!gate.allowed) {
      ...
      try {
        await executeSingleTask(task, { _executionToken: isPipeline ? _taskExecutionToken : null });
        ...
        if (!isPipeline && window.ExecutionGate && _taskExecutionToken) {
          ExecutionGate.complete(_taskExecutionToken, 'success');
        }
        // Track usage: tasks_run_max (1 per task) + gen_run_max (Flow) + prompt_submit_max
        if (window.featureGate && taskPromptCount > 0) {
          window.featureGate.recordTaskRun(); // 1 task run
          window.featureGate.recordGenRun();
          window.featureGate.recordPromptSubmit(taskPromptCount, 'task');
        }
    ...
    window.eventBus.on('tasks:run_batch', async (data) => {
      ...
      if (window.ExecutionGate) {
        try {
          ...
          const gate = await ExecutionGate.request('task_run', totalPrompts, { owner: 'task', label: 'Task batch' });
          if (!gate.allowed) {
            ExecutionGate.showDeniedDialog(gate, 'Task');
            if (window.ExecutionLock) ExecutionLock.release('task');
      ...
      if (window.PromptQueue && PromptQueue.isEnabled() && !_hasNonFlowProvider) {
        ...
        const settingsPerTask = tasks.map(t => {
          ...
          const _defVid = window.ModelRegistry?.getDefault('flow', 'video') || 'Veo 3.1 - Fast';
          const _defImg = window.ModelRegistry?.getDefault('flow', 'image') || 'Nano Banana 2';
          return {
            genType: gt,
        ...
        window._currentTaskExecutionToken = null;
        // Track usage: pipeline batch completed - calculate actual completed prompts
        const pipelineTotalPrompts = tasks.reduce((sum, t) => {
          return sum + ((t.multi_prompt && t.prompts?.length > 1) ? t.prompts.length : 1);
        }, 0);
        if (window.featureGate && result.completed > 0) {
          ...
          window.featureGate.recordGenRun();
          window.featureGate.recordPromptSubmit(pipelineTotalPrompts, 'task_pipeline');
        }
        // Emit tracker completed
        if (window.eventBus) {
          window.eventBus.emit('execution:tracker_update', {
      ...
      if (window.ExecutionGate && _batchExecutionToken) {
        ExecutionGate.complete(_batchExecutionToken, window._taskBatchStopped ? 'partial' : 'success');
        window._currentTaskExecutionToken = null;
      }
      ...
      if (typeof notifyCompletion === 'function') {
        notifyCompletion('SlncTrZ VM', window.I18n?.t('app.batchComplete', { count: tasks.length }) || `Đã hoàn tất batch ${tasks.length} tasks!`);
      }
      if (window.eventBus) {
        window.eventBus.emit('tasks:batch_complete');
      }
  ...
  async function _resolveChatGPTTaskRefs(task, adapter) {
    ...
    const idsCapped = ids.slice(0, maxRef);
    if (ids.length > maxRef) {
  ...
  async function _executeTaskViaChatGPT(task, ctx = {}) {
    ...
    try {
      ...
      const refImagesResolved = await _resolveChatGPTTaskRefs(task, adapter);
      ...
      // Load af_settings để lấy downloadFolder + downloadTemplate (giống Task Grok pattern).
      // Trước fix: hardcode 'flow-output' + template '[Date]_[Project]_[Prompt]_[Index]' →
      // user setting downloadFolder bị ignore.
      const _cgTaskSettings = await new Promise((resolve) => {
        chrome.storage.local.get(['af_settings'], (res) => resolve(res.af_settings || {}));
      });
      const _cgTaskDownloadFolder = _cgTaskSettings.downloadFolder || 'flow-output';
      const _cgTaskDownloadTemplate = _cgTaskSettings.downloadTemplate || '[Date]_[Project]_[Prompt]_[Index]';
      ...
      try {
        ...
        if (freshTask) {
          if (collectedIds.length > 0) {
            // Merge results for multi-prompt tasks (accumulate results from each prompt)
            const existingIds = (freshTask.result_file_ids || '').split(',').map(s => s.trim()).filter(Boolean);
            const mergedIds = [...existingIds, ...collectedIds];
  ...
  async function _executeTaskViaGrok(task, ctx = {}) {
    ...
    try {
      ...
      const refImagesResolved = await _resolveGrokTaskRefs(task, adapter);

      // Auto-download settings cho task path
      const taskAutoDownload = !!(task.auto_download) && !!(window.featureGate?.canUse?.('auto_download'));

      // Load af_settings để lấy downloadFolder + downloadTemplate (giống GenTab pattern).
      // Trước fix: hardcode 'flow-output' + template '[Date]_[Project]_[Prompt]_[Index]' →
      // user setting downloadFolder bị ignore.
      const _grokTaskSettings = await new Promise((resolve) => {
        chrome.storage.local.get(['af_settings'], (res) => resolve(res.af_settings || {}));
      });
      const _grokTaskDownloadFolder = _grokTaskSettings.downloadFolder || 'flow-output';
      const _grokTaskDownloadTemplate = _grokTaskSettings.downloadTemplate || '[Date]_[Project]_[Prompt]_[Index]';
      ...
      const _grokTabId = window.GrokSession?.getTabInfo?.()?.tabId;
    ...
    try {
      ...
      if (window.MessageBridge) {
        ...
        runResult = await window.MessageBridge.runAutoPrompt({
          prompts,
          delayBetweenMs: (window.storageSettings?.getSettings()?.delayBetweenPrompts || 5) * 1000,
          inputTimeoutMs: window.storageSettings?.getSettings()?.inputTimeout || 1200,  // [Fix] Truyền inputTimeout cho content.js
          fileIds,
  ...
  function setupAuthUI() {
    ...
    chrome.runtime.onMessage.addListener((msg) => {
      ...
      (async () => {
        ...
        if (window.authManager?.token === msg.token) {
          console.log('[SlncTrZ] OAuth: Skip message handler, storage handler đã xử lý');
          return;
        }
        ...
        if (window.featureGate) {
          try {
            ...
            console.log('[SlncTrZ] OAuth: Entitlements refreshed');
          } catch (e) {
            console.warn('[SlncTrZ] OAuth: Không thể refresh entitlements', e);
          }
        }
    ...
    if (multiTabCloseBtn) {
      multiTabCloseBtn.addEventListener('click', async () => {
        try {
          ...
          console.warn('[SlncTrZ] Failed to close other tabs:', e.message);
        }
      });
    }
    ...
    if (window.eventBus) {
      ...
      window.eventBus.on('auth:login', async () => {
        ...
        if (isAllowed) {
          ...
          if (tasksPane.__multiTaskTab?.taskList) {
            ...
            initializeTab('tab-tasks').catch(e => console.warn('[SlncTrZ] tab-tasks init error:', e));
          }
        }
      });
      ...
      window.eventBus.on('auth:logout', () => {
        console.log('[SlncTrZ] Auth logout → reload UI lists (data cleared)');
      ...
      window.eventBus.on('api:auth_error', () => {
        console.warn('[SlncTrZ] API auth error → switching to local mode');
        if (window.storageManager?.mode === 'api') {
          window.storageManager.switchToLocal();
        }
      ...
      window.eventBus.on('sse:system_settings_changed', (data) => {
        ...
        // Refresh ChatGPT config khi admin update (section = 'chatgpt' hoặc không có section)
        if (!data?.section || data.section === 'chatgpt') {
          console.log('[SSE] Refreshing ChatGPT config...');
          window.ChatGPTConfig?.refresh?.();
        }
        // Refresh Grok config khi admin update (section = 'grok' hoặc không có section)
        if (!data?.section || data.section === 'grok') {
          console.log('[SSE] Refreshing Grok config...');
          window.GrokConfig?.refresh?.();
        }
      });

      // Phase H: refresh ChatGPT/Grok error_patterns khi admin update qua
      // /admin/providers → API Configs (event provider:api_config_updated với key='error_patterns').
      window.eventBus.on('provider:api_config_updated', ({ provider, key }) => {
        if (key !== 'error_patterns') return;
        if (provider === 'chatgpt') {
          console.log('[SSE] Provider API config error_patterns updated — refresh ChatGPTConfig');
          window.ChatGPTConfig?.refresh?.();
        } else if (provider === 'grok') {
          console.log('[SSE] Provider API config error_patterns updated — refresh GrokConfig');
          window.GrokConfig?.refresh?.();
        }
      });
      ...
      window.eventBus.on('album:use', async (data) => {
        ...
        if (window.GenTab && window.GenTab.fileIdsInput && window.AlbumList) {
          ...
          if (newIds.length > 0) {
            ...
            // Switch to gen tab
            const genTabBtn = document.querySelector('.slnctrz-tab[data-tab="tab-gen"]');
      ...
      window.eventBus.on('capture:start', async (data) => {
        if (!window.ScreenCapture) {
          console.warn('[SlncTrZ] ScreenCapture not available');
          return;
        }
        ...
        if (data?.targetAlbumId && window.ImageStore) {
          try {
            ...
            console.log('[SlncTrZ] Capture added to album:', data.targetAlbumId);
            // Refresh album list
            window.eventBus.emit('album:refresh');
          } catch (e) {
            console.error('[SlncTrZ] Failed to add capture to album:', e);
          }
        }
      });
      ...
      window.eventBus.on('upload:completed', async (data) => {
        ...
        try {
          ...
          for (const img of matching) {
            ...
            console.log('[SlncTrZ] Album image updated after upload:', img.id, data.tile_id);
          }
          ...
          console.warn('[SlncTrZ] Failed to update album image after upload:', e);
        }
      });

      // Listen for plan changes (triggered when fetchUser detects plan_slug change)
      window.eventBus.on('plan:changed', (data) => {
        console.log(`[SlncTrZ] Plan changed event: ${data.oldPlan} → ${data.newPlan}`);
        updateAuthUI();
        refreshModuleOverlays();
    ...
    chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
      ...
      // Scheduler trigger từ background.js alarm
      if (msg.action === 'workflow:scheduler_trigger' && msg.workflowId) {
        console.log('[SlncTrZ] Scheduler: running workflow', msg.workflowId);
        if (window.workflowExecutor) {
          window.workflowExecutor.execute(msg.workflowId).catch(e => {
            console.warn('[SlncTrZ] Scheduler: execute failed', e.message);
          });
        } else {
          // Load workflow và execute
          window.storageManager?.getWorkflow(msg.workflowId).then(wf => {
            if (wf && window.WorkflowExecutor) {
              const exec = new WorkflowExecutor();
              exec.execute(wf.wf_id).catch(e => {
                console.warn('[SlncTrZ] Scheduler: execute failed', e.message);
              });
            }
          });
      ...
      // n8n WebSocket command — execute qua queue (serialize concurrent batch)
      if (msg.action === 'n8n:execute') {
        _queueN8nExecute(msg).then(results => {
          chrome.runtime.sendMessage({
            action: 'n8n:result',
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes.af_auth) {
        ...
        if (!newVal && wasLoggedIn) {
          ...
          console.log('[SlncTrZ] Auth: Phát hiện đăng xuất từ context khác');
          ...
          if (window.SseClient?.disconnect) {
            try {
              ...
              console.warn('[SlncTrZ] Auth: Lỗi disconnect SSE external logout', e.message);
            }
          }
          ...
          if (oldToken) {
            console.log('[SlncTrZ] Auth: Gọi sse/end-session với old token...');
            ...
            chrome.runtime.sendMessage({
              ...
              if (chrome.runtime.lastError) {
                console.warn('[SlncTrZ] Auth: Lỗi gọi sse/end-session:', chrome.runtime.lastError.message);
              } else if (response?.success) {
                console.log('[SlncTrZ] Auth: SSE session đã được xóa thành công');
              } else {
                // 401 là bình thường nếu token đã bị invalidate bởi auth/logout
                console.warn('[SlncTrZ] Auth: sse/end-session thất bại (có thể token đã hết hạn):', response?.error?.message);
              }
            });
          }
          ...
          // CRITICAL: Reset FeatureGate TRƯỚC khi refresh overlays và emit event
          // Để UI hiển thị đúng trial entitlements thay vì logged-in user's data
          (async () => {
            if (window.featureGate) {
              try {
                await window.featureGate.resetForLogout();
                console.log('[SlncTrZ] Auth: FeatureGate reset sau external logout');
              } catch (err) {
                console.warn('[SlncTrZ] Auth: Lỗi reset FeatureGate:', err.message);
              }
            }
            refreshModuleOverlays();
            if (window.eventBus) {
          ...
          if (window._oauthLoginProcessing) {
            console.log('[SlncTrZ] Auth: Skip storage handler, message handler đang xử lý');
            return;
          }
          // Check nếu token đã được set bởi message handler (chạy trước storage handler)
          if (window.authManager?.token === newVal.token) {
            console.log('[SlncTrZ] Auth: Skip storage handler, token đã được set');
            return;
          }
          console.log('[SlncTrZ] Auth: Phát hiện đăng nhập từ context khác');
          ...
          (async () => {
            ...
            if (window.storageManager) {
              await window.storageManager.switchToApi();
              console.log('[SlncTrZ] Storage: Switched to API mode');
            }

            if (window.featureGate) {
              try {
                await window.featureGate.resetForLogin();
                console.log('[SlncTrZ] Storage: Entitlements refreshed sau login');
              } catch (e) {
                console.warn('[SlncTrZ] Storage: Không thể refresh entitlements', e);
              }
            }
      ...
      if (changes.af_entitlements) {
        console.log('[SlncTrZ] Entitlements changed, reload memory + refresh overlays');
        const newCache = changes.af_entitlements.newValue;
        if (newCache && window.featureGate) {
    ...
    async function refreshPermissions(source = 'unknown') {
      ...
      if (window.featureGate?._isLoggingOut || window.featureGate?._refreshPending) {
        console.log(`[SlncTrZ] refreshPermissions skip (${source}) — đang logout hoặc đã có refresh`);
        return false;
      }

      console.log(`[SlncTrZ] Refreshing permissions (${source})...`);
      try {
        // Clear FeatureGate cache first to force fresh fetch
        if (window.featureGate) {
          window.featureGate.lastFetch = 0;
        }

        // Fetch user info and entitlements in parallel
        await Promise.all([
        ...
        console.log(`[SlncTrZ] Permissions refreshed: plan=${plan?.slug || 'unknown'}`);
        return true;
      } catch (err) {
        console.warn('[SlncTrZ] Refresh failed:', err.message);
        return false;
      }
    }

    // Periodic sync: refresh permissions every 2 minutes (fallback khi SSE không hoạt động)
    setInterval(() => {
      // [Fix B] Guard periodic refresh khi đang logout/refresh
    ...
    const REFRESH_COOLDOWN = 10000; // 10s cooldown
    ...
    document.addEventListener('visibilitychange', async () => {
      ...
      if (window.featureGate?._isLoggingOut || window.featureGate?._refreshPending) {
        console.log('[SlncTrZ] visibilitychange skip refresh — đang logout hoặc đã có refresh');
        return;
      }
      ...
      if (window.authManager?.isLoggedIn()) {
        ...
        if (window.featureGate) {
          console.log('[SlncTrZ] Refreshing FeatureGate trial config (focus)...');
          await window.featureGate.refresh();
          updateTrialFooterBars();
          refreshModuleOverlays();
        }
    ...
    window.addEventListener('focus', async () => {
      ...
      // Tránh race với resetForLogout/resetForLogin → 2 /entitlements concurrent.
      if (window.featureGate?._isLoggingOut || window.featureGate?._refreshPending) {
        console.log('[SlncTrZ] window-focus skip refresh — đang logout hoặc đã có refresh');
        return;
      }
      ...
      if (window.authManager?.isLoggedIn()) {
        ...
        // Not login user: refresh FeatureGate (trial config)
        if (window.featureGate) {
          console.log('[SlncTrZ] Refreshing FeatureGate trial config (window-focus)...');
          await window.featureGate.refresh();
          updateTrialFooterBars();
          refreshModuleOverlays();
        }
    ...
    setInterval(async () => {
      ...
      if (!window.authManager?.isLoggedIn() && window.featureGate && !window.SseClient?.isConnected()) {
        console.log('[SlncTrZ] Refreshing FeatureGate trial config (interval)...');
        await window.featureGate.refresh();
        updateTrialFooterBars();
        refreshModuleOverlays();
      }
  ...
  // Extension scope: 'flow' for SlncTrZ VM, 'grok' for AutoGrok
  ...
  async function fetchPlans() {
    ...
    try {
      ...
      console.warn('[SlncTrZ] Fetch plans failed:', e.message);
      return null;
    }
  }
  ...
  async function fetchActiveProviders() {
    try {
      ...
      console.warn('[SlncTrZ] Fetch active providers failed:', e.message);
      return null;
    }
  }
  ...
  function renderUpgradeModal(plans) {
    ...
    if (selectedPlan.price_monthly != null) availableCycles.push({ key: 'monthly', label: t('dialog.month', 'Tháng'), price: selectedPlan.price_monthly });
    if (selectedPlan.price_yearly != null) availableCycles.push({ key: 'yearly', label: t('dialog.year', 'Năm'), price: selectedPlan.price_yearly });
    if (selectedPlan.price_lifetime != null) availableCycles.push({ key: 'lifetime', label: t('dialog.upgradeLifetime', 'Trọn đời'), price: selectedPlan.price_lifetime });
    ...
    html += `
        <div class="upgrade-price">${Number(currentPrice).toLocaleString('vi-VN')} \u0111</div>
        ...
        <div class="vietqr-payment-body">
          ...
          <div class="vietqr-info-section">
            ...
            <div class="vietqr-info-row vietqr-amount-row">
              ...
              <span class="vietqr-info-value vietqr-amount">${(data.amount || 0).toLocaleString('vi-VN')}đ</span>
            </div>
            <div class="vietqr-info-row">
    ...
    overlay.querySelectorAll('.vietqr-copyable').forEach(el => {
      el.addEventListener('click', async () => {
        ...
        try {
          ...
          console.error('[SlncTrZ] Copy failed:', e);
        }
      });
    });
          ...
          if (status === 'paid') {
            ...
            if (window.featureGate?.refreshAsync) {
              try {
                ...
                console.error('[SlncTrZ] Failed to refresh entitlements after payment:', e);
              }
            }
            return;
          }
        } catch (err) {
          console.error('[SlncTrZ] Payment status check failed:', err.message);
        }
  ...
  function showPlanActivatedOverlay(data) {
    ...
    console.log('[SlncTrZ] Plan activated overlay shown for order:', data.order_id);
  }
  ...
  function setupUpgradeUI() {
    ...
    const upgradeErrorWrapper = document.getElementById('upgradeErrorWrapper');
    ...
    async function openUpgradeModal() {
      ...
      if (upgradeErrorWrapper) upgradeErrorWrapper.classList.add('hidden');
      if (upgradeLoginBtn) upgradeLoginBtn.classList.add('hidden');
    ...
    function _applyActiveProviders(providers) {
      ...
      const hasVietQR = providers.includes('vietqr');
      const hasStripe = providers.includes('stripe');
      const hasPayPal = providers.includes('paypal');
      const hasPolar = providers.includes('polar');
      const activeCount = (hasVietQR ? 1 : 0) + (hasStripe ? 1 : 0) + (hasPayPal ? 1 : 0) + (hasPolar ? 1 : 0);
    ...
    async function renderPremiumBenefits() {
      ...
      try {
        ...
        console.error('[SlncTrZ] Failed to load premium benefits:', e);
    ...
    async function handlePayment(provider) {
      const payBtnMap = { vietqr: 'payVietQR', stripe: 'payStripe', paypal: 'payPayPal', polar: 'payPolar' };
      ...
      try {
        ...
        if (upgradeError) {
          upgradeError.textContent = e.message || window.I18n?.t('upgrade.checkoutFailed') || 'Thanh toán thất bại, vui lòng thử lại';
        }
        if (upgradeErrorWrapper) {
          upgradeErrorWrapper.classList.remove('hidden');
        }
        if (upgradeLoginBtn) {
          upgradeLoginBtn.classList.toggle('hidden', !isAuthError);
        }
        if (typeof sendLog === 'function') {
    ...
    // Login button in upgrade modal error
    if (upgradeLoginBtn) {
  ...
  function setupExtensionLink() {
    ...
    // Check and show extension link from SystemConfig
    const DEFAULT_EXTENSION_URL = 'https://chromewebstore.google.com/detail/autoflow-pro-flow-automat/iicjfgdnngmpfocfanpiammedafmomin';
    const updateExtensionLink = async () => {
      try {
        const settings = await window.SystemConfig?.fetch();
        const extensionUrl = settings?.extension_url || DEFAULT_EXTENSION_URL;
        if (extensionUrl) {
        ...
        console.warn('[ExtensionLink] Failed to fetch settings:', err);
        // Fallback to default URL on error
        extensionLinkBtn.classList.remove('hidden');
        extensionLinkBtn.dataset.url = DEFAULT_EXTENSION_URL;
      }
    };
  ...
  function setupOnboarding() {
    chrome.storage.local.get('af_onboarding_done', (result) => {
      ...
      const steps = [
        {
          target: null,
          title: window.I18n?.t('onboarding.welcomeTitle') || 'Chào mừng đến SlncTrZ VM!',
      ...
      function finishOnboarding() {
        if (highlightedEl) highlightedEl.classList.remove('slnctrz-onboarding-highlight');
        overlay.classList.add('hidden');
      ...
      function showStep(index) {
        ...
        if (highlightedEl) {
          highlightedEl.classList.remove('slnctrz-onboarding-highlight');
          highlightedEl = null;
        }
        ...
        if (step.target) {
          ...
          if (targetEl) {
            ...
            targetEl.classList.add('slnctrz-onboarding-highlight');
  ...
  // ─── n8n Command Execution ─────────────────────────────
  // Queue serialization cho n8n commands (xử lý concurrent batch từ ws-bridge)
  const _n8nQueue = [];
  let _n8nProcessing = false;

  async function _processN8nQueue() {
    if (_n8nProcessing || _n8nQueue.length === 0) return;
    _n8nProcessing = true;
    const item = _n8nQueue.shift();
    log('[Queue] processing id=' + (item.msg.commandId || '?') + ' queue=' + (_n8nQueue.length + 1) + ' remaining=' + _n8nQueue.length);
    try {
      const result = await _doHandleN8nExecute(item.msg);
      item.resolve(result);
    } catch (err) {
      log('[Queue] error id=' + (item.msg.commandId || '?') + ' err=' + err.message);
      item.reject(err);
    } finally {
      log('[Queue] done id=' + (item.msg.commandId || '?') + ' processing=' + _n8nProcessing + ' remaining=' + _n8nQueue.length);
      _n8nProcessing = false;
      _processN8nQueue();
    }
  }

  function _queueN8nExecute(msg) {
    log('[Queue] enqueue id=' + (msg.commandId || '?') + ' queueSize=' + (_n8nQueue.length + 1));
    return new Promise((resolve, reject) => {
      _n8nQueue.push({ msg, resolve, reject });
      _processN8nQueue();
    });
  }

  async function _doHandleN8nExecute(cmd) {
    const { provider, prompts, images, config } = cmd;
    log('[n8n] Execute:', provider, prompts?.length, 'prompts');

    if (!provider || !prompts?.length) throw new Error('Missing provider or prompts');
    if (!window.MessageBridge) throw new Error('MessageBridge not available');

    if (provider === 'flow') {
      return await _handleFlowN8nExecute(prompts, images, config);
    }

    throw new Error('Unsupported provider: ' + provider);
  }

  async function _handleFlowN8nExecute(prompts, images, config) {
    const cfg = config || {};
    const mode = cfg.mode || 'image';
    const genType = (mode === 'video' || cfg.videoMode === 'video') ? 'Video' : 'Image';
    const modelName = cfg.model || null;
    const aspectRatio = cfg.ratio || cfg.aspectRatio || '16:9';

    log('[n8n] Flow execute: genType=' + genType + ', model=' + modelName + ', prompts=' + prompts.length);

    let fileIds = [];
    let fileNameMap = {};

    if (images && images.length > 0) {
      log('[n8n] Uploading ' + images.length + ' reference images to Flow...');
      const filesData = images.map((b64, i) => {
        let base64 = b64;
        ...
        return { base64, name: 'n8n_ref_' + (i + 1) + '.' + ext, type: mimeType };
      });

      try {
        const uploadResult = await window.MessageBridge.uploadFilesToFlow(filesData);
        if (uploadResult?.orderedTileIds?.length > 0) {
          fileIds = uploadResult.orderedTileIds;
          log('[n8n] Uploaded ' + fileIds.length + ' files, tileIds: ' + fileIds.join(', '));
        } else if (uploadResult?.tileIds?.length > 0) {
          fileIds = uploadResult.tileIds;
          log('[n8n] Uploaded ' + fileIds.length + ' files (unordered), tileIds: ' + fileIds.join(', '));
        } else {
          log('[n8n] Upload returned no tileIds, continuing without reference images');
        }
      } catch (uploadErr) {
        log('[n8n] Image upload failed: ' + uploadErr.message + ', continuing without references');
      }
    }

    const inputTimeoutMs = window.storageSettings?.getSettings()?.inputTimeout || 1200;
    const delayBetweenMs = (window.storageSettings?.getSettings()?.delayBetweenPrompts || 5) * 1000;
    ...
    const payload = {
      ...
      noTileWait: false,
      autoDownload: false,
      downloadResolution: cfg.resolution || cfg.size || '1k',
      videoDownloadResolution: cfg.resolution || '720p',
      refImageMode: fileIds.length > 0 ? 'all' : 'none',
    };

    log('[n8n] Calling runAutoPrompt with ' + prompts.length + ' prompts...');
    let runResult;
    for (let retry = 0; retry < 30; retry++) {
      runResult = await window.MessageBridge.runAutoPrompt(payload).catch(err => ({ error: err.message }));
      if (runResult?.blocked) {
        log('[n8n] runAutoPrompt blocked (attempt ' + (retry + 1) + '/30), waiting 2s...');
        await new Promise(r => setTimeout(r, 2000));
        continue;
      }
      break;
    ...
    if (runResult?.error) {
      log('[n8n] runAutoPrompt error: ' + runResult.error);
      return [runResult];
    }

    if (runResult?.blocked) {
      log('[n8n] runAutoPrompt blocked after 30 retries: ' + runResult.reason);
      return [{ error: 'Execution blocked after 30 retries: ' + runResult.reason }];
    }

    const resultTileIds = runResult?.resultTileIds || [];
    log('[n8n] runAutoPrompt completed, resultTileIds: ' + resultTileIds.join(', '));

    let results = [];

    if (resultTileIds.length > 0) {
      for (const tileId of resultTileIds) {
        try {
          const mediaResult = await window.MessageBridge.sendToContentScript('waitForTileMediaUrl', {
            tileId,
            isVideo: genType === 'Video'
          });
          if (mediaResult?.url) {
            log('[n8n] Got URL for ' + tileId + ': ' + mediaResult.url.substring(0, 80) + '...');
            // Follow redirect để lấy CDN URL public (có cookies Google)
            const resolved = await window.MessageBridge.sendToContentScript('resolveMediaUrl', { url: mediaResult.url });
            if (resolved?.url && resolved.url !== mediaResult.url) {
              log('[n8n] Resolved to CDN URL: ' + resolved.url.substring(0, 80) + '...');
              results.push(resolved.url);
            } else if (resolved?.url) {
              log('[n8n] No redirect, using original URL');
              results.push(resolved.url);
            } else {
              log('[n8n] resolveMediaUrl failed: ' + (resolved?.error || 'unknown') + ', using original URL');
              results.push(mediaResult.url);
            }
          } else {
            log('[n8n] No URL for ' + tileId + ': ' + (mediaResult?.error || 'unknown'));
            results.push(null);
          }
        } catch (err) {
          log('[n8n] waitForTileMediaUrl failed for ' + tileId + ': ' + err.message);
          results.push(null);
        }
      }
    }
    ...
    const resultSummary = results.map((r, i) => r ? (resultTileIds[i] || i) + ':URL' : (resultTileIds[i] || i) + ':null').join(', ');
    log('[n8n] Final results: ' + results.length + ' items [' + resultSummary + ']');
    return results;
  }
  ...
  function setupLanguageModal() {
    ...
    languageOptions.forEach(option => {
      option.addEventListener('click', () => {
        ...
        console.log('[SlncTrZ] Language changed to:', lang);
  ...
  function setupReferralUI() {
    ...
    async function loadReferralData() {
      ...
      try {
        ...
        console.warn('[SlncTrZ] Failed to load referral data:', e);
      }
    }
    ...
    if (copyBtn) {
      copyBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          console.warn('[SlncTrZ] Clipboard write failed:', e);
        }
      });
    }
    ...
    if (shareBtn) {
      shareBtn.addEventListener('click', async () => {
        const code = codeValue?.textContent;
        if (!code || code === '---') return;
        const shareText = window.I18n?.t('app.shareReferralText', { code }) || `Dùng thử SlncTrZ VM! Dùng mã giới thiệu của mình: ${code}`;
        if (navigator.share) {
          try {
            await navigator.share({ title: 'SlncTrZ VM', text: shareText });
          } catch (e) {
  ...
  function setupConversionTriggers() {
    ...
    window.eventBus.on('feature:locked', (data) => {
      ...
      if (data?.container && data.container instanceof HTMLElement) {
        const existing = data.container.querySelector('.slnctrz-conversion-inline-nudge');
        if (existing) return;

        const nudge = document.createElement('div');
        nudge.className = 'slnctrz-conversion-inline-nudge';
        nudge.innerHTML = `
          <div class="slnctrz-conversion-inline-nudge__icon">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
              <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
            </svg>
          </div>
          <span class="slnctrz-conversion-inline-nudge__text">${window.I18n?.t('upgrade.unlockFeature') || 'Nâng cấp để mở khóa tính năng này'}</span>
          <button class="btn btn-primary btn-sm slnctrz-conversion-inline-nudge__btn">${window.I18n?.t('common.upgrade') || 'Nâng cấp'}</button>
        `;
        nudge.querySelector('.slnctrz-conversion-inline-nudge__btn').addEventListener('click', showUpgradeOverlay);
  ...
  // Do NOT auto-init here - content.js will call SlncTrZApp.init()
