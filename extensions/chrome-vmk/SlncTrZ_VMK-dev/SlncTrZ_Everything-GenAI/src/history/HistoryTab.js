...
(function() {
  ...
  class HistoryTab {
    ...
    async init() {
      ...
      console.log('[SlncTrZ] HistoryTab đã khởi tạo');
    }
        ...
        <div class="history-tab">
          <div class="section">
            <div class="section-header">
              ...
              <button class="btn btn-secondary btn-sm slnctrz-refresh-btn" id="historyRefreshBtn" data-tooltip="${t('common.reload')}" data-tooltip-pos="bottom">
          ...
          <div class="slnctrz-load-more-row hidden" id="historyLoadMore"></div>
    ...
    _renderItem(record) {
      ...
      return `
        <div class="history-item" data-id="${id}" data-media-type="${this._escapeAttr(mediaType)}" data-provider="${this._escapeAttr(provider)}" data-ratio="${this._escapeAttr(record.ratio || '')}">
          ${thumbHtml}
    ...
    async loadHistory(append = false) {
      ...
      try {
        ...
        console.error('[SlncTrZ] Tải lịch sử thất bại:', e);
        if (!append) {
    ...
    _rerunPrompt(record) {
      ...
      console.log('[SlncTrZ] Đã điền prompt từ lịch sử (provider:', record.provider || 'flow', ')');
    }
        ...
        const h = async (e) => {
          ...
          if (action === 'favorite') {
            ...
            if (fullRecord) {
              ...
              // Fallback: extract từ data-full-prompt attribute (set ở _renderItem)
              ...
              this._rerunPrompt({ prompt, media_type: mediaType, ratio, provider });
            }
          }
        };
    ...
    _renderLoadMore(loadMoreEl, meta) {
      ...
      loadMoreEl.innerHTML = `
        <button class="slnctrz-load-more-btn" id="loadMoreHistoryBtn">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
          ${loadMoreLabel}
          <span class="slnctrz-load-more-count">${renderedCount} / ${total}</span>
        </button>
      `;
