...
(function() {
  ...
  class ImagePickerModal {
     ...
     * @returns {number|null} max count hoặc null nếu không xác định / unlimited
     */
    static resolveMaxSelections({ provider, mode = 'image', isFrames = false } = {}) {
    ...
    open(options = {}) {
      ...
      // null = unlimited. Có thể truyền trực tiếp hoặc resolve qua ImagePickerModal.resolveMaxSelections().
      this.maxSelections = (typeof options.maxSelections === 'number' && options.maxSelections > 0)
          ...
          <div class="imgpicker-content">
            ...
            <div class="imgpicker-pane" id="paneUpload">
              <div class="imgpicker-drop-zone" id="imgpickerDropZone">
                ...
                <input type="file" id="imgpickerFileInput" multiple accept="image/*" style="display:none;" />
              </div>
    ...
    _scanPageImages() {
      ...
      // Scan tiles trên Google Flow page
      ...
      // Fallback: scan img tags with media URL pattern
      if (images.length === 0) {
    ...
    _showMaxReachedWarning() {
      ...
      if (window.SlncTrZNotify?.warning) {
        window.SlncTrZNotify.warning(msg);
