...
if (self.__slnctrzContentLoaded__) {
  // Silent skip - đã loaded trước đó
} else {
self.__slnctrzContentLoaded__ = true;
// ═══════════════════════════════════════════════════════════════════════════════
...
var FloatingTracker = {
  ...
  _create() {
    ...
    var existing = document.getElementById('slnctrz-flow-tracker');
    if (existing) existing.remove();
    this._el = null;

    var el = document.createElement('div');
    el.id = 'slnctrz-flow-tracker';
    ...
    header.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>' +
      ...
      '<span class="slnctrz-ft-counter" style="font-size:11px;opacity:0.7;font-variant-numeric:tabular-nums;"></span>' +
      '<span class="slnctrz-ft-elapsed" style="font-size:10px;opacity:0.5;font-variant-numeric:tabular-nums;"></span>' +
      '<button class="slnctrz-ft-stop-all" title="' + _getTrackerT('stopAll') + '" style="width:22px;height:22px;background:rgba(239,68,68,0.2);border:none;border-radius:5px;color:#ef4444;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' +
        '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="6" width="12" height="12" rx="1"/></svg>' +
      '</button>';
    el.appendChild(header);

    var self = this;
    header.querySelector('.slnctrz-ft-stop-all').addEventListener('click', function() {
    ...
    progress.innerHTML = '<div class="slnctrz-ft-progress-fill" style="height:100%;width:100%;background:linear-gradient(90deg,#3b82f6,#60a5fa,#a78bfa);transform:scaleX(0);transform-origin:left;transition:transform 0.3s ease-out;will-change:transform;"></div>';
    el.appendChild(progress);

    // CSS animations (inject once)
    if (!document.getElementById('slnctrz-ft-animations')) {
      var style = document.createElement('style');
      style.id = 'slnctrz-ft-animations';
      style.textContent = '@keyframes slnctrz-pulse{0%,100%{opacity:1}50%{opacity:0.6}}' +
        '@keyframes slnctrz-progress-glow{0%,100%{opacity:0.5}50%{opacity:1}}' +
        '.slnctrz-ft-progress-fill.active{animation:slnctrz-progress-glow 1.5s ease-in-out infinite}' +
        '.slnctrz-ft-dot-pulse{animation:slnctrz-pulse 1.5s ease-in-out infinite}';
      document.head.appendChild(style);
    }

    // Pipeline status row
    var pipelineRow = document.createElement('div');
    pipelineRow.className = 'slnctrz-ft-pipeline';
    ...
    jobsWrap.className = 'slnctrz-ft-jobs';
  ...
  /**
   * Thiết lập event delegation trên container .slnctrz-ft-jobs.
  ...
  update(data) {
    ...
    if (!isRunning || total === 0) {
      ...
      if (completed > 0) {
        ...
        el.querySelector('.slnctrz-ft-counter').textContent = completed + '/' + total + ' ' + _getTrackerT('done');
        var progressFill = el.querySelector('.slnctrz-ft-progress-fill');
        progressFill.style.transform = 'scaleX(1)';
        progressFill.classList.remove('active');
        el.querySelector('.slnctrz-ft-elapsed').textContent = '';
        // Ẩn stop button và pipeline row khi đã xong
        var stopBtn = el.querySelector('.slnctrz-ft-stop-all');
        if (stopBtn) stopBtn.style.display = 'none';
        var pRow = el.querySelector('.slnctrz-ft-pipeline');
        if (pRow) pRow.style.display = 'none';
        el.querySelector('.slnctrz-ft-jobs').innerHTML = '';
    ...
    el.querySelector('.slnctrz-ft-counter').textContent = completed + '/' + total;
    var progressFill = el.querySelector('.slnctrz-ft-progress-fill');
    progressFill.style.transform = 'scaleX(' + (pct / 100) + ')';
    progressFill.classList.add('active'); // Enable glow animation
    el.querySelector('.slnctrz-ft-elapsed').textContent = this._formatTime(data.elapsed);
  ...
  _renderPipelineRow(pipeline) {
    var row = this._el.querySelector('.slnctrz-ft-pipeline');
  ...
  _renderJobs(jobs) {
    var jobsEl = this._el.querySelector('.slnctrz-ft-jobs');
    ...
    for (var i = 0; i < jobs.length; i++) {
      ...
      // Job header row
      html += '<div class="slnctrz-ft-job-header" data-job-toggle="' + j.id + '" style="display:flex;align-items:center;gap:6px;cursor:pointer;padding:3px 0;">';
      html += '<span class="' + (isActive && !isPaused ? 'slnctrz-ft-dot-pulse' : '') + '" style="width:8px;height:8px;border-radius:50%;background:' + color + ';flex-shrink:0;"></span>';
...
var ExecutionBlocker = {
  ...
  _blockEvent(e) {
    ...
    if (e.target && e.target.closest && e.target.closest('#slnctrz-flow-tracker')) return;
  ...
  _injectStyles() {
    ...
    var style = document.createElement('style');
    style.id = 'slnctrz-execution-blocker-styles';
    ...
    style.textContent = `
      ...
      @keyframes slnctrz-glow-paused {
        0%, 100% { opacity: 0.3; }
        50% { opacity: 0.5; }
      }

      #slnctrz-execution-blocker {
        position: fixed;
        inset: 0;
        z-index: 2147483647;
        pointer-events: all;
      ...
      #slnctrz-execution-blocker::before {
        content: '';
        position: absolute;
        inset: 0;
        ...
        animation: slnctrz-glow-pulse 1.8s ease-in-out infinite;
        will-change: opacity;
        pointer-events: none;
      }

      /* Paused state - slower animation, yellow tint */
      #slnctrz-execution-blocker.slnctrz-paused::before {
        border-color: #facc15;
        box-shadow:
        ...
        animation: slnctrz-glow-paused 3s ease-in-out infinite;
      }

      /* Force Flow context menus (Radix UI) unclickable when blocker active */
      body.slnctrz-execution-blocking [data-radix-popper-content-wrapper],
      body.slnctrz-execution-blocking [role="menu"][data-state="open"] {
  ...
  _attachBlockers() {
    ...
    document.body.classList.add('slnctrz-execution-blocking');
  },
  ...
  _detachBlockers() {
    ...
    document.body.classList.remove('slnctrz-execution-blocking');
  },
  ...
  show(options) {
    ...
    var el = document.createElement('div');
    el.id = 'slnctrz-execution-blocker';
    document.body.appendChild(el);
    this._el = el;
  },
  ...
  setPaused(paused) {
    if (!this._el) return;
    if (paused) {
      this._el.classList.add('slnctrz-paused');
    } else {
      this._el.classList.remove('slnctrz-paused');
    }
  },
...
function getDownloadSettings() {
  return new Promise(resolve => {
    chrome.storage.local.get(['af_settings'], (res) => {
      ...
      resolve({
        folder: s.downloadFolder || 'slnctrz_output',
...
function _buildFilename({ template, project, prompt, index, taskName, folder, ext }) {
  ...
  // Build full path
  const baseFolder = folder || 'slnctrz_output';
...
async function downloadTileMedia(tileId, promptText, taskName, fileName, resolution, flowFileId, index, videoResolution) {
  ...
  console.log(`[SlncTrZ] downloadTileMedia: attempting Flow menu for ${tileId.substring(0, 20)}, res=${res}`);
  const menuSuccess = await downloadViaFlowMenu(tileId, res, fileName, promptText, taskName, index);
  if (menuSuccess) {
    console.log(`[SlncTrZ] downloadTileMedia: Flow menu SUCCESS for ${tileId.substring(0, 20)}`);
    return true;
  }
  console.log(`[SlncTrZ] downloadTileMedia: Flow menu FAILED for ${tileId.substring(0, 20)}, trying legacy fallback`);
  ...
  if (legacySuccess) {
    console.log(`[SlncTrZ] downloadTileMedia: Legacy fallback SUCCESS for ${tileId.substring(0, 20)}`);
    return true;
  }
  console.warn(`[SlncTrZ] downloadTileMedia: BOTH methods FAILED for ${tileId.substring(0, 20)}`);
  return false;
}
...
async function _downloadTileMediaLegacy(tileId, promptText, taskName, fileName, resolution, index) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (attempt < MAX_RETRIES - 1) {
      console.log(`[SlncTrZ] _downloadTileMediaLegacy: tile not found, retry ${attempt + 1}/${MAX_RETRIES}...`);
      await sleep(RETRY_INTERVAL);
    }
  }

  if (!tile) {
    console.warn(`[SlncTrZ] _downloadTileMediaLegacy: tile not found after ${MAX_RETRIES} retries: ${tileId.substring(0, 30)}`);
    return false;
  }
  ...
  if (fileName) {
    ...
    if (currentFn && currentFn !== fileName) {
      console.warn(`[SlncTrZ] _downloadTileMediaLegacy: cross-project collision`);
  ...
  if (!mediaEl) {
    console.warn(`[SlncTrZ] _downloadTileMediaLegacy: tile media not ready after ${mediaTimeout / 1000}s, tileId=${tileId.substring(0, 30)}`);
    return false;
  }
  ...
  try {
    ...
    if (!mediaSrc ||
      ...
      console.warn(`[SlncTrZ] _downloadTileMediaLegacy: invalid/placeholder URL rejected: ${mediaSrc?.substring(0, 80)}`);
      return false;
    }
    ...
    console.log(`[SlncTrZ] _downloadTileMediaLegacy: downloading via chrome.downloads: ${mediaSrc.substring(0, 100)}...`);
    ...
    if (response.success) {
      ...
      console.warn(`[SlncTrZ] _downloadTileMediaLegacy: chrome.downloads failed: ${response.error}`);
      return false;
    }
  } catch (e) {
    console.warn(`[SlncTrZ] _downloadTileMediaLegacy: exception: ${e.message}`);
...
async function downloadViaFlowMenu(tileId, resolution, fileName, promptText, taskName, index) {
  ...
  for (let attempt = 0; attempt < MAX_TILE_RETRIES; attempt++) {
    ...
    if (attempt < MAX_TILE_RETRIES - 1) {
      console.log(`[SlncTrZ] downloadViaFlowMenu: tile not found, retry ${attempt + 1}/${MAX_TILE_RETRIES} in ${TILE_RETRY_INTERVAL}ms...`);
      await sleep(TILE_RETRY_INTERVAL);
    }
  }

  if (!tile) {
    console.warn(`[SlncTrZ] downloadViaFlowMenu: tile not found after ${MAX_TILE_RETRIES} retries: ${tileId.substring(0, 20)}`);
    return false;
  }
  ...
  if (fileName) {
    const currentFn = extractFileName(tile);
    if (currentFn && currentFn !== fileName) {
      console.warn(`[SlncTrZ] downloadViaFlowMenu: cross-project collision`);
      sendLog(`Tile ${tileId.substring(0, 15)}... thuộc project khác, bỏ qua.`, 'warn');
      return false;
    }
  }
  ...
  if (!readyMedia) {
    console.warn(`[SlncTrZ] downloadViaFlowMenu: tile media not ready after ${mediaTimeout / 1000}s, tileId=${tileId.substring(0, 20)}`);
    return false;
  }
  ...
  try {
    ...
    console.log(`[SlncTrZ] downloadViaFlowMenu: right-click on ${targetEl.tagName} at (${Math.round(cx)}, ${Math.round(cy)}), rect: ${Math.round(rect.width)}x${Math.round(rect.height)}`);
    ...
    if (!contextMenu) {
      console.warn('[SlncTrZ] downloadViaFlowMenu: context menu not found');
      return false;
    }
    await sleep(50);
    ...
    if (!downloadItem) {
      console.warn('[SlncTrZ] downloadViaFlowMenu: download menu item not found');
      _closeContextMenu();
      return false;
    }
    ...
    if (!subMenu) {
      console.warn('[SlncTrZ] downloadViaFlowMenu: sub-menu not found after 5 attempts');
      _closeContextMenu();
      return false;
    }
      ...
      for (const item of subItems) {
        ...
        if (text.startsWith(tryLabel)) {
          if (item.getAttribute('aria-disabled') === 'true') {
            console.log(`[SlncTrZ] downloadViaFlowMenu: ${tryLabel} is disabled (aria-disabled), trying lower resolution`);
    ...
    if (!targetItem) {
      ...
      console.warn(`[SlncTrZ] downloadViaFlowMenu: ${resLabel} not found in sub-menu. Available: [${availableItems}]`);
      for (const item of subItems) {
        if (item.getAttribute('aria-disabled') !== 'true') {
          targetItem = item;
      ...
      if (!targetItem) {
        console.warn('[SlncTrZ] downloadViaFlowMenu: no downloadable option found');
        _closeContextMenu();
        return false;
      }
    }
    ...
    if (mediaForValidation) {
      ...
      if (isPlaceholder) {
        console.warn('[SlncTrZ] downloadViaFlowMenu: media URL is placeholder after menu opened, aborting download');
        _closeContextMenu();
        return false;
      }
    }
    ...
    console.error('[SlncTrZ] downloadViaFlowMenu error:', e);
    _closeContextMenu();
    return false;
  } finally {
    _releaseCtxMenuLock();
...
function _waitForTileMediaReady(tile, timeoutMs = 10000, preferVideo = false) {
  return new Promise(resolve => {
    ...
    const check = () => {
      ...
      if (status === 'failed') {
        console.warn('[SlncTrZ] _waitForTileMediaReady: tile is failed, skip download');
        resolve(null);
        return;
      }
      ...
      if (preferVideo) {
        ...
        if (!media && status === 'success' && (Date.now() - startTime >= videoFallbackMs)) {
          ...
          if (img) {
            ...
            if (isValidUrl) {
              console.warn('[SlncTrZ] _waitForTileMediaReady: video not in DOM after ' + Math.round(videoFallbackMs / 1000) + 's, falling back to img early');
              resolve(img);
              return;
            }
          }
        ...
        if (preferVideo) {
          ...
          if (img) {
            ...
            const isValidUrl = imgSrc.startsWith('http') &&
              ...
              !imgRaw.endsWith('.html');
            if (isValidUrl) {
              console.warn('[SlncTrZ] _waitForTileMediaReady: video not ready, falling back to img');
              resolve(img);
              return;
            }
          }
...
function clickTileRetryButton(tileId) {
  ...
  if (propsKey && retryBtn[propsKey] && typeof retryBtn[propsKey].onClick === 'function') {
    try {
      ...
      console.warn('[SlncTrZ] React onClick retry failed:', e.message);
    }
  }
...
async function _acquireRetryMutex() {
  ...
  while (_retryMutex) {
    if (Date.now() - startWait > maxWait) {
      console.warn('[SlncTrZ] Retry mutex stuck > 120s, force release');
      _retryMutex = null;
      break;
    }
    await _retryMutex;
...
async function retryFailedTilesViaButton(failedTileIds, timeoutMs = 120000, excludeTileIds = null) {
  ...
  if (alreadyClicked.length > 0) {
    console.log(`[SlncTrZ] ${alreadyClicked.length} tile đã click retry trước đó (skip để tránh tạo dư):`, alreadyClicked.map(t => t.substring(0, 12)));
  ...
  if (toRetry.length === 0) {
    console.log('[SlncTrZ] Tất cả tiles đã đang được retry hoặc đã click retry, skip');
    return {
      succeeded: [],
  ...
  try {
    ...
    for (const tileId of toRetry) {
      ...
      if (currentStatus === 'success') {
        console.log(`[SlncTrZ] Retry skipped: tile ${tileId.substring(0, 12)}... đã success (transient 'failed' state)`);
        succeeded.push(tileId);
        continue;
      }
      if (currentStatus === 'processing') {
        console.log(`[SlncTrZ] Retry skipped: tile ${tileId.substring(0, 12)}... đang processing`);
      ...
      if (!clicked) {
        console.warn('[SlncTrZ] Retry button not found for tile:', tileId);
        stillFailed.push(tileId);
        continue;
      }
      ...
      if (newTiles.length > 1) {
        console.log(`[SlncTrZ] Retry single tile got ${newTiles.length} tiles, taking oldest`);
      ...
      if (newTiles.length > 0) {
        ...
        if (status === 'failed') {
          ...
          console.log(`[SlncTrZ] Retry tile ${tileId.substring(0, 12)}... → new tile ${newTileId.substring(0, 12)}... FAILED`);
        } else if (status === 'success') {
          succeeded.push(newTileId);
          console.log(`[SlncTrZ] Retry tile ${tileId.substring(0, 12)}... → new tile ${newTileId.substring(0, 12)}... SUCCESS`);
        } else {
          ...
          stillFailed.push(newTileId);
          console.log(`[SlncTrZ] Retry tile ${tileId.substring(0, 12)}... → new tile ${newTileId.substring(0, 12)}... unexpected PROCESSING status, treat as failed`);
        }
      } else {
        ...
        console.log(`[SlncTrZ] Retry tile ${tileId.substring(0, 12)}... → cả 2 phase timeout (~120s), skip để tránh tạo dư`);
      }
...
async function refreshFlowSession(trigger) {
  ...
  try {
    ...
    if (result?.success) {
      console.log('[SlncTrZ][FAR-1] Refresh OK', { trigger, durationMs });
    ...
    console.warn('[SlncTrZ][FAR-1] Refresh FAIL', { trigger, durationMs, error: result?.error || 'unknown' });
    try { _incrementDailyStat('flow_refresh_fail'); } catch (e) {}
    return false;
  } catch (e) {
    const durationMs = Date.now() - startedAt;
    console.warn('[SlncTrZ][FAR-1] Refresh exception', { trigger, durationMs, error: e.message });
    try { _incrementDailyStat('flow_refresh_fail'); } catch (e) {}
    return false;
  }
}
...
function _slateBridgeCall(action, detail) {
  return new Promise((resolve) => {
    ...
    console.log('[SlncTrZ] Bridge call:', action, 'rid:', requestId);
    let resolved = false;
    const handler = (e) => {
      ...
      if (e.data && e.data.source === 'flow-auto-slate-result' && e.data.requestId === requestId) {
        ...
        console.log('[SlncTrZ] Bridge response:', action, e.data.success, e.data.error || '');
    ...
    setTimeout(() => {
      if (!resolved) {
        window.removeEventListener('message', handler);
        console.warn('[SlncTrZ] Bridge TIMEOUT:', action, requestId);
...
async function clearEditor(editor) {
  ...
  if (result.success) {
    console.log('[SlncTrZ] clearEditor: Slate bridge OK (1 trong 3 inner tier work)');
    await sleep(200); // Đợi React re-render
    return;
  }
  // Cả 3 inner tier Slate fail → abort. KHÔNG dùng DOM fallback (xem JSDoc trên).
  console.warn('[SlncTrZ] clearEditor: ❌ Slate bridge failed (cả 3 inner tier):', result.error);
  console.warn('[SlncTrZ] clearEditor: ⚠️ Aborting — no DOM fallback (tránh crash Flow page)');
}
...
async function _insertTextSingle(text) {
  ...
  if (result.success) {
    console.log('[SlncTrZ] insertText: Slate bridge returned success (1 trong 3 inner tier work)');
    ...
    while (waited < maxWaitDom) {
      ...
      if (isDomUpdated()) {
        console.log('[SlncTrZ] insertText: DOM verified OK after', waited, 'ms');
        return;
      }
      // Check text content thay vì chỉ placeholder
      if (isTextAlreadyInserted()) {
        console.log('[SlncTrZ] insertText: Text already in editor after', waited, 'ms');
        return;
      }
    }
    ...
    console.log('[SlncTrZ] insertText: Slate bridge success BUT DOM not updated after 2s');
    console.log('[SlncTrZ] insertText: Skipping DOM fallbacks to avoid Slate state corruption');
  ...
  console.warn('[SlncTrZ] insertText: ❌ Slate bridge failed (cả 3 inner tier):', result.error);
  console.warn('[SlncTrZ] insertText: ⚠️ Aborting — no DOM fallback (tránh crash Flow page)');
}
...
async function addFileToPrompt(fileId, fileName, flowFileId) {
    ...
    if (els.length > 0 && fileName) {
        ...
        const currentFn = extractFileName(tile);
        if (currentFn && currentFn !== fileName) {
            console.log(`[SlncTrZ] addFileToPrompt: tile_id ${fileId.substring(0, 20)}... exists but file_name mismatch (${currentFn.substring(0, 15)}... vs ${fileName.substring(0, 15)}...) → cross-project collision, finding by file_name`);
    ...
    if (els.length === 0 && fileName) {
        ...
        for (const tile of allTiles) {
            ...
            if (fn === fileName) {
                ...
                console.log(`[SlncTrZ] addFileToPrompt: resolved ${fileId.substring(0, 20)}... → ${newId.substring(0, 20)}... via file_name`);
                fileId = newId;
...
async function runAutoPrompt(payload) {
  ...
  if (!shouldStop) {
    ...
    notifyCompletion('SlncTrZ VM', `Hoàn tất: ${completedCount}/${prompts.length} prompts`);
  }
...
async function ensureFlowSettingsApplied() {
  ...
  if (_flowSettingsApplied) {
    console.log('[SlncTrZ] ensureFlowSettingsApplied: SKIPPED (already applied in this session)');
    return;
  }
  ...
  if (!flowSettingsBtn) {
    ...
    console.log('[SlncTrZ] ensureFlowSettingsApplied: settings_2 icon chưa có — Flow page chưa ready, retry next call');
    return;
  }

  console.log('[SlncTrZ] ensureFlowSettingsApplied: applying Grid view + show tile details (1-time)...');
  ...
  console.log(`[SlncTrZ] ensureFlowSettingsApplied: DONE (gridClicked=${gridClicked}, detailsClicked=${detailsClicked})`);
}
...
async function ensureFlowTilesLoaded() {
  ...
  if (tilesBeforeCount >= 50) {
    console.log(`[SlncTrZ] ensureFlowTilesLoaded: skipping zoom (${tilesBeforeCount} tiles already loaded)`);
    return;
  }
  ...
  console.log(`[SlncTrZ] ensureFlowTilesLoaded: ${tilesBeforeCount} → ${tilesAfterLoad} tiles (after zoom)`);
}
...
try { injectSlateBridge(); } catch(e) { console.log('[SlncTrZ] Bridge inject deferred:', e.message); }

chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  // Ping handler — background.js dùng để check content.js đã load chưa trước khi auto-inject
  if (message.action === 'ping') {
    sendResponse({ pong: true, loadedAt: self.__slnctrzContentLoaded__ });
  ...
  const handlers = {
    ...
    'correctStaleFileIds': async () => {
      ...
      function scanByThumbnailUrl(idsToCheck) {
        ...
        for (const oldId of idsToCheck) {
          ...
          if (savedFn && tileFn && savedFn !== tileFn) {
            console.warn(`[SlncTrZ] [Tầng 2] Cross-project collision detected: oldId=${oldId.substring(0,20)}... savedFn=${savedFn.substring(0,20)}... vs tileFn=${tileFn.substring(0,20)}...`);
          ...
          if (savedFn && !tileFn) {
            console.warn(`[SlncTrZ] [Tầng 2] Skipping correction: savedFn=${savedFn.substring(0,20)}... but tile has no file_name yet`);
            continue;
          }
      ...
      for (const id of allIds) {
        ...
        if (tile) {
          ...
          if (validateTileIdWithFileName(id, savedFn)) {
            ...
            console.log(`[SlncTrZ] Cross-project detected: tile_id ${id.substring(0, 20)}... exists but file_name mismatch`);
      ...
      console.log(`[SlncTrZ] correctStaleFileIds: ${allIds.length} IDs check | ${validIds.length} valid | ${needsCorrectionIds.length} need correction | ${totalTiles} tiles trên DOM`);
      ...
      if (tier0Count > 0) {
        console.log(`[SlncTrZ] [Tầng 0 file_id]: ${tier0Count}/${needsCorrectionIds.length} corrected`);
      }
      ...
      if (fnCount > 0) {
        ...
        console.log(`[SlncTrZ] [Tầng 1 file_name]: ${fnCount}/${afterTier0.length} corrected`);
      }
      ...
      if (uncorrected.length > 0) {
        ...
        if (urlCount > 0) {
          ...
          console.log(`[SlncTrZ] [Tầng 2 thumbnail_url]: ${urlCount}/${uncorrected.length} corrected`);
        }
        if (urlCrossProject.length > 0) {
          allCrossProjectIds.push(...urlCrossProject);
          console.log(`[SlncTrZ] [Tầng 2 cross-project]: ${urlCrossProject.length} detected`);
        }
      ...
      if (uncorrected.length > 0) {
        console.log(`[SlncTrZ] [Tầng 3]: còn ${uncorrected.length} IDs chưa match, gọi ensureFlowTilesLoaded...`);
        await ensureFlowTilesLoaded();
        const tilesAfterLoad = document.querySelectorAll('[data-tile-id]').length;
        console.log(`[SlncTrZ] [Tầng 3]: sau ensureFlowTilesLoaded: ${tilesAfterLoad} tiles trên DOM`);
        ...
        console.log(`[SlncTrZ] [Tầng 3] retry: ${uncorrected.length - finalUncorrected.length} thêm | ${finalUncorrected.length} vẫn missing`);
      }

      // Dedupe crossProjectIds
      allCrossProjectIds = [...new Set(allCrossProjectIds)];

      console.log(`[SlncTrZ] correctStaleFileIds KẾT QUẢ: ${Object.keys(corrections).length}/${needsCorrectionIds.length} corrected, ${allCrossProjectIds.length} cross-project`);
      for (const [oldId, newId] of Object.entries(corrections)) {
        console.log(`[SlncTrZ]   ${oldId.substring(0, 30)}... → ${newId.substring(0, 30)}...`);
      }
      if (allCrossProjectIds.length > 0) {
        console.log(`[SlncTrZ] Cross-project IDs:`, allCrossProjectIds.map(id => id.substring(0, 25) + '...'));
      }
    ...
    'startCropSelection': () => {
      return new Promise((resolve) => {
        ...
        overlay.className = 'slnctrz-crop-overlay';
        overlay.innerHTML = `
        ...
        if (!document.getElementById('slnctrz-crop-styles')) {
          const style = document.createElement('style');
          style.id = 'slnctrz-crop-styles';
          style.textContent = `
            ...
            .slnctrz-crop-btn svg {
              flex-shrink: 0;
            }
            .slnctrz-crop-btn-capture {
              background: #cdff01;
              color: #1c1c1f;
            }
            .slnctrz-crop-btn-capture:hover {
              background: #d4ff33;
              transform: scale(1.02);
            }
            .slnctrz-crop-btn-cancel {
              background: rgba(40,40,45,0.95);
              color: #fff;
              border: 1px solid rgba(255,255,255,0.15);
            }
            .slnctrz-crop-btn-cancel:hover {
    ...
    'cancelCropSelection': () => {
      const overlay = document.getElementById('slnctrz-crop-overlay');
    ...
    'providerConfigUpdated': () => {
      ...
      console.log('[SlncTrZ] Provider config updated via SSE — selector cache invalidated');
      return { success: true };
    },
    // === API configs (download_resolutions, ratios, error_patterns) ===
    'providerApiConfigUpdated': () => {
      ...
      console.log('[SlncTrZ] Provider api_config updated via SSE — api configs cache invalidated');
      return { success: true };
    },
    // === n8n bridge: follow redirect để lấy CDN URL public ===
    'resolveMediaUrl': async () => {
      const url = message.url;
      if (!url || !url.startsWith('http')) return { url: null, error: 'Invalid URL' };
      try {
        const resp = await fetch(url, { method: 'GET', redirect: 'follow' });
        const finalUrl = resp.url;
        if (!resp.ok) return { url: null, error: 'HTTP ' + resp.status };
        const contentType = resp.headers.get('content-type') || '';
        if (finalUrl && finalUrl !== url) {
          console.log('[SlncTrZ] resolveMediaUrl: ' + url.substring(0, 60) + ' → ' + finalUrl.substring(0, 80));
        }
    ...
    // === n8n bridge: lấy media URL từ tile, đợi đến khi load xong ===
    'waitForTileMediaUrl': async () => {
      const tileId = message.tileId;
      if (!tileId) return { url: null, error: 'No tileId' };
      const tile = _getTileById(tileId);
      if (!tile) return { url: null, error: 'Tile not found' };
      const isVideo = message.isVideo || false;
      try {
        const mediaEl = await _waitForTileMediaReady(tile, 15000, isVideo);
        if (!mediaEl) return { url: null, error: 'Media not ready' };
        let url = mediaEl.src || '';
        if (isVideo && (!url || url.startsWith('blob:'))) {
          const videoUrl = extractVideoUrl(tile);
          if (videoUrl) url = videoUrl;
        }
        if (!url || url.startsWith('blob:') || url.includes('chrome-extension')) {
          return { url: null, error: 'Invalid URL: ' + (url ? url.substring(0, 60) : 'empty') };
        }
        ...
        btn.addEventListener('click', (e) => {
            ...
            try {
                ...
                if (!chrome.runtime?.id) {
                    console.warn('[SlncTrZ] Extension context invalidated, reloading page...');
                    location.reload();
                    return;
                }
                chrome.runtime.sendMessage({
                    ...
                    if (chrome.runtime.lastError) {
                        ...
                        if (errMsg.includes('Extension context invalidated') || errMsg.includes('Receiving end does not exist')) {
                            console.warn('[SlncTrZ] Extension reloaded, reloading page...');
                            location.reload();
                        } else {
                ...
                if (errMsg.includes('Extension context invalidated')) {
                    ...
                    console.error('[SlncTrZ] addImageToGenTab error:', err);
                }
            }
        });
