...
async function resolveModelName(value: AiType | `${string}:${string}`): Promise<`${string}:${string}`> {
  if (AiTypeValues.includes(value as AiType)) {
    ...
    if (agentUseModeVal?.value == "1") {
      ...
      if (!agentDeployData?.modelName) throw new Error(`Advanced config mode: model config not found for ${value}`);
    ...
    if (agentUseModeVal?.value == "0") {
      ...
      if (!mainlyData?.modelName) throw new Error(`Simple config mode: deployment config not found for ${value}`);
      return mainlyData?.modelName as `${number}:${string}`;
    }

    // agentUseModeVal not found, use legacy logic
    const agentDeployData = await u.db("o_agentDeploy").where("key", value).first();
    let modelName = null;
    ...
    if (!agentDeployData?.modelName) {
      ...
      const mainlyData = await u.db("o_agentDeploy").where("key", mainly).first();
      if (!mainlyData?.modelName) throw new Error(`Deployment config not found for ${value}`);
...
async function getModelConfig(value: AiType | `${string}:${string}`) {
  if (AiTypeValues.includes(value as AiType)) {
    ...
    // agentUseModeVal not found, use legacy flow
    const agentDeployData = await u.db("o_agentDeploy").where("key", value).first();
...
async function getVendorTemplateFn(fnName: FnName, modelName: `${string}:${string}`): Promise<any> {
  ...
  if (!vendorConfigData) throw new Error(`Vendor config not found id=${id}`);
  const modelList = await u.vendor.getModelList(id);
  const selectedModel = modelList.find((i: any) => i.modelName == name);
  if (!selectedModel) throw new Error(`Model not found ${name} id=${id}`);
  ...
  if (!fn) throw new Error(`Function not found in vendor config: ${fnName} id=${id}`);
...
interface TaskRecord {
  taskClass: string; // Task classification
  describe: string; // Task description
  relatedObjects: string; // Related object info for traceability
...
type VideoMode =
  ...
  | "endFrameOptional" // Start & end frames (end optional)
  | "startFrameOptional" // Start & end frames (start optional)
  | "text" // Text only
  | (`videoReference:${number}` | `imageReference:${number}` | `audioReference:${number}`)[]; // Multi-reference (number = limit)
