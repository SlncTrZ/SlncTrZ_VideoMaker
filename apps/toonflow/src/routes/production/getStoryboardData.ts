...
export default router.post(
  ...
  async (req, res) => {
    ...
    // Fix: o_assets.id references o_assets2Storyboard.assetId, filter by storyboardId
    ...
    // Group by storyboardId, generate characters list
    ...
    // Assemble final data, conforms to Shot interface format
    const result = await Promise.all(
      data.map(async (item) => {
        const characters = storyboardCharactersMap[item.id as number] ?? [];
        // Process avatar OSS paths in characters
