...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (!ids.length) return res.status(400).send(error("Please select storyboards first"));
    const storyboardDataList = await u.db("o_storyboard").whereIn("id", ids).where("projectId", projectId).select("id", "track", "trackId", "flowId");
    if (!storyboardDataList.length) return res.status(400).send(error("Selected storyboards do not exist"));
