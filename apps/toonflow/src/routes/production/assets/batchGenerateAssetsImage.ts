...
export default router.post(
  ...
  async (req, res) => {
    ...
    // Batch create image records for all assets and mark as "generating"
    ...
    res.status(200).send(success("Start generating asset images"));
    const generateSingleAsset = async (item: any) => {
        ...
        messages: [
          {
            role: "user",
            content: `
            Parent asset description: ${item.parentDescribe || "No detailed description"}
            Current asset description: ${item.describe || "No detailed description"}`,
          },
        ],
      });
      ...
      try {
        ...
        const imageCls = await u.Ai.Image(projectSettingData?.imageModel as `${string}:${string}`).run(
          ...
          {
            taskClass: "generate_image",
            describe: "Asset image generation",
        ...
        await u
          .db("o_image")
          .where({ id: imageId })
          .update({ state: "generation_failed", errorReason: u.error(e).message });
        return {
          id: item.id!,
    ...
    // Batch concurrent execution by concurrentCount
