...
interface VideoItem {
  id: number;
  src: string;
  state: "not_generated" | "generating" | "completed" | "generation_failed";
}
...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (!projectData?.videoModel) {
      return res.status(400).json(success("Project has not configured a video model"));
    }
    let videoMode = "";
    try {
    ...
    // Asset data grouped by storyboardId, key is storyboardId
    const otherDataMap: Record<number, any[]> = {};
    // Parse the number of audioReferences in videoMode, e.g. 'audioReference:3' => 3
    ...
    for (const trackId of trackIdMap) {
      ...
      trackList.push({
        ...
        prompt: item?.prompt || "",
        state: (item?.state as "not_generated" | "generating" | "completed" | "generation_failed") ?? "not_generated",
        reason: item?.reason ?? "",
        ...
        medias: (() => {
          ...
          // When audioReference is present, truncate audio assets by count
          ...
          const filteredAssets = uniqueAssets.filter((a) => {
            ...
            // Count total audio in current track, filter if exceeds limit
        ...
        videoList: await Promise.all(
          videoList
            ...
            .map(async (v) => ({
              ...
              state: v.state === "completed" ? "completed" : v.state === "generating" ? "generating" : v.state === "generation_failed" ? "generation_failed" : "not_generated",
