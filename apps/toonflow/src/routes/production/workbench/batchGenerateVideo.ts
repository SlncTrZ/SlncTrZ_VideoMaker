...
export default router.post(
  ...
  async (req, res) => {
    ...
    // Preprocess data for each track and insert into database, return task list
    ...
    for (const { videoId, videoPath, prompt, duration, images } of tasks) {
      // All tasks execute concurrently in background, completely non-blocking
      ...
      aiVideo
        .run(
          ...
          },
          {
            ...
            describe: "Generate video from prompt",
            relatedObjects: JSON.stringify(relatedObjects),
          },
        )
        .then(async () => await aiVideo.save(videoPath))
        .then(async () => await u.db("o_video").where("id", videoId).update({ state: "generation_successful" }))
