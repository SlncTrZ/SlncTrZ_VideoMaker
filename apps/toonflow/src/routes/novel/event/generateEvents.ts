...
// Clean novel text and generate event list
export default router.post(
  ...
  async (req, res) => {
    ...
    if (allChapters.length === 0) {
      return res.status(400).send(success("No corresponding chapters"));
    }
    ...
    return res.status(200).send(success("Events generated successfully"));
  },
);
