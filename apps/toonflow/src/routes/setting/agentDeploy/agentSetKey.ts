...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (!vendorConfigData.inputValues) return res.status(500).send(error("Model config data not found"));
    ...
    try {
      const resText = await u.Ai.Text(`toonflow:claude-haiku-4-5-20251001`).invoke({
        prompt: "What is 1+1? Just answer 2, no explanation",
      });
      if (resText.text) {
        ...
        res.status(200).send(success("Auto-filled successfully"));
      }
    } catch (err) {
      console.error(err);
      inputValue.apiKey = "";
      ...
      res.status(400).send(error("Invalid KEY, please re-enter"));
    }
  },
);
