...
const APP_VERSION: string = (() => {
  ...
  // Dev fallback: read from package.json
...
export default router.post(
  ...
  async (req, res) => {
    ...
    if (!versionInfo) return res.status(400).send(error("Unable to get version info"));
    const { version: tagger, time, data } = versionInfo;

    const sourceData = data[source];
    if (!sourceData) return res.status(400).send(error("Unable to get download info for this source"));
    ...
    if (taggerList[0] > currentVersionList[0]) {
      if (!installerItem) return res.status(400).send(error("No installer for current system from this source"));
      return res
        .status(200)
    ...
    if (taggerList[2] > currentVersionList[2]) {
      if (!zipItem) return res.status(400).send(error("No incremental update package from this source"));
