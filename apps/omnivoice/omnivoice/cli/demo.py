...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--port", type=int, default=8881, help="Server port (default: 8881)."
    )
    parser.add_argument(
        "--root-path",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Voice profile directory. Default: ~/Omnivoice/my_profiles",
    )
    return parser
...
def build_demo(
    ...
    # ── Profile directory ────────────────────────────────────────────────────
    _profiles_dir = Path(
        profile_dir or (Path(platformdirs.user_data_dir("omnivoice")) / "profiles")
    )
    _AUDIO_EXTS = {".wav", ".mp3", ".m4a", ".flac", ".ogg"}

    def _find_audio_file(pdir: Path):
    ...
    def _list_profiles():
        if not _profiles_dir.exists():
            return []
        return sorted(
            p.name for p in _profiles_dir.iterdir()
            if p.is_dir() and _find_audio_file(p)
        )

    def _get_profile_meta(profile_id):
        safe = "".join(c for c in profile_id if c.isalnum() or c in "-_")
        pdir = _profiles_dir / safe
        ...
        if meta_file.exists():
            try:
                ...
                pass
        txt_file = pdir / "ref_text.txt"
        if txt_file.exists():
            try:
                return {"ref_text": txt_file.read_text(encoding="utf-8").strip()}
            except Exception:
                return None
        return None

    def _save_profile(profile_id, audio_src, ref_text):
        if not profile_id or not profile_id.strip():
            return "Enter a profile ID."
        if not audio_src:
            return "Upload a reference audio file."
        safe = "".join(c for c in profile_id if c.isalnum() or c in "-_")
        if not safe:
            return "Invalid profile ID (alphanumeric, dashes, underscores only)."
        pdir = _profiles_dir / safe
        pdir.mkdir(parents=True, exist_ok=True)
        src_ext = Path(audio_src).suffix.lower()
        dst = pdir / f"ref_audio{src_ext}"
...
"""
        ...
        with gr.Tabs():
            ...
            # Saved Profiles
            # ==============================================================
            with gr.TabItem("Saved Profiles / Hồ sơ giọng nói"):
                with gr.Row():
                    with gr.Column(scale=1):
                        gr.Markdown("### Manage / Quản lý")
                        sp_refresh = gr.Button("🔄 Refresh / Làm mới")
                        sp_dropdown = gr.Dropdown(
                            label="Select Profile / Chọn hồ sơ",
                            choices=[],
                            interactive=True,
                        )
                        sp_audio = gr.Audio(
                        ...
                        sp_text_display = gr.Textbox(
                            label="Reference Text", lines=2, interactive=False
                        )
                        with gr.Row():
                            sp_delete_btn = gr.Button(
                                "🗑 Delete / Xoá", variant="stop"
                            )
                        gr.Markdown("---")
                        gr.Markdown("### Save New / Lưu mới")
                        sp_new_id = gr.Textbox(
                            label="Profile ID / Tên hồ sơ",
                            placeholder="e.g. my-voice-1",
                        )
                        sp_new_audio = gr.Audio(
                            label="Upload Reference Audio / Tải giọng mẫu",
                            type="filepath",
                            elem_classes="compact-audio",
                        )
                        sp_new_text = gr.Textbox(
                            label="Reference Text (optional)", lines=2
                        )
                        sp_save_btn = gr.Button(
                            "💾 Save Profile / Lưu", variant="primary"
                        )
                        sp_status = gr.Textbox(label="Status / Trạng thái", lines=1)

                    with gr.Column(scale=1):
                        gr.Markdown("### Synthesize / Tổng hợp")
                        sp_text = gr.Textbox(
                            label="Text to Synthesize / Văn bản",
                            lines=4,
                        ...
                        with gr.Accordion("Generation Settings", open=False):
                            sp_ns = gr.Slider(
                                4, 64, value=32, step=1, label="Steps"
                            )
                            sp_gs = gr.Slider(
                                0.0, 4.0, value=2.0, step=0.1, label="Guidance"
                            )
                            sp_dn = gr.Checkbox(label="Denoise", value=True)
                            sp_sp = gr.Slider(
                        ...
                        sp_gen_btn = gr.Button(
                            "🎤 Generate / Tổng hợp", variant="primary"
                        )
                        sp_output = gr.Audio(
                            label="Output Audio / Kết quả", type="numpy"
                        )
                        sp_out_status = gr.Textbox(
                            label="Status / Trạng thái", lines=2
                        )

                # ── Event handlers ───────────────────────────────────────────

                def _refresh():
                    return gr.Dropdown(choices=_list_profiles() or [])

                def _on_select(profile_id):
                    if not profile_id:
                        return None, ""
                    pdir = _profiles_dir / profile_id
                    audio_file = _find_audio_file(pdir)
                    audio_val = str(audio_file) if audio_file else None
                    meta = _get_profile_meta(profile_id)
                    ref_text = (meta or {}).get("ref_text") or ""
                    return audio_val, ref_text

                def _on_save(profile_id, audio_path, ref_text):
                    msg = _save_profile(profile_id, audio_path, ref_text)
                    choices = _list_profiles() or []
                    return msg, gr.Dropdown(choices=choices)

                def _on_delete(profile_id):
                    msg = _delete_profile(profile_id)
                    choices = _list_profiles() or []
                    return msg, gr.Dropdown(choices=choices, value=None), None, ""
                ...
                def _sp_gen(
                    ...
                    if not profile_id:
                        return None, "Select a profile first."
                    pdir = _profiles_dir / profile_id
                    audio_file = _find_audio_file(pdir)
                    if not audio_file:
                        return None, "Profile audio file not found."
                    meta = _get_profile_meta(profile_id)
                    ref_text = (meta or {}).get("ref_text")
                    return _gen(
                        text,
                        lang,
                        str(audio_file),
                ...
                sp_refresh.click(fn=_refresh, outputs=[sp_dropdown])
                sp_dropdown.change(
                    fn=_on_select,
                    inputs=[sp_dropdown],
                    outputs=[sp_audio, sp_text_display],
                )
                sp_save_btn.click(
                    fn=_on_save,
                    inputs=[sp_new_id, sp_new_audio, sp_new_text],
                    outputs=[sp_status, sp_dropdown],
                )
                sp_delete_btn.click(
                    fn=_on_delete,
                    inputs=[sp_dropdown],
                    outputs=[sp_status, sp_dropdown, sp_audio, sp_text_display],
                )
                sp_gen_btn.click(
                    fn=_sp_gen,
                    inputs=[
                    ...
                    outputs=[sp_output, sp_out_status],
                )
...
def main(argv=None) -> int:
    ...
    profile_dir = args.profile_dir or str(
        Path.home() / "Omnivoice" / "my_profiles"
    )
    demo = build_demo(model, checkpoint, profile_dir=profile_dir)
